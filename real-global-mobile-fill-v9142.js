...
const VERSION = "9142";
const STYLE_ID = "realGlobalMobileFillStyle9142";
...
function installStyles() {
  ...
  style.textContent = `
    ...
    .rf-fill-hidden-head-v9142{
      display:none!important;
    }
    .rf-fill-active-panel-v9142.sheet-panel,
    .rf-fill-active-panel-v9142.pm-sheet-panel,
    .rf-fill-active-panel-v9142.cm-sheet-panel,
    .rf-fill-active-panel-v9142.sheet{
...
function clearActive() {
  if (activeHead) activeHead.classList.remove("rf-fill-hidden-head-v9142");
  if (activePanel) activePanel.classList.remove("rf-fill-active-panel-v9142");
  document.body.classList.remove("rf-fill-active-v9142");
  activeHead = null;
  activePanel = null;
}
...
function applyActive(target) {
  ...
  activePanel = panel;
  activeHead = head;
  if (activePanel) activePanel.classList.add("rf-fill-active-panel-v9142");
  if (activeHead) activeHead.classList.add("rf-fill-hidden-head-v9142");
  document.body.classList.add("rf-fill-active-v9142");
}
...
function syncFillState() {
  ...
  const panel = findPanel(target);
  const head = findHead(panel);
  if (panel !== activePanel || head !== activeHead || !activeHead?.classList.contains("rf-fill-hidden-head-v9142")) {
    applyActive(target);
  }
}
