(() => {
  ...
  let mode = localStorage.getItem(KEY) === "DISTRIBUTOR_REDZED" ? "DISTRIBUTOR_REDZED" : "DIRECT_CUSTOMER";
  const label = value => value === "DISTRIBUTOR_REDZED" ? "DISTRIBUTOR" : "CUSTOMER";
  function style() {
  ...
  function init() {
    ...
    if (!document.getElementById("rrRelationTabs9704")) {
      head.insertAdjacentHTML("afterend", '<div id="rrRelationTabs9704" class="rrRelationTabs9704"><button data-relation-filter="DIRECT_CUSTOMER">CUSTOMERS</button><button data-relation-filter="DISTRIBUTOR_REDZED">DISTRIBUTORS</button></div>');
