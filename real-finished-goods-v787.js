(() => {
  ...
  const state = { profile:null, packPlan:null, packLots:[], packWorkers:[], selectedPack:null, packWorkerValue:'', readyBoxes:[], receiveBoxes:[], piId:null, piLines:[], stock:[], cpis:[], mode:params.get('mode')==='REAL'?'REAL':'TEST', mountedLot:(params.get('lot')||'').trim() };
  const packWorkerCacheKey='real_factory_pack_workers_v788';
  ...
  function packMatrix(cells){
    const rows = {};
    (cells||[]).forEach(x=>{
      const colour = String(x.colour_code||'').trim();
      const size = String(x.size_code||'').trim();
    ...
    const sizes = [...new Set([...SIZE_ORDER,...(cells||[]).map(x=>String(x.size_code||'').trim()).filter(Boolean)])]
      .sort((a,b)=>(SIZE_ORDER.indexOf(a)<0?99:SIZE_ORDER.indexOf(a))-(SIZE_ORDER.indexOf(b)<0?99:SIZE_ORDER.indexOf(b))||a.localeCompare(b,undefined,{numeric:true}));
  ...
  async function boot(){
    bind(); installPackAutoLoadGuard(); scheduleInitialPackLoad(true);
    const auth=await RR.requireRoles(['owner','admin','manager','packing','store','sales','accounts']); state.profile=auth.profile;
    const role=String(state.profile.role_code||'').toLowerCase();$('operator').textContent=['owner','admin'].includes(role)?'SUPER ADMIN':(state.profile.full_name||'Authorized User');
    await Promise.allSettled([loadPackLots(),loadPackWorkers(),loadReadyBoxes(),loadChallans(),loadStock(),loadCpis(),loadSuggestions()]);
    scheduleInitialPackLoad(true);
  }
  ...
  async function ensurePackCardsLoaded(force=false){
    ...
    try{if(!state.packWorkers.length)await loadPackWorkers();await loadPackLots();}catch(e){console.warn(e);}
  }
  ...
  function bind(){
    ...
    $('refreshPackLots').onclick=()=>ensurePackCardsLoaded(true);$('packLotSearch').oninput=renderPackLots;$('closePackLot').onclick=closePackLot;$('assignPack').onclick=assignPack;$('acceptPack').onclick=acceptPack;$('runPackAlgo').onclick=generatePack;$('submitPack').onclick=submitPack;$('loadReadyBoxes').onclick=loadReadyBoxes;$('submitDispatch').onclick=submitDispatch;
    $('packWorker').addEventListener('focus',()=>{if(!state.packWorkers.length)loadPackWorkers().catch(console.warn);});$('packWorker').addEventListener('pointerdown',()=>{if(!state.packWorkers.length)loadPackWorkers().catch(console.warn);});$('packWorker').addEventListener('change',()=>{state.packWorkerValue=$('packWorker').value;});$('packWorker').addEventListener('input',()=>{state.packWorkerValue=$('packWorker').value;});
  ...
  function cachedPackWorkers(){try{return JSON.parse(localStorage.getItem(packWorkerCacheKey)||'[]')}catch(_){return[];}}
  function packWorkersFromLots(){return state.packLots.filter(x=>x.worker_user_id&&x.worker_name).map(x=>({user_id:x.worker_user_id,display_name:x.worker_name,worker_code:''}));}
  function normalPackWorkers(data){const seen=new Set();return (data||[]).map(x=>({user_id:x.user_id||x.linked_auth_user_id||x.worker_id,display_name:x.display_name||x.worker_name,worker_code:x.worker_code||''})).filter(x=>x.user_id&&x.display_name&&!seen.has(String(x.user_id))&&seen.add(String(x.user_id)));}
  function currentPackWorkerValue(){const el=$('packWorker');return el?.value||state.packWorkerValue||el?.selectedOptions?.[0]?.value||'';}
  function samePackWorkers(a,b){return JSON.stringify((a||[]).map(x=>[String(x.user_id),String(x.display_name),String(x.worker_code||'')]))===JSON.stringify((b||[]).map(x=>[String(x.user_id),String(x.display_name),String(x.worker_code||'')]));}
  function setPackWorkerOptions(clean){const el=$('packWorker'),keep=currentPackWorkerValue();if(samePackWorkers(state.packWorkers,clean)&&el?.options?.length){if(keep&&clean.some(x=>String(x.user_id)===String(keep))){el.value=keep;state.packWorkerValue=keep;}return;}state.packWorkers=clean;if(clean.length)localStorage.setItem(packWorkerCacheKey,JSON.stringify(clean));el.innerHTML='<option value="">Select Packing Worker…</option>'+clean.map(x=>`<option value="${x.user_id}">${esc(x.display_name)}${x.worker_code?' · '+esc(x.worker_code):''}</option>`).join('');if(keep&&clean.some(x=>String(x.user_id)===String(keep))){el.value=keep;state.packWorkerValue=keep;}}
  async function loadPackWorkers(){try{let data=await rpc('rr_fg_packing_workers_v788')||[];if(!data.length){const depts=['PACKING','PACK','FINISHED GOODS','FG',''];const calls=[...depts.flatMap(d=>[rpc('rr_upm_worker_list_v8_4',{p_department_code:d}),rpc('rr_upm_worker_list_v8_3',{p_department_code:d}),rpc('rr_upm_worker_list_v731',{p_department_code:d})]),rpc('rr_owner_worker_directory_v8_4',{p_department_code:null,p_include_inactive:false}),rows('rr_user_assignments_v2','user_id,display_name,worker_code,department_code,is_active')];const lists=await Promise.allSettled(calls);data=lists.flatMap(r=>r.status==='fulfilled'?(r.value||[]):[]);}const clean=normalPackWorkers([...data,...cachedPackWorkers(),...packWorkersFromLots()]);setPackWorkerOptions(clean);if(!clean.length)msg('Packing worker mapping empty aa rahi hai. Worker Directory / UPM me PACKING worker active-map check karein.','error');}catch(e){console.warn(e);const clean=normalPackWorkers([...cachedPackWorkers(),...packWorkersFromLots()]);if(clean.length)setPackWorkerOptions(clean);else{$('packWorker').innerHTML='<option value="">Packing workers load nahi hue</option>';msg('Packing workers load nahi hue: '+e.message,'error');}}}
  async function openPackLot(lot){const x=state.packLots.find(v=>String(v.lot_no)===String(lot));if(!x)return;await loadPackWorkers();const canPack=x.is_mine||canAssign();state.selectedPack=x;state.packWorkerValue='';$('packWorker').value='';$('packWorker').disabled=false;state.packPlan=null;$('packWorkspace').hidden=false;$('selectedPackLot').textContent=`Lot ${x.lot_no}`;$('selectedPackMeta').innerHTML=`<span class="fg-chip">Ready PCS <b>${x.ready_qty}</b></span><span class="fg-chip">Status <b>${esc(x.status_label)}</b></span>${x.worker_name?`<span class="fg-chip">Packer <b>${esc(x.worker_name)}</b></span>`:''}${canAssign()&&x.assignment_id&&!x.is_mine?'<span class="fg-chip">Admin/Manager behalf</span>':''}`;$('assignPackBlock').hidden=!canAssign()||Boolean(x.assignment_id);$('workerPackBlock').hidden=!(canPack&&x.assignment_status==='ASSIGNED');$('packAlgoBlock').hidden=!(canPack&&['ACCEPTED','SUBMITTED'].includes(x.assignment_status));$('submitPack').disabled=true;$('packRows').innerHTML='';$('packSummary').innerHTML='';$('packWorkspace').scrollIntoView({behavior:'smooth',block:'start'});if(x.pack_plan_id)renderPackPlan(x.pack_plan_id,x.assignment_status);}
  function closePackLot(){$('packWorkspace').hidden=true;state.selectedPack=null;state.packPlan=null;}
  async function assignPack(){try{if(!state.selectedPack)throw Error('Lot card select karein.');const worker=$('packWorker').value||state.packWorkerValue||$('packWorker').selectedOptions?.[0]?.value;if(!worker)throw Error('Packing Worker select karein.');state.packWorkerValue=worker;await rpc('rr_fg_assign_packing_v788',{p_lot_no:state.selectedPack.lot_no,p_worker_user_id:worker,p_data_mode:'TEST'});msg(`Lot ${state.selectedPack.lot_no} packing worker ko assigned.`,'ok');closePackLot();await loadPackLots();}catch(e){msg(e.message,'error');}}
  ...
  function receiveEntries(){return [...$('receiveBoxRows').querySelectorAll('tr')].filter(r=>r.querySelector('[data-receive-check]')?.checked).map(r=>({box_id:r.dataset.boxId,qty:Number(r.querySelector('[data-receive-qty]').value)}));}
  function updateReceiveSummary(){const entries=receiveEntries(),qty=entries.reduce((n,x)=>n+(Number.isInteger(x.qty)&&x.qty>0?x.qty:0),0);$('receiveSummary').innerHTML=`<span class="fg-chip">Selected Boxes <b>${entries.length}</b></span><span class="fg-chip">Receive PCS <b>${qty}</b></span>`;}
  function renderReceiveBoxes(){const o=$('receiveChallan').selectedOptions[0];try{state.receiveBoxes=JSON.parse(o?.dataset.boxes||'[]')}catch(_){state.receiveBoxes=[]}$('receiveBoxRows').innerHTML=state.receiveBoxes.length?state.receiveBoxes.map(x=>`<tr data-box-id="${esc(x.box_id)}"><td><input type="checkbox" data-receive-check checked aria-label="Receive ${esc(x.box_code)}"></td><td><b>${esc(x.box_code)}</b></td><td>${esc(x.type||'')}</td><td>${Number(x.qty)}</td><td><input class="fg-qty-input" data-receive-qty type="number" min="1" step="1" required inputmode="numeric" placeholder="Mandatory"></td></tr>`).join(''):'<tr><td colspan="5" class="fg-muted">Pending Challan select karein.</td></tr>';$('receiveBoxRows').querySelectorAll('input').forEach(x=>x.addEventListener('input',updateReceiveSummary));updateReceiveSummary();}
  async function uploadDifferenceEvidence(file,despatchId){if(!file)return null;const ext=(file.name.split('.').pop()||'jpg').replace(/[^a-z0-9]/gi,'').toLowerCase()||'jpg',path=`despatch/${despatchId}/${Date.now()}-${crypto.randomUUID()}.${ext}`;const up=await supabaseClient.storage.from('production-evidence').upload(path,file,{contentType:file.type||'image/jpeg',upsert:false});if(up.error)throw up.error;return path;}
  async function acceptReceive(){try{const id=$('receiveChallan').value;if(!id)throw Error('Challan select karein.');const entries=receiveEntries();if(!entries.length)throw Error('Kam se kam ek Box select karein.');let mismatch=false;for(const e of entries){if(!Number.isInteger(e.qty)||e.qty<=0)throw Error('Har selected Box ki Receive Qty mandatory hai.');const box=state.receiveBoxes.find(x=>String(x.box_id)===String(e.box_id));if(!box)throw Error('Selected Box challan me nahi hai.');if(e.qty!==Number(box.qty))mismatch=true;}if(mismatch&&!$('receiveRemarks').value.trim())throw Error('Qty difference par Reason / Remarks mandatory hai.');const photo=$('receiveEvidence').files[0];if(mismatch&&!photo)throw Error('Qty difference par Photo mandatory hai.');const evidence=await uploadDifferenceEvidence(photo,id);const r=await rpc('rr_fg_receive_despatch_v7981',{p_despatch_id:id,p_boxes:entries,p_remarks:$('receiveRemarks').value,p_evidence_path:evidence});if(r.status==='DIFFERENCE_HOLD'){msg(`Qty mismatch: ${r.difference_boxes} Box DIFFERENCE HOLD me. Stock post nahi hua.`,'error');}else{msg(`STORE ACCEPTED: ${r.accepted_boxes} boxes / ${r.accepted_qty} PCS. Webstore stock updated.`,'ok');}await Promise.all([loadChallans(),loadStock(),loadSuggestions()]);}catch(e){msg(e.message,'error');}}
