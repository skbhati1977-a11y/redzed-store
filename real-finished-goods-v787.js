(() => {
  ...
  const state = { profile:null, packPlan:null, packLots:[], packWorkers:[], selectedPack:null, selectedPackWorker:'', readyBoxes:[], receiveBoxes:[], piId:null, piLines:[], stock:[], cpis:[], mode:params.get('mode')==='REAL'?'REAL':'TEST', mountedLot:(params.get('lot')||'').trim() };
  ...
  function bind(){
    ...
    $('refreshPackLots').onclick=loadPackLots;$('packLotSearch').oninput=renderPackLots;$('closePackLot').onclick=closePackLot;$('packWorker').onchange=()=>{state.selectedPackWorker=$('packWorker').value;renderPackWorkerChoices();};$('assignPack').onclick=assignPack;$('acceptPack').onclick=acceptPack;$('runPackAlgo').onclick=generatePack;$('submitPack').onclick=submitPack;$('loadReadyBoxes').onclick=loadReadyBoxes;$('submitDispatch').onclick=submitDispatch;
