(() => {
  ...
  function bind(){
    ...
    $('packWorker').addEventListener('focus',()=>loadPackWorkers().catch(console.warn));$('packWorker').addEventListener('pointerdown',()=>loadPackWorkers().catch(console.warn));$('packWorker').addEventListener('change',()=>{state.packWorkerValue=$('packWorker').value;});$('packWorker').addEventListener('input',()=>{state.packWorkerValue=$('packWorker').value;});
  ...
  function setPackWorkerOptions(clean){state.packWorkers=clean;if(clean.length)localStorage.setItem(packWorkerCacheKey,JSON.stringify(clean));$('packWorker').innerHTML='<option value="">Select Packing Worker…</option>'+clean.map(x=>`<option value="${x.user_id}">${esc(x.display_name)}${x.worker_code?' · '+esc(x.worker_code):''}</option>`).join('');}
