(() => {
  ...
  function compositionTable(cells){
    ...
    return `<div class="fg-pack-composition">${m.colours.flatMap(colour=>m.sizes.filter(size=>Number(m.rows[colour][size]||0)>0).map(size=>`<span><b>${esc(colour)}</b><i>${esc(size)}</i><strong>${Number(m.rows[colour][size])} PCS</strong></span>`)).join('')}</div>`;
  }
  ...
  function canAssign(){return ['owner','super_admin','admin','manager'].includes(String(state.profile?.role_code||'').toLowerCase());}
  ...
  async function loadPackWorkers(){try{const data=await rpc('rr_fg_packing_workers_v788');$('packWorker').innerHTML='<option value="">Select Packing Worker…</option>'+data.map(x=>`<option value="${x.user_id}">${esc(x.display_name)}${x.worker_code?' · '+esc(x.worker_code):''}</option>`).join('');if(state.selectedPack?.worker_user_id)$('packWorker').value=String(state.selectedPack.worker_user_id);}catch(e){console.warn(e);}}
  function openPackLot(lot){const x=state.packLots.find(v=>String(v.lot_no)===String(lot));if(!x)return;const canPack=x.is_mine||canAssign();state.selectedPack=x;if($('packWorker'))$('packWorker').value=String(x.worker_user_id||'');state.packPlan=null;$('packWorkspace').hidden=false;$('selectedPackLot').textContent=`Lot ${x.lot_no}`;$('selectedPackMeta').innerHTML=`<span class="fg-chip">Ready PCS <b>${x.ready_qty}</b></span><span class="fg-chip">Status <b>${esc(x.status_label)}</b></span>${x.worker_name?`<span class="fg-chip">Packer <b>${esc(x.worker_name)}</b></span>`:''}${canAssign()&&x.assignment_id&&!x.is_mine?'<span class="fg-chip">Admin/Manager behalf</span>':''}`;$('assignPackBlock').hidden=!canAssign()||Boolean(x.assignment_id);$('workerPackBlock').hidden=!(canPack&&x.assignment_status==='ASSIGNED');$('packAlgoBlock').hidden=!(canPack&&['ACCEPTED','SUBMITTED'].includes(x.assignment_status));$('submitPack').disabled=true;$('packRows').innerHTML='';$('packSummary').innerHTML='';$('packWorkspace').scrollIntoView({behavior:'smooth',block:'start'});if(x.pack_plan_id)renderPackPlan(x.pack_plan_id,x.assignment_status);}
  ...
  function renderPackBoxes(boxes){$('packRows').innerHTML=packBoxGroups(boxes).map(g=>`<tr><td data-label="Box No.">${esc(packGroupBoxLabel(g))}</td><td data-label="Consignment">${esc(g.type)}</td><td data-label="Box PCS">${esc(packGroupQty(g))}</td><td data-label="Colour / Size Composition">${compositionTable(g.sample.cells)}</td></tr>`).join('');}
