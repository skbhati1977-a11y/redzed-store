...
async function load(){client=client||(window.RR?.getClient?RR.getClient():window.supabaseClient);if(!client)throw Error("Supabase client not available.");const {data,error}=await client.rpc("rr_material_purchase_bootstrap_v805_1",{p_data_mode:$("dataMode").value});if(error)throw error;state=data||state;{
  ...
  if(!types.some(t=>String(t.type_code||"").toUpperCase()==="MATCHING_CLOTH")){
    types.unshift({type_code:"MATCHING_CLOTH",type_name:"Matching Cloth",default_purchase_unit:"KG",default_consumption_unit:"KG"});
  }
  $("type").innerHTML=`<option value="">Select…</option>`+types.map(t=>`<option value="${esc(t.type_code)}">${esc(t.type_name)}</option>`).join("");
}$("supplier").innerHTML=ledgerOptions(state.ledgers.filter(x=>["SUPPLIER","PARTY","GENERAL"].includes(String(x.ledger_kind||"").toUpperCase())));$("cashBank").innerHTML=ledgerOptions(state.ledgers.filter(x=>["CASH","BANK"].includes(String(x.ledger_kind||"").toUpperCase())));$("purchaseLedger").innerHTML=ledgerOptions(state.ledgers);renderTable();try{const {data:{user}}=await client.auth.getUser();if(user){const {data:p}=await client.from("rr_user_profiles").select("full_name,role_code").eq("auth_user_id",user.id).maybeSingle();$("who").textContent=p?`${p.full_name||"User"} · ${String(p.role_code||"").toLowerCase()==="owner"?"Super Admin":p.role_code}`:(user.email||"User")}}catch{}}
function renderTable(){const r=state.materials||[];$("body").innerHTML=r.length?r.map(m=>`<tr><td>${esc(m.material_type_name)}</td><td>${esc(m.material_no||"—")}</td><td>${esc(m.material_name)}</td><td>${esc(m.purchase_unit)}</td><td>${esc(m.base_stock_unit)}</td><td>${esc(m.consumption_unit)}</td><td>${num(m.current_balance_stock_qty)} ${esc(m.base_stock_unit)}</td><td>${money(m.running_purchase_value)}</td><td>${num(m.running_consumption_qty)} ${esc(m.consumption_unit)}</td><td>${money(m.running_weighted_avg_cost_per_consumption_unit)} / ${esc(m.consumption_unit)}</td><td>${esc(m.consumption_basis)}</td><td>${esc((m.applicable_to?.tags||[]).join(", ")||"—")}</td></tr>`).join(""):`<tr><td colspan="12">No generic materials created yet.</td></tr>`}
function clearSelection(){selected=null;$("no").value="";$("sourceHistoryPanel")?.classList.add("hidden");$("sourceHistoryBody")&&($("sourceHistoryBody").innerHTML=`<tr><td colspan="8">Select Sticker / Metal ID.</td></tr>`);$("balanceStrip").classList.remove("show");$("sourceNotice").classList.add("hidden");$("savePost").disabled=false;for(const id of ["purchaseUnit","stockUnit","consumptionUnit"])$(id).classList.remove("locked");calc()}
async function searchMapped(){const type=$("type").value;if(!type)return $("suggestions").classList.add("hidden");const {data,error}=await client.rpc("rr_material_source_search_v805_1",{p_type_code:type,p_search:$("name").value||"",p_data_mode:$("dataMode").value,p_limit:20});if(error)throw error;const rows=data||[];$("suggestions").innerHTML=rows.length?rows.map((r,i)=>`<button class="suggestion" data-i="${i}" type="button"><span><strong>${esc(r.material_name)}</strong><small>${esc(r.material_no||r.source_type)}</small></span><small>${r.current_balance_qty==null?"Mapped":`Bal ${num(r.current_balance_qty)} ${esc(r.stock_unit)}`}</small></button>`).join(""):`<div style="padding:10px;color:#9aa4af">No mapped match. For a generic material, Super Admin can create a new one.</div>`;$("suggestions").classList.remove("hidden");$("suggestions").querySelectorAll("[data-i]").forEach(b=>b.onclick=()=>selectMapped(rows[Number(b.dataset.i)]))}

async function loadSourceHistory(r){
  const panel=$("sourceHistoryPanel"),body=$("sourceHistoryBody"),title=$("sourceHistoryTitle");
  if(!panel||!body||!title)return;
  const t=String($("type").value||"").toUpperCase();
  if(!r?.source_managed || !["STICKER","METAL_ID"].includes(t) || !r.source_id){
    panel.classList.add("hidden"); return;
  }
  panel.classList.remove("hidden");
  title.textContent=`Existing ${t==="STICKER"?"Sticker":"Metal ID"} Purchases · ${[r.material_no,r.material_name].filter(Boolean).join(" · ")}`;
  body.innerHTML=`<tr><td colspan="8">Loading source purchase history…</td></tr>`;
  try{
    const {data,error}=await client.rpc("rr_accessory_purchase_history_v804",{
      p_item_type:t,p_master_id:r.source_id,p_data_mode:$("dataMode").value
    });
    if(error)throw error;
    const rows=data||[];
    body.innerHTML=rows.length?rows.map(x=>`<tr>
      <td>${esc(x.bill_date||String(x.created_at||"").slice(0,10)||"—")}</td>
      <td>${esc(x.entry_type||"—")}</td><td>${esc(x.vendor_name||"—")}</td>
      <td>${esc(x.bill_no||"—")}</td><td>${num(x.qty)}</td>
      <td>${money(x.rate_per_piece)}</td><td>${money(Number(x.qty||0)*Number(x.rate_per_piece||0))}</td>
      <td>${esc(x.notes||"—")}</td></tr>`).join("")
      :`<tr><td colspan="8">No source purchase entry yet.</td></tr>`;
  }catch(e){
    body.innerHTML=`<tr><td colspan="8" class="err">${esc(e?.message||String(e))}</td></tr>`;
  }
}
