...
window.REAL_FACTORY_PRODUCT_MASTER_VERSION = "V804-ACCESSORY-MASTER-INVENTORY";
...
const state = {
  ...
  arts:[], prints:[], stickerMasters:[], metalIdMasters:[], stickerInstructions:[], metalIdInstructions:[], assignments:[], printAssignments:[], stickerAssignments:[], metalIdAssignments:[], media:[], lots:[],
  ...
  lotCostAdjustments:[], pmDecisionRows:[], pmDecisionCounts:{}, accessoryBalances:[], dataMode:"TEST", activeDetail:null, operation:null
};
...
const TABLES = {
  ...
  stickerMasters:"rr_sticker_master_library_v803", metalIdMasters:"rr_metal_id_master_library_v803",
...
async function loadData(){
  ...
  try{
    const [categories,gallery,purchases,rolls,colours,allocations,arts,prints,stickerMasters,metalIdMasters,stickerInstructions,metalIdInstructions,assignments,printAssignments,stickerAssignments,metalIdAssignments,media,lots,_mcLoaded,grEntries,exchangeEntries,damageClaims,damageMedia,lotCostAdjustments,pmDecisionRows,pmDecisionCounts]=await Promise.all([
      requiredTable(TABLES.categories),loadGalleryRows(),requiredTable(TABLES.purchases,"*","created_at"),optionalTable(TABLES.rolls),requiredTable(TABLES.colours,"*","colour_order"),optionalTable(TABLES.allocations),requiredTable(TABLES.arts,"*","updated_at"),loadPrints(),requiredTable(TABLES.stickerMasters),requiredTable(TABLES.metalIdMasters),optionalTable(TABLES.stickerInstructions,"*","sequence_no"),optionalTable(TABLES.metalIdInstructions,"*","sequence_no"),requiredTable(TABLES.assignments,"*","updated_at"),requiredTable(TABLES.printAssignments,"*","sequence_no"),optionalTable(TABLES.stickerAssignments,"*","sequence_no"),optionalTable(TABLES.metalIdAssignments,"*","sequence_no"),optionalTable(TABLES.media),optionalTable(TABLES.lots,"*","created_at"),loadMc(),optionalTable(TABLES.gr,"*","created_at"),optionalTable(TABLES.exchanges,"*","created_at"),optionalTable(TABLES.damage,"*","created_at"),optionalTable(TABLES.damageMedia,"*","created_at"),optionalTable(TABLES.lotAdjustments,"*","created_at"),loadPmDecisionRows(),loadPmDecisionCounts()
    ]);
    Object.assign(state,{categories,galleryRows:gallery,purchases,rolls,colours,allocations,arts,prints,stickerMasters,metalIdMasters,stickerInstructions,metalIdInstructions,assignments,printAssignments,stickerAssignments,metalIdAssignments,media,lots,grEntries,exchangeEntries,damageClaims,damageMedia,lotCostAdjustments,pmDecisionRows,pmDecisionCounts});
    const bal=await state.client.from("rr_accessory_stock_balance_v804").select("*").eq("data_mode",state.dataMode);
    if(!bal.error)state.accessoryBalances=bal.data||[];
...
function metalIdIdsForAssignment(a){if(!a)return[];return state.metalIdAssignments.filter(x=>String(x.assignment_id)===String(a.id)).sort((x,y)=>Number(x.sequence_no||0)-Number(y.sequence_no||0)).map(x=>state.metalIdInstructions.find(i=>String(i.id)===String(x.metal_id_instruction_id))?.metal_id_master_id).filter(Boolean).map(String);}
function stickerOptionsForArt(){return state.stickerMasters.filter(x=>x.is_active!==false);}
function metalOptionsForArt(){return state.metalIdMasters.filter(x=>x.is_active!==false);}
function accessoryBalance(type,id){return state.accessoryBalances.find(b=>String(b.item_type)===type&&String(type==="STICKER"?b.sticker_master_id:b.metal_id_master_id)===String(id))||{};}
...
function renderAssignmentPickers(){
  ...
  $("stickerPicker").innerHTML=[
    ...
    ...stickers.map(s=>`<button type="button" class="pick ${state.stickerMode==="SELECTED"&&state.selectedStickerIds.includes(String(s.id))?"selected":""}" data-sticker="${safe(s.id)}">${s.image_url?`<img src="${safe(s.image_url)}">`:""}<strong>${safe(s.sticker_no||"STICKER")}</strong><small>${safe((s.sticker_name||"")+" · "+(s.sticker_quality||""))}<br>Stock ${safe(Number(accessoryBalance("STICKER",s.id).physical_stock_qty||0))} · Req Now ${safe(Number(accessoryBalance("STICKER",s.id).req_now_qty||0))}</small></button>`)
  ].join("");
  ...
  $("metalIdPicker").innerHTML=[
    ...
    ...metals.map(m=>`<button type="button" class="pick ${state.metalIdMode==="SELECTED"&&state.selectedMetalIdIds.includes(String(m.id))?"selected":""}" data-metal="${safe(m.id)}">${m.image_url?`<img src="${safe(m.image_url)}">`:""}<strong>${safe(m.metal_id_no||"METAL ID")}</strong><small>${safe((m.metal_id_name||"")+" · "+(m.id_size||""))}<br>Stock ${safe(Number(accessoryBalance("METAL_ID",m.id).physical_stock_qty||0))} · Req Now ${safe(Number(accessoryBalance("METAL_ID",m.id).req_now_qty||0))}</small></button>`)
  ].join("");
...
function validateDecisionStep(){
  ...
  if(state.decisionStep==="sticker"&&state.stickerMode==="SELECTED"&&!state.selectedStickerIds.length)return "Sticker select karein, ya N.A./DUE choose karein.";
  if(state.decisionStep==="metalId"&&state.metalIdMode==="SELECTED"&&!state.selectedMetalIdIds.length)return "Metal ID select karein, ya N.A./DUE choose karein.";
  return "";
}

async function currentDataMode(){
  try{const r=await state.client.rpc("rr_app_data_mode_state_v786");if(!r.error&&r.data?.default_mode)return String(r.data.default_mode).toUpperCase()}catch(e){console.warn("Data mode state unavailable",e)}
  return "TEST";
}
...
async function saveDecisionBundle(){
  ...
  try{
    ...
    const r=await state.client.rpc("rr_pm_save_decision_bundle_v804",{
      ...
      p_sticker_master_ids:state.selectedStickerIds,
      p_metal_id_mode:state.metalIdMode,
      p_metal_id_master_ids:state.selectedMetalIdIds,
      p_data_mode:mode
    });
