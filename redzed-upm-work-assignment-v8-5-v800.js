(() => {
'use strict';
const VERSION='8.5.0',$=id=>document.getElementById(id);
const safe=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const state={client:null,lotId:'',lotNo:'',department:'',context:[],selected:new Set(),workers:[],commonWorkerId:''};
...
<label class="field"><span>Worker — Unified Directory</span><select id="rrAssignCommonWorker"><option value="">Select worker</option></select></label></div>
...
state.selected.clear();state.commonWorkerId='';$('rrAssignLotNo').textContent=state.lotNo||'—';$('rrAssignDept').innerHTML=departmentOptions();state.department=$('rrAssignDept').value;
...
async function loadContext(){$('rrAssignStatus').textContent='Loading Cutting colour-size quantities…';const rows=await rpc('rr_upm_get_work_assign_context_v800',{p_canonical_lot_id:state.lotId||null,p_lot_no:state.lotNo||null,p_department_code:state.department});
state.context=Array.isArray(rows)?rows:[];state.selected.clear();renderColourPick();$('rrAssignRows').innerHTML='';$('rrAssignStatus').textContent=state.context.length?`${state.context.length} Cutting colour mapped. Select colour; row opens immediately.`:'Cutting colour-size mapping not found for this Lot.';}
...
<label class="field"><span>Actual Rate / Pc — Manual</span><input data-row-rate type="number" min="0" step="0.01" value="${r.actual_rate==null?'':Number(r.actual_rate)}" placeholder="Enter rate" required></label></div></section>`).join('');
$('rrAssignStatus').textContent=`${rows.length} colour selected. Worker and Actual Rate fill karke Assign Work karein.`;}
...
const rows=nodes.map(n=>{const colour=n.dataset.assignRow,worker=n.querySelector('[data-row-worker]').value,qty=Number(n.querySelector('[data-row-qty]').value||0),txt=n.querySelector('[data-row-rate]').value,rate=Number(txt);
if(!worker)throw new Error(`Worker is required for colour ${colour}.`);if(txt===''||!Number.isFinite(rate)||rate<0)throw new Error(`Valid Actual Rate is required for colour ${colour}.`);
return {colour_code:colour,worker_id:worker,assigned_qty:qty,actual_rate:rate};});
$('rrAssignSave').disabled=true;$('rrAssignSave').textContent='ASSIGNING…';await rpc('rr_upm_claim_colours_with_accept_v800',{p_canonical_lot_id:state.lotId||null,p_lot_no:state.lotNo||null,p_department_code:state.department,p_rows:rows,p_remarks:$('rrAssignRemarks').value||null});
...
function enhanceCards(){document.querySelectorAll('[data-entry]').forEach(btn=>{const id=btn.dataset.entry;if(!id||btn.parentElement.querySelector(`[data-work-assign="${CSS.escape(id)}"]`))return;
const card=btn.closest('.card'),lot=card?.querySelector('.lot-number')?.textContent?.trim()||card?.querySelector('h3')?.textContent?.trim()||id,b=document.createElement('button');
b.type='button';b.className='rr-assign-open';b.dataset.workAssign=id;b.textContent='Assign Work';b.onclick=()=>openAssign(id,lot);btn.parentElement.insertBefore(b,btn);});}
