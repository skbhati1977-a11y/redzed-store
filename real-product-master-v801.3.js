...
window.REAL_FACTORY_PRODUCT_MASTER_VERSION = "V801.3-DECISION-TABS";
...
const state = {
  ...
  arts:[], prints:[], assignments:[], printAssignments:[], stickerAssignments:[], metalIdAssignments:[], media:[], lots:[],
...
async function loadData(){
  ...
  try{
    const [categories,gallery,purchases,rolls,colours,allocations,arts,prints,assignments,printAssignments,stickerAssignments,metalIdAssignments,media,lots,_mcLoaded,grEntries,exchangeEntries,damageClaims,damageMedia,lotCostAdjustments]=await Promise.all([
      requiredTable(TABLES.categories),loadGalleryRows(),requiredTable(TABLES.purchases,"*","created_at"),optionalTable(TABLES.rolls),requiredTable(TABLES.colours,"*","colour_order"),optionalTable(TABLES.allocations),requiredTable(TABLES.arts,"*","updated_at"),loadPrints(),requiredTable(TABLES.assignments,"*","updated_at"),requiredTable(TABLES.printAssignments,"*","sequence_no"),optionalTable(TABLES.stickerAssignments,"*","sequence_no"),optionalTable(TABLES.metalIdAssignments,"*","sequence_no"),optionalTable(TABLES.media),optionalTable(TABLES.lots,"*","created_at"),loadMc(),optionalTable(TABLES.gr,"*","created_at"),optionalTable(TABLES.exchanges,"*","created_at"),optionalTable(TABLES.damage,"*","created_at"),optionalTable(TABLES.damageMedia,"*","created_at"),optionalTable(TABLES.lotAdjustments,"*","created_at")
    ]);
    Object.assign(state,{categories,galleryRows:gallery,purchases,rolls,colours,allocations,arts,prints,assignments,printAssignments,stickerAssignments,metalIdAssignments,media,lots,grEntries,exchangeEntries,damageClaims,damageMedia,lotCostAdjustments});
...
function printDecisionDone(a){return Boolean(a&&(a.print_not_applicable||state.printAssignments.some(x=>String(x.assignment_id)===String(a.id))));}
function stickerDecisionDone(a){return Boolean(a&&(a.sticker_not_applicable||state.stickerAssignments.some(x=>String(x.assignment_id)===String(a.id))));}
function metalIdDecisionDone(a){return Boolean(a&&(a.metal_id_not_applicable||state.metalIdAssignments.some(x=>String(x.assignment_id)===String(a.id))));}
...
function cbCardHtml(g,d){
  const unitId=d.division_id;
  const a=assignmentFor(unitId);
  const art=artFor(a);
  const ps=printsForAssignment(a);
  const cols=coloursFor(g.cbId);
  const lots=lotsForUnit(unitId);
  const lotText=lots.map(x=>x.lot_no).filter(Boolean).join(" · ");

  const artDecided=Boolean(a&&art);
  const printDecided=printDecisionDone(a);
  const stickerDecided=stickerDecisionDone(a);
  const metalIdDecided=metalIdDecisionDone(a);
  ...
  const search=[
    g.cbNo,canonicalD(d),artNo(art),ps.map(printNo).join(" "),
    a?.print_not_applicable?"print na":"",
    a?.sticker_not_applicable?"sticker na":"",
    a?.metal_id_not_applicable?"metal id na":"",
    lots.map(x=>x.lot_no).join(" "),
    purchasesFor(g.cbId).map(x=>`${x.vendor_name} ${x.vendor_bill_no} ${x.fabric_name}`).join(" "),
    cols.map(x=>x.colour_name).join(" ")
  ].join(" ").toLowerCase();

  return `<article class="card"
    data-kind="cb"
    data-ready="${artDecided?"1":"0"}"
    data-art-decided="${artDecided?"1":"0"}"
    data-print-decided="${printDecided?"1":"0"}"
    data-sticker-decided="${stickerDecided?"1":"0"}"
    data-metal-id-decided="${metalIdDecided?"1":"0"}"
    data-search="${safe(search)}">
    <div class="card-body">
      <div class="card-head">
        <div>
          <span class="chip ${artDecided?"ready":""}">${artDecided?"ART DECIDED":"ART DUE"}</span>
          <h3>${safe(g.cbNo)}</h3>
          <strong>${safe(canonicalD(d))}</strong>
        </div>
        ${hero?`<img src="${safe(hero)}" alt="" style="width:78px;height:78px;object-fit:cover;border-radius:14px">`:""}
      </div>
      <div class="metrics">
        <div class="metric"><small>D No.</small><strong>${safe(canonicalD(d))}</strong></div>
        <div class="metric"><small>Art</small><strong>${safe(artNo(art)||"Due")}</strong></div>
        <div class="metric"><small>Print</small><strong>${safe(a?.print_not_applicable?"N/A":ps.map(printNo).join(" · ")||"Due")}</strong></div>
        <div class="metric"><small>Sticker</small><strong>${stickerDecided?(a?.sticker_not_applicable?"N/A":"Decided"):"Due"}</strong></div>
        <div class="metric"><small>Metal ID</small><strong>${metalIdDecided?(a?.metal_id_not_applicable?"N/A":"Decided"):"Due"}</strong></div>
        <div class="metric"><small>Weight</small><strong>${kg(d.allocated_qty??d.divided_weight)}</strong></div>
        <div class="metric"><small>Colours</small><strong>${cols.length}</strong></div>
        <div class="metric"><small>Lot No.</small><strong>${safe(lotText||"Due")}</strong></div>
      </div>
      <div class="colour-strip">${cols.map(c=>`<span class="colour-pill">${c.image_url?`<img src="${safe(c.image_url)}">`:""}<b>${safe(c.colour_name)}</b></span>`).join("")}</div>
      <div class="card-actions">
        <button class="secondary" data-cb-detail="${safe(g.cbId)}">CB Details</button>
        <button class="primary" data-assign="${safe(unitId)}">${a?"Change Art / Print":"Assign Art / Print"}</button>
      </div>
    </div>
  </article>`;
}
...
function filterPass(card){
  ...
  if(f==="planning")return card.dataset.kind==="cb"&&card.dataset.artDecided==="0";
  if(f==="ready")return card.dataset.kind==="cb"&&card.dataset.artDecided==="1";
  if(f==="print_decided")return card.dataset.kind==="cb"&&card.dataset.printDecided==="1";
  if(f==="sticker_decided")return card.dataset.kind==="cb"&&card.dataset.stickerDecided==="1";
  if(f==="metal_id_decided")return card.dataset.kind==="cb"&&card.dataset.metalIdDecided==="1";
  return true;
}
