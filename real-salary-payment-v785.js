(()=>{'use strict';window.REAL_FACTORY_SALARY_PAYMENT_VERSION='786.3.10-LIVE-PAYMENT-TOTAL';
...
function updateLivePaymentTotals(){
...
if(n>0&&!isRound100(n)){state.manual.set(id,0);say('Manual payment ₹100 के multiple में भरें.','error')}else{state.manual.set(id,n)};const tr=i.closest('tr'),x=split(r);tr.children[8].textContent=`₹${money(x.old)}`;tr.children[9].textContent=`₹${money(x.cur)}`;tr.children[10].textContent=`₹${money(newBal(r))}`;cards();updateLivePaymentTotals()};i.onblur=()=>i.value=Number(state.manual.get(i.dataset.amount)||0).toFixed(2);i.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();const n=inputs[k+1];if(n){n.focus();n.select()}else $('submitBulkPayment').focus()}}});cards()}
