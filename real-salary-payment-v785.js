(()=>{'use strict';window.REAL_FACTORY_SALARY_PAYMENT_VERSION='786.3.11-MANUAL-DEFAULT-BULK-APPLY';
...
function updateLivePaymentTotals(flash=false){
  ...
  if($('liveCurrentPaymentTop'))$('liveCurrentPaymentTop').textContent=`₹${money(t.paid)}`;

  if(flash && $('livePaymentTotalBox')){
    const box=$('livePaymentTotalBox');
...
state.manual.set(id,n);const tr=i.closest('tr'),x=split(r);tr.children[8].textContent=`₹${money(x.old)}`;tr.children[9].textContent=`₹${money(x.cur)}`;tr.children[10].textContent=`₹${money(newBal(r))}`;cards();updateLivePaymentTotals(true)};i.onblur=()=>{const value=Number(state.manual.get(i.dataset.amount)||0);if(value>0&&!isRound100(value)){state.manual.set(i.dataset.amount,0);say('Current Payment ₹100 के multiple में होना चाहिए.','error');render();return}i.value=value.toFixed(2)};i.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();const n=inputs[k+1];if(n){n.focus();n.select()}else $('submitBulkPayment').focus()}}});cards();updateLivePaymentTotals(false)}
