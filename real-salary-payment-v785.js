(()=>{'use strict';window.REAL_FACTORY_SALARY_PAYMENT_VERSION='786.3.0-SMART-CHOICE';
...
const inputs=[...$('ledgerBody').querySelectorAll('[data-amount]')];inputs.forEach((i,k)=>{i.oninput=()=>{const id=i.dataset.amount,r=state.rows.find(x=>String(x.worker_id)===id),max=Number(r?.scope_payable||0),n=Math.min(Math.max(Number(i.value||0),0),max);state.manual.set(id,n);const tr=i.closest('tr'),x=split(r);tr.children[8].textContent=`₹${money(x.old)}`;tr.children[9].textContent=`₹${money(x.cur)}`;tr.children[10].textContent=`₹${money(newBal(r))}`;cards()};i.onblur=()=>i.value=Number(state.manual.get(i.dataset.amount)||0).toFixed(2);i.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();const n=inputs[k+1];if(n){n.focus();n.select()}else $('submitBulkPayment').focus()}}});cards()}
...
async function submitBulkPayment(){try{if(!state.preview)throw Error('पहले Load Workers — No Payment दबाएँ.');if(!state.selected.size)throw Error('कम से कम एक worker select करें.');if(!$('voucherNo').value.trim())throw Error('Voucher / Reference required.');if(method()==='PARTIAL_RATIO'){const entered=Number($('bulkAmount').value||0);if(!(entered>0))throw Error('Bulk Payment Amount 0 से ज्यादा होना चाहिए.');say('Payment amount calculate हो रहा है…','info');await recalc({applyAmount:true})}const paid=method()==='WORKER_LEDGER_WISE'?state.rows.reduce((sum,r)=>sum+(state.selected.has(String(r.worker_id))?Number(state.manual.get(String(r.worker_id))||0):0),0):Number($('bulkAmount').value||0);if(!(paid>0))throw Error('Payment amount 0 से ज्यादा होना चाहिए.');const ok=confirm(`${category()}
...
async function smartChoiceAndSubmit(){
  try{
    ...
    if(!(entered>0))throw Error('Payment Amount 0 से ज्यादा होना चाहिए.');
    if(entered>currentPayable+0.005){
      throw Error(`Payment Amount ₹${money(entered)} Current Payable ₹${money(currentPayable)} से ज्यादा नहीं हो सकता.`);
    }

    $('paymentChoiceSummary').textContent=
      `Current Payable ₹${money(currentPayable)} · Entered Payment ₹${money(entered)}`;

    const exact=Math.abs(entered-currentPayable)<=0.005;
    $('choiceComplete').disabled=!exact;
    $('choiceComplete').style.opacity=exact?'1':'0.45';
...
async function postBySmartChoice(choice){
  try{
    ...
    if(choice==='complete'){
      ...
      $('bulkAmount').value=currentPayable.toFixed(2);
      ...
      if(manualTotal<=0){
        throw Error('Selected Workers Partial के लिए worker rows में Payment amount भरें.');
      }
    }else{
      return;
    }
...
function bind(){$('loadPreview').onclick=load;$('submitBulkPayment').onclick=smartChoiceAndSubmit;if($('loadHistory'))$('loadHistory').onclick=history;if($('selectAll'))$('selectAll').onclick=()=>{state.selected=new Set(state.rows.map(r=>String(r.worker_id)));state.useAll=false;if(method()==='WORKER_LEDGER_WISE')render();else cards()};if($('clearAll'))$('clearAll').onclick=()=>{state.selected.clear();state.useAll=false;if(method()==='WORKER_LEDGER_WISE')render();else cards()};if($('workerSearch'))$('workerSearch').oninput=render;if($('advanceCard')&&$('advancePanel'))$('advanceCard').onclick=()=>$('advancePanel').classList.toggle('hidden');$('payrollCategory').onchange=async()=>{setPeriod();updateRule();state.useAll=true;state.selected.clear();state.manual.clear();state.bulkApplied=false;$('bulkAmount').value='';await history();await load()};$('dataMode').onchange=async()=>{state.useAll=true;state.selected.clear();state.manual.clear();state.bulkApplied=false;await history();await load()};$('paymentMethod').onchange=async()=>{updateRule();state.useAll=true;state.selected.clear();state.manual.clear();state.bulkApplied=false;$('bulkAmount').value='';await load()};$('paymentScope').onchange=async()=>{state.useAll=true;state.selected.clear();state.manual.clear();state.bulkApplied=false;await load()};$('periodStart').onchange=()=>{state.preview=null;state.rows=[];render();say('Period changed. Load Workers again.','info')};$('periodEnd').onchange=()=>{setPeriod();state.preview=null;state.rows=[];render();say('Period changed. Load Workers again.','info')};$('bulkAmount').oninput=()=>{if(method()==='PARTIAL_RATIO'){let raw=String($('bulkAmount').value||'').replace(/,/g,'').replace(/[^0-9.]/g,''),parts=raw.split('.'),clean=parts.length>1?`${parts.shift()}.${parts.join('').slice(0,2)}`:parts[0];if($('bulkAmount').value!==clean)$('bulkAmount').value=clean;cards()}};$('bulkAmount').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();$('submitBulkPayment').click()}};$('choiceSelected').onclick=()=>postBySmartChoice('selected');$('choiceRatio').onclick=()=>postBySmartChoice('ratio');$('choiceComplete').onclick=()=>postBySmartChoice('complete');}
