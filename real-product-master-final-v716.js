...
window.REDZED_PRODUCT_MASTER_VERSION = "716";
...
let cardColumnCount = Number(
  localStorage.getItem("redzedProductCardColumns") || 3
);
...
function formatSaveError(error) {
  ...
  ].join(" — ") || "Purchase could not be saved.";
}
...
function categoryByCode(code) {
  return categories.find(
    item => item.category_code === code
  ) || null;
}

function materialName(entry) {
  return categoryById(
    entry.materialCategoryId
...
function getSelectedDivisionCount() {
  ...
  if (value === "custom") {
    return Math.max(
      ...
      Math.min(
        50,
        Number($("customDivisionCount").value || 0)
      )
    );
  }
...
function currentDivisionChoices() {
  ...
  return Array.from(
    { length: getSelectedDivisionCount() },
    (_, index) => ({
      index: index + 1,
      id: null,
      label: `S${index + 1}`
...
function defaultRollRowCount() {
  return Math.max(
    1,
    currentDivisionChoices().length ||
      getSelectedDivisionCount()
  );
}

function makeColourRollDraft(
  rollCount = defaultRollRowCount()
) {
  return {
    rolls: Array.from(
      { length: Math.max(1, rollCount) },
      () => ({ quantity: "" })
    ),
...
function syncAutoRollRows(
  colour,
  targetCount = defaultRollRowCount()
) {
  if (!colour) return;

  if (
    !Array.isArray(colour.rolls) ||
    !colour.rolls.length
  ) {
  ...
  /*
    Once Add Roll or Remove is used,
    preserve that colour's uneven roll count.
  */
  if (colour.autoRollTemplate === false) {
    return;
  }
  ...
  /*
    Never delete a row containing quantity
    when Division count is reduced.
  */
  while (colour.rolls.length > target) {
    let removableIndex = -1;

    for (
      let index = colour.rolls.length - 1;
      index >= 1;
      index -= 1
    ) {
...
function syncAllAutoRollRows() {
  ...
  formEntries.forEach(entry => {
    entry.colours.forEach(colour => {
      syncAutoRollRows(colour, target);
    });
  });
}
...
function ensureColourDrafts(count) {
  ...
  formEntries.forEach(entry => {
    while (entry.colours.length < count) {
      entry.colours.push(
        makeColourRollDraft(defaultRows)
      );
    }
    ...
    entry.colours.forEach(colour => {
      syncAutoRollRows(colour, defaultRows);
    });
  });
}
...
function makeEntry({
  ...
  const defaultCategory = categoryCode
    ...
    : categories.find(
        item =>
          item.category_code !== "regular-cloth"
      ) || categories[0];
...
function closeSheet(sheet) {
  ...
  if (
    purchaseSheet.classList.contains("pm-hidden") &&
    detailSheet.classList.contains("pm-hidden") &&
    artSheet.classList.contains("pm-hidden")
  ) {
    ...
    <div class="pm-allocation-box">
      ...
      <div class="pm-allocation-row">
        <label class="pm-radio-chip">
          <input
            type="radio"
            name="scope-${entry.key}"
            value="all"
            ${
              entry.allocationScope === "all"
                ? "checked"
                : ""
            }
          >
...
function colourPreview(index) {
  const colour =
    cbColourDrafts[index] || {};

  const source =
    colour.objectUrl || colour.imageUrl;
...
function renderRolls(entry, colourIndex) {
  const rolls =
    entry.colours[colourIndex]?.rolls || [];
    ...
    >
      ...
      ${
        rollIndex > 0
          ? `
            <button
              class="pm-remove-roll"
              type="button"
              aria-label="Remove Roll ${rollIndex + 1}"
            >
              ×
            </button>
          `
...
function renderColourCard(
  entry,
  entryIndex,
  colourIndex
) {
  const draft =
    cbColourDrafts[colourIndex] || {};

  const isIdentitySource =
    formMode === "create" &&
    entryIndex === 0;
  ...
  return `
    <article
      class="pm-material-colour-card"
      data-colour-index="${colourIndex}"
    >
      <div class="pm-colour-card-head">
        <div class="pm-colour-thumb">
          ${colourPreview(colourIndex)}
        </div>
            ...
            isIdentitySource
              ? `
                <input
                  ...
                  type="text"
                  placeholder="Colour ${colourIndex + 1} name"
      ...
      <div class="pm-roll-list">
        ${renderRolls(entry, colourIndex)}
      </div>
...
function renderEntry(entry, entryIndex) {
  ...
  return `
    <article
      class="pm-purchase-entry"
      data-entry-key="${safe(entry.key)}"
    >
      ...
      <div class="pm-entry-fields">
        ...
        <label>
          ...
          <input
            class="pm-vendor-name"
            type="text"
            placeholder="Vendor name"
            value="${safe(entry.vendorName)}"
          >
        </label>
        ...
        <label>
          ...
          <input
            class="pm-fabric-name"
            type="text"
            placeholder="Fabric name"
            value="${safe(entry.fabricName)}"
          >
        </label>
...
function bindEntryEvents(
  ...
  node.querySelectorAll(
    ".pm-material-colour-card"
  ).forEach(colourNode => {
    const colourIndex = Number(
      colourNode.dataset.colourIndex
    );

    colourNode.querySelector(
      ".pm-colour-name"
    )?.addEventListener(
      "input",
      event => {
      ...
      input.addEventListener(
        ...
        () => {
          ...
          if (
            ...
            say(
              `Colour ${colourIndex + 1}: select an image file.`,
              "error"
            );
    ...
    colourNode.querySelectorAll(
      ...
      const rollIndex = Number(
        rollNode.dataset.rollIndex
      );

      const rollInput =
        rollNode.querySelector(
          ".pm-roll-qty"
        );
...
function entryQuantity(entry) {
  return entryRolls(entry).reduce(
...
function updateGrandSummary() {
  const totalQty = formEntries.reduce(
    (sum, entry) => sum + entryQuantity(entry),
    0
  );
...
function resetCreateForm() {
  ...
  $("customDivisionWrap").classList.add(
    "pm-hidden"
  );

  $("newCbIdentityCard").classList.remove(
    "pm-hidden"
  );

  $("existingCbContext").classList.add(
    "pm-hidden"
  );

  $("purchaseSheetKicker").textContent =
    "CB Purchase";

  $("purchaseSheetTitle").textContent =
    "New CB";
...
function openCreateForm() {
  if (!dataReady || !categories.length) {
    say(
      "Product Master data is not ready. Press Refresh and try again.",
      "error"
    );
    return;
  }
...
function openAppendForm(cbId) {
  ...
  if (!existingColours.length) {
    say(
      `${group.cb_no}: colour records are missing.`,
      "error"
    );
    return;
  }

  cbColourDrafts = existingColours.map(
    colour => ({
  ...
  $("newCbIdentityCard").classList.add(
    "pm-hidden"
  );

  $("existingCbContext").classList.remove(
    "pm-hidden"
  );

  $("existingCbName").textContent =
    group.cb_no;

  $("existingCbMeta").textContent =
    `${group.divisions.length} Divisions · ${cbColourDrafts.length} Colours`;
  ...
  if (!formEntries.length) {
    throw new Error(
      "Add at least one material purchase."
    );
  }

  if (formMode === "create") {
    const regularId =
      categoryByCode("regular-cloth")?.id;

    if (
      !regularId ||
      String(
        formEntries[0].materialCategoryId
      ) !== String(regularId)
    ) {
      throw new Error(
        "First purchase must be Regular Cloth."
      );
    }
  }
  ...
  formEntries.forEach((entry, index) => {
    ...
    if (!entry.materialCategoryId) {
      throw new Error(
        `Purchase ${number}: select Material.`
      );
    }

    if (!entry.vendorName.trim()) {
      throw new Error(
        `Purchase ${number}: enter Vendor Name.`
      );
    }

    if (!entry.fabricName.trim()) {
      throw new Error(
        `Purchase ${number}: enter Fabric Name.`
      );
    }

    if (!entry.billNo.trim()) {
      throw new Error(
        `Purchase ${number}: enter Bill No.`
      );
    }

    if (!entry.billDate) {
      throw new Error(
        `Purchase ${number}: select Bill Date.`
      );
    }

    if (Number(entry.rate || 0) <= 0) {
      throw new Error(
        `Purchase ${number}: enter Rate.`
      );
    }

    if (entryQuantity(entry) <= 0) {
      throw new Error(
        `Purchase ${number}: enter colour-wise roll quantity.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Purchase ${number}: select at least one Division.`
      );
    }
  });

  if (formMode === "create") {
    cbColourDrafts.forEach(
      (colour, index) => {
        if (!colour.name.trim()) {
          throw new Error(
            `Enter Colour ${index + 1} name.`
          );
        }

        if (!colour.file) {
          throw new Error(
            `Select Colour ${index + 1} image.`
          );
        }
        ...
        const colourQty =
          formEntries[0]
            ...
            .reduce(
              (sum, roll) =>
                sum +
                Number(roll.quantity || 0),
              0
            );

        if (colourQty <= 0) {
          throw new Error(
            `Regular Cloth: enter roll quantity for ${
              colour.name.trim() ||
              `Colour ${index + 1}`
            }.`
          );
  ...
  const selectedIndexes =
    entry.allocationScope === "all"
      ? divisions.map(
          item =>
            Number(item.division_index)
        )
      : entry.selectedDivisionIndexes.map(
          Number
        );
...
async function uploadColourMedia(
  cbPurchaseId,
  index,
  name
) {
  ...
  if (!draft?.file) {
    throw new Error(
      `Colour ${index + 1} image is missing.`
    );
  }
  ...
  if (!media) {
    throw new Error(
      `Colour ${index + 1} image upload returned no result.`
    );
  }
...
async function insertPurchaseEntry(
  cbPurchaseId,
  entry,
  divisions,
  colours,
  ...
  const rolls = entryRolls(entry).map(
    roll => ({
      purchase_entry_id: data.id,
      cb_colour_id:
        colours[roll.colourIndex].id,
      roll_no: roll.rollNo,
      quantity: Number(
        roll.quantity.toFixed(3)
      )
    })
  );
  ...
  if (!skipAllocation) {
    ...
    if (!divisionIds.length) {
      throw new Error(
        "No division selected for material allocation."
      );
    }

    const { error: allocationError } =
      await supabaseClient.rpc(
        "rr_allocate_cb_purchase_entry",
        {
...
async function rollbackAppendedEntries(
  entryIds
) {
...
async function saveCreateMode() {
  ...
  const regularQuantity =
    entryQuantity(regularEntry);

  const regularAmount =
    regularQuantity *
    Number(regularEntry.rate);

  const regularRolls =
    entryRolls(regularEntry).length;
  ...
  if (
    $("divisionCount").value ===
      "custom" &&
    !Number(
      $("customDivisionCount").value ||
      0
    )
  ) {
    throw new Error(
      "Enter Custom Division count."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Division count must be between 1 and 50."
    );
  }

  try {
    showPurchaseMessage(
      "Creating CB identity and divisions…",
      "progress"
    );
    ...
    const {
      ...
      "rr_create_cb_v713",
      {
        ...
        p_regular_qty: Number(
          regularQuantity.toFixed(3)
        ),
        ...
        p_fabric_name:
          regularEntry.fabricName.trim(),
        p_regular_division_indexes:
          regularEntry.allocationScope ===
          "all"
            ? currentDivisionChoices().map(
                item => item.index
              )
    ...
    cbPurchaseId =
      normalizeRpcId(rpcData);

    if (!cbPurchaseId) {
      throw new Error(
        "CB was created but its ID was not returned."
      );
    }
    ...
    for (
      ...
      showPurchaseMessage(
        `Uploading colour image ${index + 1} of ${colourCount}…`,
        "progress"
      );

      const media =
        await uploadColourMedia(
          cbPurchaseId,
          index,
          cbColourDrafts[
            index
    ...
    for (
      let entryIndex = 0;
      entryIndex < formEntries.length;
      entryIndex += 1
    ) {
      showPurchaseMessage(
        `Saving purchase ${entryIndex + 1} of ${formEntries.length}…`,
        "progress"
      );

      const entry =
        formEntries[entryIndex];
...
purchaseForm.addEventListener(
  ...
  async event => {
    ...
    try {
      if (
        typeof RR === "undefined" ||
        typeof RR.requireOwner !==
          "function" ||
        (
          formMode === "create" &&
          typeof RR.uploadMedia !==
            "function"
        )
      ) {
        throw new Error(
          "real-common.js owner/media helpers are not available."
        );
      }
      ...
      try {
        ...
        say(
          `Saved successfully. Refresh warning: ${formatSaveError(refreshError)}`,
          "error"
        );
      }
    } catch (error) {
  ...
  return (
    ...
    return normalizeGalleryRow({
      ...
      division_status:
        ...
        "planning",
      allocated_qty:
        division.divided_weight,
      allocated_amount:
        division.divided_amount,
      base_qty:
        division.divided_weight,
      base_amount:
        division.divided_amount,
...
function groupGalleryRows() {
  ...
  galleryRows.forEach(row => {
    ...
    if (
      !group.divisionMap.has(
        divisionKey
      )
    ) {
      group.divisionMap.set(
        divisionKey,
        row
      );
    }
...
function allocationsForDivision(
  divisionId
) {
...
function assignmentForDivision(
  divisionId
) {
...
function assignedPrintsForDivision(
  divisionId
) {
  const assignment =
    assignmentForDivision(
      divisionId
    );
...
function carouselItemsForAssignment(
  art,
  prints = []
) {
  if (!art && !prints.length) {
    return [];
  }
...
function carouselHtml(
  items,
  instanceId,
  emptyLabel = "ART PHOTO"
) {
  if (!items.length) {
    return `
      <div class="pm-art-placeholder">
        <span>${safe(emptyLabel)}</span>
        <small>
          Photo will pull from Master
        </small>
      </div>
    `;
  }
  ...
  return `
    <div
      class="pm-swipe-track"
      data-carousel="${safe(instanceId)}"
    >
  ...
  root.querySelectorAll(
    ...
    button.addEventListener(
      ...
      event => {
        ...
        const holder =
          button.closest(
            ".pm-card-hero,.pm-selected-carousel"
          );
...
function applyCardColumns() {
  ...
  if (!valid.includes(cardColumnCount)) {
    cardColumnCount = 3;
  }
  ...
  gallery.classList.add(
    `pm-cols-${cardColumnCount}`
  );

  if ($("pmCardColumns")) {
    $("pmCardColumns").value =
      String(cardColumnCount);
  }
}
...
function renderGallery() {
  ...
  const cards = divisionCards().filter(
    ({
      group,
      division,
      assignment
      ...
      const purchases =
        purchasesFor(group.cb_id);

      const colours =
        coloursFor(group.cb_id);
        ...
        >
          ...
          <div class="pm-work-caption">
            <div class="pm-caption-id-row">
              ...
              <span>
                <small>CB CHILD</small>
                <strong>
                  ${safe(
                    division.division_code ||
                    `S${division.division_index}`
                  )}
                </strong>
              </span>
            </div>
            ...
            <p class="pm-product-line">
              ${safe(
                art?.product_name ||
                art?.item_name ||
                art?.category ||
                "Art Master selection pending"
       )}
           </p>

            <p class="pm-print-line">
              <small>PRINT</small>
              ${safe(printCaption)}
            </p>
  ...
  function artAssignmentSay(
  text,
  type = ""
) {
  const node =
    $("artAssignmentMessage");

  node.textContent = text || "";
  node.className =
    `rr-message ${type}`.trim();
}

function divisionContext(divisionId) {
  for (
    const group of groupGalleryRows()
  ) {
    const division =
      group.divisions.find(
        row =>
          String(row.division_id) ===
          String(divisionId)
      );
...
function printFrameSummary(print) {
  ...
  if (
    ...
    print.frame_labels.length
  ) {
    return print.frame_labels.join(
      " · "
    );
  }

  return (
    "Frame details inside Print Master"
  );
}
...
function printPickerCardHtml(print) {
  ...
  return `
    <button
      ...
      "
      type="button"
      data-select-print="${safe(print.id)}"
      aria-pressed="${
        selected ? "true" : "false"
      }"
    >
      ...
      >
        ...
        <span class="pm-print-frame-caption">
          ${safe(frameText)}
        </span>
      </span>
    </button>
...
function renderPrintPicker() {
  ...
  const list = printRows.filter(
    print => {
      if (print.is_active === false) {
        return false;
      }
  ...
  $("printPickerGrid").innerHTML =
    list.length
      ? list
          .map(printPickerCardHtml)
          .join("")
      : `
        <div class="pm-art-empty">
          No matching Print found
          in Print Master.
        </div>
      `;

  $("selectedPrintCount").textContent =
  ...
  $("printPickerGrid")
    ...
    .forEach(button => {
      button.addEventListener(
        ...
        () => {
          const id =
            button.dataset.selectPrint;

          const index =
            selectedPrintIds.findIndex(
              item =>
                String(item) ===
                String(id)
            );
...
function openArtAssignment(
  divisionId
) {
  const context =
    divisionContext(divisionId);
  ...
  selectedArtId =
    context.assignment?.art_id ||
    null;

  selectedPrintIds =
    assignedPrintsForDivision(
      divisionId
    ).map(print => String(print.id));

  $("artAssignmentKicker").textContent =
    context.assignment
      ? "Change Art & Print"
      : "Assign Art & Print";

  $("artAssignmentTitle").textContent =
    context.division.division_code ||
    `S${context.division.division_index}`;

  $("artCbNo").textContent =
    context.group.cb_no || "—";

  $("artCbChild").textContent =
    context.division.division_code ||
    `S${context.division.division_index}`;

  $("artCbQty").textContent =
    qty(
      context.division.allocated_qty
    );
...
async function saveArtPrintAssignmentRecord(
  cbUnitId,
  artId,
  printIds
) {
  ...
  console.warn(
    "V715 RPC unavailable; using owner direct-write fallback.",
    rpc.error
  );
...
function rollSummaryForPurchase(
  purchaseEntryId
) {
  ...
  return [...grouped.entries()]
    .map(([colourId, rolls]) => {
      const colour =
        colourMap.get(colourId);

      const rollText = rolls
        .map(
          row =>
            `R${row.roll_no}: ${Number(
              row.quantity || 0
            ).toFixed(3)}`
        )
        .join(" · ");
...
function renderCbDetails(cbId) {
  ...
  const allocations = allocationsForDivision(
    division.division_id
  );

  const assignment = assignmentForDivision(
    division.division_id
  );
...
function refreshOpenDetail() {
  if (
    !activeDetailCbId ||
    detailSheet.classList.contains(
      "pm-hidden"
    )
  ) {
    return;
...
function openCbDetails(cbId) {
  ...
  $("cbDetailTitle").textContent =
    group.cb_no;

  $("cbDetailBody").innerHTML =
    renderCbDetails(cbId);
...
async function loadCbPrintAssignments() {
  const result =
    await supabaseClient
      ...
      .select(
        "id,assignment_id,print_id,sequence_no,created_at,updated_at"
      )
      .order("sequence_no");

  if (result.error) {
    throw new Error(
      `Run V715 separate Art/Print SQL: ${result.error.message}`
    );
  }
...
async function loadMasterMedia(
  ...
  for (
    ...
    const chunk =
      ids.slice(start, start + 80);

    const query =
      await supabaseClient
        .from("rr_media")
        .select(
          "id,entity_type,entity_id,media_category,file_url,file_name,caption,is_cover,sort_order,created_at"
        )
        .in("entity_id", chunk);

    if (query.error) {
      console.warn(
        "Master media could not load.",
        query.error
      );
  ...
  try {
    ...
    if (colourResult.error) {
      throw colourResult.error;
    }

    if (rollResult.error) {
      throw new Error(
        `Run V713 SQL patch: ${rollResult.error.message}`
      );
    }

    if (allocationResult.error) {
      throw new Error(
        `Run V713 SQL patch: ${allocationResult.error.message}`
      );
    }

    if (artResult.error) {
      throw new Error(
        `Art Master could not load: ${artResult.error.message}`
      );
    }

    if (assignmentResult.error) {
      throw new Error(
        `Run V714 Art assignment SQL: ${assignmentResult.error.message}`
      );
    }
    ...
    if (!categories.length) {
      throw new Error(
        "No active material categories were found."
      );
    }
  ...
  .addEventListener(
    ...
    () => {
      cardColumnCount = Number(
        $("pmCardColumns").value || 3
      );

      localStorage.setItem(
        "redzedProductCardColumns",
        String(cardColumnCount)
      );
...
document.addEventListener(
  ...
  event => {
    ...
    if (
      !artSheet.classList.contains(
        "pm-hidden"
      )
    ) {
      closeSheet(artSheet);
    } else if (
      !purchaseSheet.classList.contains(
        "pm-hidden"
      )
    ) {
      closeSheet(purchaseSheet);
...
$("pmSearch").addEventListener(
  "input",
  renderGallery
);

$("refreshCb").addEventListener(
  "click",
  () => {
    withTimeout(
      loadData(),
...
function getSupabaseClient() {
  ...
  try {
    ...
    console.warn(
      "Direct Supabase client check failed",
      error
    );
  }
...
async function waitForRuntime() {
  ...
  if (
    ...
    missing.push(
      "RR from real-common.js"
    );
  }

  throw new Error(
    `Required runtime not found: ${missing.join(", ")}.`
  );
}
...
async function ensureOwnerAccess() {
  ...
  if (!session) {
    if (
      ...
      throw new Error(
        "Owner login helper is not available."
      );
    }
  ...
  if (
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "Owner permission helper timed out; authenticated session exists and database RLS remains active."
        );
...
console.info(
  "REDZED Product Master V716 single-file boot script loaded."
);
  ...
  try {
    gallery.innerHTML = `
      <article class="pm-empty-card">
        ...
        <h3>
          Connecting Product Master V716
        </h3>

        <p>
          Checking login and
          CB Purchase database…
        </p>
      </article>
    `;

    say(
      "Starting Product Master V716…"
    );
    ...
    console.error(
      "Product Master V716 boot failed:",
      error
    );
    ...
    gallery.innerHTML = `
      <article class="pm-empty-card">
        <h3>
          Product Master could not start
        </h3>
