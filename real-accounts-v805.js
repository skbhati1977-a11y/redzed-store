(()=>{
  ...
  async function loadLedgerSummary(){message("ledgerSummaryMsg","Loading…");try{const rows=await rpc("rr_accounts_ledger_summary_v9785",{p_scope:state.summaryScope,p_search:$("ledgerSummarySearch").value||"",p_from_date:$("summaryFrom").value,p_to_date:$("summaryTo").value,p_data_mode:mode()});state.summaryRows=Array.isArray(rows)?rows:[];$("ledgerSummaryResult").innerHTML=renderLedgerSummary(state.summaryRows);message("ledgerSummaryMsg",`${state.summaryRows.length} ledger${state.summaryRows.length===1?"":"s"}.`,"ok")}catch(e){state.summaryRows=[];$("ledgerSummaryResult").innerHTML=`<div class="empty">${esc(errorText(e))}</div>`;message("ledgerSummaryMsg",errorText(e),"error")}}
  function renderLedgerSummary(rows){if(!rows.length)return '<div class="empty">No matching ledger.</div>';const salary=state.summaryScope==="SALARY";return `<div class="scroll"><table class="freeze-first"><thead><tr><th>${salary?"Worker / Staff":"Party / Ledger"}</th>${salary?"<th>Department</th><th>Salary Type</th>":""}<th>Opening</th><th>Pieces</th><th>Total Debit</th><th>Total Credit</th><th>Closing</th><th>Share</th></tr></thead><tbody>${rows.map((r,i)=>`<tr class="ledger-row" data-ledger-id="${esc(r.ledger_id)}" data-ledger-name="${esc(r.ledger_name)}"><td><button type="button" class="suggestion" style="min-height:34px;padding:6px;width:100%;text-align:left">${esc(r.ledger_name)}</button></td>${salary?`<td>${esc(r.department_code||"—")}</td><td>${esc(r.payroll_category==="SALARIED"?"Monthly Salary":"Piece-rate Wages")}</td>`:""}<td>${balanceText(r.opening_balance)}</td><td>${fmt(r.total_pieces||0)}</td><td>${money(r.total_debit)}</td><td>${money(r.total_credit)}</td><td><b>${balanceText(r.closing_balance)}</b></td><td><button type="button" data-share-ledger-summary="${i}">Share Summary</button></td></tr>`).join("")}</tbody></table></div>`}
