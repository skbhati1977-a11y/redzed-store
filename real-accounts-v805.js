(()=>{
  ...
  function wire(){
    ...
    $("closeEntryPopup")?.addEventListener("click",()=>closeEntry());$("sharePopupEntry")?.addEventListener("click",()=>openRealChatShare(state.activeEntry?.posting_lines||[],{kind:"VOUCHER",title:`Voucher ${state.activeEntry?.voucher_no||""}`,ledgerName:"Double-entry Voucher"}));$("shareLedgerRealChat")?.addEventListener("click",()=>openRealChatShare(state.statementRows,{kind:"LEDGER",title:"Ledger Statement",ledgerName:state.statementLedger?.name||"Ledger"}));$("closeRealChatShare")?.addEventListener("click",()=>closeRealChatShare());$("realChatShareSearch")?.addEventListener("input",renderChatShareTargets);$("realChatShareRows")?.addEventListener("click",e=>{const b=e.target.closest("[data-send-account-chat]");if(b)sendAccountsToChat(b.dataset.sendAccountChat,b)});$("statementResult")?.addEventListener("click",e=>{const b=e.target.closest("[data-share-statement-row]");if(!b)return;e.preventDefault();e.stopPropagation();shareVoucherEntry(state.statementRows[Number(b.dataset.shareStatementRow)]?.voucher_no)});$("exportPopupEntry")?.addEventListener("click",()=>downloadCsv(state.activeEntry?.posting_lines||[],"accounts-voucher.csv"));$("printPopupEntry")?.addEventListener("click",()=>window.print());
