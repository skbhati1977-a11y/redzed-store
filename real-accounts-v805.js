(()=>{
  ...
  async function openLedgerStatement(id,name){state.statementLedger={id,name};$("statementLedgerName").textContent=name;$("statementScope").textContent=scopeNames[state.summaryScope]||"Ledger Statement";$("statementFrom").value=$("summaryFrom").value;$("statementTo").value=$("summaryTo").value;$("statementSearch").value="";$("ledgerStatementPage").classList.remove("hidden");history.pushState({accountsView:"statement"},"",`${location.pathname}${location.search}#ledger-statement`);await loadLedgerStatement()}
  function closeLedgerStatement(fromPop=false){$("ledgerStatementPage").classList.add("hidden");state.statementLedger=null;if(!fromPop&&location.hash==="#ledger-statement")history.back()}
  async function loadLedgerStatement(){if(!state.statementLedger)return;message("statementMsg","Loading…");try{const rows=await rpc("rr_accounts_ledger_statement_v9767",{p_ledger_id:state.statementLedger.id,p_from_date:$("statementFrom").value,p_to_date:$("statementTo").value,p_data_mode:mode()});state.statementRows=Array.isArray(rows)?rows:[];renderLedgerStatement();message("statementMsg",`${state.statementRows.length} entr${state.statementRows.length===1?"y":"ies"}.`,"ok")}catch(e){state.statementRows=[];renderLedgerStatement();message("statementMsg",errorText(e),"error")}}
  ...
  function wire(){
    ...
    $("openAccountsMenu")?.addEventListener("click",()=>$("accountsDrawer")?.classList.remove("hidden"));$("closeAccountsMenu")?.addEventListener("click",()=>$("accountsDrawer")?.classList.add("hidden"));
    document.querySelectorAll("[data-open-tab]").forEach(b=>b.addEventListener("click",()=>{document.querySelector(`[data-tab='${b.dataset.openTab}']`)?.click();$("accountsDrawer")?.classList.add("hidden");if(b.dataset.reportQuery&&$("reportSearch")){$("reportSearch").value=b.dataset.reportQuery;searchReports(b.dataset.reportQuery)}if(b.dataset.focusId)setTimeout(()=>$(b.dataset.focusId)?.focus(),80)}));
