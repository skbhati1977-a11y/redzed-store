(()=>{
  ...
  function chatShareText(){const s=state.chatShare||{},rows=s.rows||[],name=s.ledgerName||s.title||"Accounts";if(s.kind==="LEDGER"){const dr=rows.reduce((n,r)=>n+Number(r.debit||0),0),cr=rows.reduce((n,r)=>n+Number(r.credit||0),0),last=rows.at(-1);return `${name} · Ledger Statement\n${$("statementFrom").value} to ${$("statementTo").value}\nEntries: ${rows.length} · Debit: ${money(dr)} · Credit: ${money(cr)}\nClosing: ${last?balanceText(last.running_balance):"—"}`}return `${name} · Accounts Entry\n${entryText(rows[0]).slice(0,3000)}`}
  function renderChatShareTargets(){const q=String($("realChatShareSearch")?.value||"").trim().toLowerCase(),rows=q?state.chatInbox.filter(x=>String(x.customer_name||"").toLowerCase().includes(q)):state.chatInbox;$("realChatShareRows").innerHTML=rows.map(x=>`<button type="button" class="suggestion" data-send-account-chat="${esc(x.chat_id)}"><b>${esc(x.customer_name)}</b><small>Send to this Real Chat</small></button>`).join("")||'<div class="empty">No matching active chat.</div>'}
  async function openRealChatShare(rows,{kind="ENTRY",title="Accounts Entry",ledgerName=""}={}){const list=Array.isArray(rows)?rows.filter(Boolean):[];if(!list.length)return message("statementMsg","No entry available to share.","error");state.chatShare={rows:list,kind,title,ledgerName};const modal=$("realChatSharePopup");modal.classList.remove("hidden");modal.style.display="grid";$("realChatShareSearch").value="";message("realChatShareMsg","Loading authorized chats…");history.pushState({accountsView:"chat-share"},"",`${location.pathname}${location.search}#accounts-chat-share`);try{state.chatInbox=await rpc("rr_chat_staff_inbox_v9704")||[];renderChatShareTargets();message("realChatShareMsg",`${state.chatInbox.length} active chat${state.chatInbox.length===1?"":"s"}.`,"ok")}catch(e){state.chatInbox=[];renderChatShareTargets();message("realChatShareMsg",errorText(e),"error")}}
  ...
  async function sendAccountsToChat(chatId,button){if(!chatId||!state.chatShare)return;setBusy(button,true,"Sending…");try{const s=state.chatShare,text=chatShareText();await rpc("rr_chat_send_staff_v9433",{p_chat_id:chatId,p_channel:"GROUP",p_message_type:"TEXT",p_body:text,p_payload:{source:"ACCOUNTS",share_kind:s.kind,ledger_name:s.ledgerName||null,row_count:s.rows.length,rows:s.rows.slice(0,200)},p_reply_to:null,p_order_session_id:null});message("realChatShareMsg","Shared to Real Chat ✓","ok");setTimeout(()=>closeRealChatShare(),500)}catch(e){message("realChatShareMsg",errorText(e),"error")}finally{setBusy(button,false)}}
  ...
  function wire(){
    ...
    $("closeEntryPopup")?.addEventListener("click",()=>closeEntry());$("sharePopupEntry")?.addEventListener("click",()=>openRealChatShare([state.activeEntry],{title:"Accounts Entry"}));$("shareLedgerRealChat")?.addEventListener("click",()=>openRealChatShare(state.statementRows,{kind:"LEDGER",title:"Ledger Statement",ledgerName:state.statementLedger?.name||"Ledger"}));$("closeRealChatShare")?.addEventListener("click",()=>closeRealChatShare());$("realChatShareSearch")?.addEventListener("input",renderChatShareTargets);$("realChatShareRows")?.addEventListener("click",e=>{const b=e.target.closest("[data-send-account-chat]");if(b)sendAccountsToChat(b.dataset.sendAccountChat,b)});$("statementResult")?.addEventListener("click",e=>{const b=e.target.closest("[data-share-statement-row]");if(!b)return;e.preventDefault();e.stopPropagation();openRealChatShare([state.statementRows[Number(b.dataset.shareStatementRow)]],{title:"Ledger Entry",ledgerName:state.statementLedger?.name||"Ledger"})});$("exportPopupEntry")?.addEventListener("click",()=>downloadCsv([state.activeEntry],"accounts-entry.csv"));$("printPopupEntry")?.addEventListener("click",()=>window.print());
