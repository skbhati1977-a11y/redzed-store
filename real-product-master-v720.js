...
function dateTime(value){if(!value)return "â€”";return new Intl.DateTimeFormat("en-IN",{dateStyle:"medium",timeStyle:"short"}).format(new Date(value));}
...
function errorText(error){return [...new Set([error?.message,error?.details,error?.hint,error?.code].filter(Boolean))].join(" â€” ")||"Unknown error";}
function getClient(){
  let client=null;
...
function cbCardHtml(g,d){
  const unitId=d.division_id;const a=assignmentFor(unitId);const art=artFor(a);const ps=printsForAssignment(a);const cols=coloursFor(g.cbId);const lots=lotsForUnit(unitId);const lotText=lots.map(x=>x.lot_no).filter(Boolean).join(" Â· ");
...
function renderColourList(){if(state.mode!=="create")return;$("colourList").innerHTML=state.colourDrafts.map((c,i)=>`<article class="colour-row" data-colour="${i}"><div class="grid3"><label><span>Colour ${i+1} Name *</span><input class="colour-name" value="${safe(c.name)}"></label><label><span>Image URL â€” optional</span><input class="colour-url" value="${safe(c.imageUrl)}"></label><label><span>Camera / Gallery â€” optional</span><input class="colour-file" type="file" accept="image/*"></label></div>${c.objectUrl||c.imageUrl?`<img src="${safe(c.objectUrl||c.imageUrl)}" style="margin-top:8px;width:90px;height:90px;object-fit:cover;border-radius:12px">`:""}</article>`).join("");
...
function updateCbSummary(){const qtyTotal=state.materialEntries.reduce((s,e)=>s+entryQty(e),0);const valueTotal=state.materialEntries.reduce((s,e)=>s+entryValue(e),0);$("cbTotalQty").textContent=kg(qtyTotal);$("cbTotalValue").textContent=money(valueTotal);$("cbDivisionPreview").textContent=divisionChoices().map(i=>`D${i}`).join(" Â· ");}
function renderCbForm(){renderColourList();renderMaterialList();updateCbSummary();}

function validateEntry(e,index){const q=entryQty(e);if(!e.categoryId)throw new Error(`Purchase ${index+1}: Material à¤šà¥à¤¨à¥‡à¤‚.`);if(!e.vendor.trim())throw new Error(`Purchase ${index+1}: Vendor Name required.`);if(!e.fabric.trim())throw new Error(`Purchase ${index+1}: Fabric / Material Name required.`);if(!e.billNo.trim())throw new Error(`Purchase ${index+1}: Bill No required.`);if(!e.billDate)throw new Error(`Purchase ${index+1}: Bill Date required.`);if(q<=0)throw new Error(`Purchase ${index+1}: Roll Qty required.`);if(entryRate(e)<=0)throw new Error(`Purchase ${index+1}: Bill Rate à¤¯à¤¾ Bill Value required.`);if(entryValue(e)<=0)throw new Error(`Purchase ${index+1}: Bill Value required.`);if(e.scope==="selected"&&!e.selected.length)throw new Error(`Purchase ${index+1}: à¤•à¤® à¤¸à¥‡ à¤•à¤® à¤à¤• Division à¤šà¥à¤¨à¥‡à¤‚.`)}
...
async function saveCbForm(ev){ev.preventDefault();const btn=$("saveCb");setBusy(btn,true,state.mode==="create"?"Creatingâ€¦":"Savingâ€¦");formSay("cbSaveMessage","Validatingâ€¦","info");let createdCbId=null;
  try{
    ...
    const cbNo=$("cbNo").value.trim().toUpperCase();const divisionCount=Math.max(1,Number($("divisionCount").value||0));if(!cbNo)throw new Error("CB No. required.");if(state.colourDrafts.some(c=>!c.name.trim()))throw new Error("à¤¹à¤° Colour à¤•à¤¾ à¤¨à¤¾à¤® required à¤¹à¥ˆ.");const regular=state.materialEntries[0];const rq=entryQty(regular),rv=entryValue(regular);
...
function openMcNew(){if(!state.mc.card&&state.mc.error){say("MC1 SQL patch à¤ªà¤¹à¤²à¥‡ run à¤•à¤°à¥‡à¤‚.","error")}$("mcForm").reset();$("mcBillDate").value=today();$("mcBillRate").value="";$("mcBillValue").value="";state.mcPricingDriver="value";formSay("mcSaveMessage","");openSheet("mcSheet");setTimeout(()=>$("mcVendor").focus(),80)}
...
async function saveMcForm(ev){ev.preventDefault();const btn=$("saveMc");setBusy(btn,true,"Savingâ€¦");formSay("mcSaveMessage","Posting MC1 purchaseâ€¦","info");try{const payload={p_vendor_name:$("mcVendor").value.trim(),p_bill_no:$("mcBillNo").value.trim().toUpperCase(),p_bill_qty:Number($("mcBillQty").value||0),p_bill_value:Number($("mcBillValue").value||0),p_bill_date:$("mcBillDate").value||today(),p_remarks:$("mcRemarks").value.trim()||null};const r=await state.client.rpc("rr_post_mc_purchase_v1",payload);if(r.error)throw r.error;closeSheet("mcSheet");await loadData();say(`MC1 updated Â· ${kg(payload.p_bill_qty)} IN.`,"success")}catch(e){console.error(e);formSay("mcSaveMessage",errorText(e),"error")}finally{setBusy(btn,false)}}
...
function vendorMessageFor(claim){
  ...
  return `Namaste, aapke Bill No. ${claim.vendor_bill_no||"â€”"} ke CB ${claim.cb_no||"â€”"}, Division ${claim.division_code||"â€”"}, ${claim.particular_label||"particular"} me ${Number(claim.damage_qty||0).toFixed(3)} kg damage paya gaya hai. Claim value ${money(claim.claim_value)} hai. Kripya claim, GR ya replacement ki pushti karein.${links.length?`\n\nDamage proof:\n${links.join("\n")}`:""}`;
}
...
function cbPurchaseCard(p,g){
  const grs=grForCbPurchase(p.id),damages=damageForPurchase(p.id),original=Number(p.original_quantity??p.quantity??0),current=Number(p.quantity||0),returned=grQty(grs),damageQty=damages.reduce((s,x)=>s+Number(x.damage_qty||0),0),rolls=rollsForPurchase(p.id),allocs=allocationsForPurchase(p.id);
  return `<article class="history purchase-ops"><div class="card-head"><div><h4>${safe(categoryById(p.material_category_id)?.category_name||"Material")}</h4><p>${safe(p.fabric_name||"")} Â· <strong>${safe(p.vendor_name||"")}</strong></p><small>Bill ${safe(p.vendor_bill_no||"")} Â· ${safe(p.bill_date||"")}</small></div><div><strong>${kg(current)}</strong><small>${money(p.rate)}/kg Â· Current ${money(current*Number(p.rate||0))}</small></div></div>
    ...
    <div class="table-wrap compact"><table><thead><tr><th>Particular</th><th>Colour</th><th>Original</th><th>Available</th></tr></thead><tbody>${rolls.map(r=>`<tr><td>Roll ${safe(r.roll_no)}</td><td>${safe(colourById(r.cb_colour_id)?.colour_name||"â€”")}</td><td>${kg(r.original_quantity??r.quantity)}</td><td><strong>${kg(r.quantity)}</strong></td></tr>`).join("")||`<tr><td colspan="4">No roll row.</td></tr>`}</tbody></table></div>
    <div class="allocation-line"><small>Division allocation:</small> ${allocs.map(a=>`${safe(canonicalD(divisionById(a.division_id)))} ${kg(a.allocated_qty)}`).join(" Â· ")||"â€”"}</div>
    <div class="card-actions">${current>0?`<button class="secondary" data-cb-gr="${safe(p.id)}" data-mode="PARTIAL">Partial GR</button><button class="danger" data-cb-gr="${safe(p.id)}" data-mode="FULL">Full GR</button><button class="warning" data-damage="${safe(p.id)}">Defect / Damage</button>`:""}</div>
...
function openCbDetails(cbId){
  ...
  $("detailBody").innerHTML=`<section class="form-card"><div class="summary"><div><small>CB No.</small><strong>${safe(g.cbNo)}</strong></div><div><small>D Cards</small><strong>${g.divisions.map(canonicalD).join(" Â· ")}</strong></div><div><small>Colours</small><strong>${cols.length}</strong></div><div><small>Purchases</small><strong>${ps.length}</strong></div></div><div class="card-actions"><button id="detailAddMaterial" class="primary">+ Add Cuff / Collar / Other Material</button></div></section><section class="form-card spaced"><h3>Purchases, GR, Exchange à¤”à¤° Damage</h3><div class="history-list">${ps.map(p=>cbPurchaseCard(p,g)).join("")||"No purchase"}</div></section>`;
...
function divisionOptions(cbId,selected=""){const g=groups().find(x=>String(x.cbId)===String(cbId));return (g?.divisions||[]).map(d=>`<option value="${safe(d.division_id)}" ${String(d.division_id)===String(selected)?"selected":""}>${safe(canonicalD(d))} Â· ${kg(d.allocated_qty)}</option>`).join("");}
function rollOptions(purchaseId,selected="",allowAuto=true){const rows=rollsForPurchase(purchaseId).filter(x=>Number(x.quantity||0)>0);return `${allowAuto?`<option value="">Auto across available rolls</option>`:""}${rows.map(r=>`<option value="${safe(r.id)}" ${String(r.id)===String(selected)?"selected":""}>Roll ${safe(r.roll_no)} Â· ${safe(colourById(r.cb_colour_id)?.colour_name||"")} Â· ${kg(r.quantity)}</option>`).join("")}`;}
function colourOptions(cbId){return coloursFor(cbId).map(c=>`<option value="${safe(c.id)}">${safe(c.colour_name)}</option>`).join("");}

function openCbGr(id,mode){const p=purchaseById(id);if(!p)return;showOperation({kind:"CB_GR",purchaseId:id,mode,qty:Number(p.quantity||0)});}
...
function renderOperation(){
  ...
  if(op.kind==="CB_GR"||op.kind==="MC_GR"){
    const p=op.kind==="CB_GR"?purchaseById(op.purchaseId):mcPurchaseById(op.purchaseId);const full=op.mode==="FULL";operationHeader("GOODS RETURN",`${full?"Full":"Partial"} GR`,`${p?.vendor_name||""} Â· Bill ${p?.vendor_bill_no||p?.bill_no||""}`);
    ...
    const g=grById(op.grId),old=op.kind==="CB_EXCHANGE"?purchaseById(g.cb_purchase_entry_id):mcPurchaseById(g.mc_purchase_id);operationHeader("EXCHANGE IN",op.kind==="CB_EXCHANGE"?"Replacement Roll Receive":"MC1 Exchange Receive",`Against GR-${g.gr_no} Â· ${old?.vendor_name||""}`);
    ...
    const p=purchaseById(op.purchaseId);operationHeader("DEFECT / DAMAGE","Create Vendor Damage Claim",`${p.vendor_name} Â· Bill ${p.vendor_bill_no}`);
    $("opsBody").innerHTML=`<section class="form-card"><div class="grid2"><label><span>Division *</span><select id="opDivision">${divisionOptions(p.cb_id)}</select></label><label><span>Roll / Particular</span><select id="opRoll">${rollOptions(p.id,"",false)}</select></label><label><span>Damage Found At *</span><select id="opStage"><option value="BEFORE_CUTTING">Before Cutting â€” reduce CB stock</option><option value="IN_LOT">During Cutting â€” deduct Lot costing</option></select></label><label id="opLotWrap" class="hidden"><span>Lot No. *</span><input id="opLotNo"></label><label><span>Damage Qty *</span><input id="opQty" type="number" min="0.001" step="0.001"></label><label><span>Damage Date *</span><input id="opDate" type="date" value="${today()}"></label><label><span>Reason *</span><input id="opReason" placeholder="Hole, stain, shade, weaving defect..."></label><label><span>Admin WhatsApp No. *</span><input id="opAdminPhone" inputmode="tel" value="${safe(adminPhoneDefault())}"></label></div><label class="spaced-top"><span>Remarks</span><textarea id="opRemarks" rows="2"></textarea></label><div class="grid2 spaced-top"><label><span>Images â€” max 5</span><input id="opImages" type="file" accept="image/*" multiple></label><label><span>Short Video / Screen Recording â€” 1</span><input id="opVideo" type="file" accept="video/*"></label></div><label class="check-line"><input id="opSendAdminNow" type="checkbox" checked> Save à¤¹à¥‹à¤¨à¥‡ à¤•à¥‡ à¤¬à¤¾à¤¦ Admin WhatsApp à¤–à¥‹à¤²à¥‡à¤‚</label><p class="note">Message à¤•à¥‡à¤µà¤² Admin à¤•à¥‡ à¤²à¤¿à¤ à¤¬à¤¨à¥‡à¤—à¤¾à¥¤ Vendor WhatsApp à¤•à¥‡à¤µà¤² Admin verification à¤•à¥‡ à¤¬à¤¾à¤¦ Owner à¤­à¥‡à¤œà¥‡à¤—à¤¾à¥¤</p></section>`;
    ...
    const c=claimById(op.claimId);operationHeader("OWNER ACTION","Vendor WhatsApp Message",`Admin verified Â· Damage-${c.damage_no}`);
    $("opsBody").innerHTML=`<section class="form-card"><label><span>Vendor WhatsApp No. *</span><input id="opVendorPhone" inputmode="tel" value="${safe(c.vendor_phone||"")}"></label><label class="spaced-top"><span>Message â€” Owner may edit</span><textarea id="opVendorMessage" rows="10">${safe(c.vendor_message||vendorMessageFor(c))}</textarea></label><p class="note">Only Owner role can send this Vendor message.</p></section>`;$("opsSave").textContent="Open Vendor WhatsApp";
  }
}
...
async function uploadDamageProof(claim,images,video){
  ...
  for(const item of files){const result=await RR.uploadMedia({file:item.file,entityType:"cb",entityId:String(claim.cb_id),mediaCategory:"damage",sourceType:"gallery",visibilityScope:"factory",caption:`Damage-${claim.damage_no} Â· ${claim.particular_label||"proof"}`});const m=result?.data||result;if(!m?.id)throw new Error(`Proof upload failed: ${item.file.name}`);const attach=await state.client.rpc("rr_attach_damage_media_v1",{p_claim_id:claim.id,p_media_id:m.id,p_media_type:item.type,p_file_url:m.file_url||m.public_url||m.url||null,p_file_name:item.file.name});if(attach.error)throw attach.error;uploaded.push({file_url:m.file_url||m.public_url||m.url||null,media_type:item.type})}
  return uploaded;
}
async function submitOperation(ev){
  ev.preventDefault();const op=state.operation,btn=$("opsSave");setBusy(btn,true,"Savingâ€¦");formSay("opsMessage","Processingâ€¦","info");
  try{
    let result,message;
...
async function saveAssignment(){if(!state.selectedArtId){formSay("assignMessage","Art select à¤•à¤°à¥‡à¤‚.","error");return}const btn=$("saveAssignment");setBusy(btn,true,"Savingâ€¦");try{const na=state.selectedPrintIds.includes("__NA__");const clean=state.selectedPrintIds.filter(x=>x!=="__NA__");if(!na){const rpc=await state.client.rpc("rr_save_cb_art_print_assignment",{p_cb_unit_id:state.activeUnitId,p_art_id:state.selectedArtId,p_print_ids:clean});if(!rpc.error){closeSheet("assignSheet");await loadData();say("Art / Print decision saved.","success");return}if(!/function|schema cache|PGRST202/i.test(`${rpc.error.code} ${rpc.error.message}`))throw rpc.error}
