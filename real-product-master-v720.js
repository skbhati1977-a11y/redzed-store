...
window.REDZED_PRODUCT_MASTER_VERSION = "720.35-CUTTING-ORIGIN-CB-LEDGER";
...
function syncEntryPricing(e,source,node){
  ...
  if(node){
    ...
    const rateText=node.querySelector(".entry-rate"),qtyText=node.querySelector(".entry-qty"),valueText=node.querySelector(".entry-value"),rollText=node.querySelector(".entry-rolls");
    if(rateText)rateText.textContent=money(entryRate(e));
    if(qtyText)qtyText.textContent=kg(q);
    if(valueText)valueText.textContent=money(entryValue(e));
    if(rollText)rollText.textContent=String(e.rolls.flat().filter(r=>Number(r.qty||0)>0).length);
  }
  updateCbSummary();
}
function renderMaterialList(){
  state.materialEntries.forEach(ensureEntryRolls);
  $("materialList").innerHTML=state.materialEntries.map((e,ei)=>`<article class="material-row" data-entry="${safe(e.id)}"><div class="material-head"><h4>${e.type==="regular"?"Regular Cloth":"Cuff / Collar / Other Material"}</h4>${e.type!=="regular"?`<button class="danger remove-entry" type="button">Remove</button>`:""}</div><div class="grid3"><label><span>Material *</span><select class="mat-category" ${e.type==="regular"?"disabled":""}>${materialOptions(e.categoryId)}</select></label><label><span>Vendor Name *</span><input class="mat-vendor" value="${safe(e.vendor)}"></label><label><span>Fabric / Material Name *</span><input class="mat-fabric" value="${safe(e.fabric)}"></label><label><span>Bill No. *</span><input class="mat-bill" value="${safe(e.billNo)}"></label><label><span>Bill Date *</span><input class="mat-date" type="date" value="${safe(e.billDate)}"></label><label><span>Bill Rate *</span><input class="mat-rate" type="number" min="0" step="0.0001" value="${safe(e.billRate)}"></label><label><span>Bill Value *</span><input class="mat-value" type="number" min="0" step="0.01" value="${safe(e.billValue)}"></label></div><div class="allocation"><label><input type="radio" name="scope-${safe(e.id)}" value="all" ${e.scope==="all"?"checked":""}>All Divisions</label><label><input type="radio" name="scope-${safe(e.id)}" value="selected" ${e.scope==="selected"?"checked":""}>Selected Divisions</label>${divisionChoices().map(i=>`<label class="division-check ${e.scope==="selected"?"":"hidden"}"><input type="checkbox" value="${i}" ${e.selected.includes(i)?"checked":""}>D${i}</label>`).join("")}</div><div class="rolls">${e.rolls.map((rolls,ci)=>`<section><strong>${safe(state.colourDrafts[ci]?.name||`Colour ${ci+1}`)}</strong><div class="roll-set" data-colour-index="${ci}">${rolls.map((r,ri)=>`<div class="roll-row" data-roll="${ri}"><b>Roll ${ri+1}</b><input class="roll-qty" type="number" min="0" step="0.001" placeholder="Qty kg" value="${safe(r.qty)}"><button class="danger remove-roll" type="button" ${ri===0?"disabled":""}>×</button></div>`).join("")}<button class="secondary add-roll" type="button" style="margin-top:7px">+ Add Roll</button></div></section>`).join("")}<div class="summary purchase-auto-summary" style="margin-top:12px"><div><small>Bill Qty — Rolls से</small><strong class="entry-qty">${kg(entryQty(e))}</strong></div><div><small>Effective Bill Rate</small><strong class="entry-rate">${money(entryRate(e))}</strong></div><div><small>Bill Value</small><strong class="entry-value">${money(entryValue(e))}</strong></div><div><small>Total Rolls</small><strong class="entry-rolls">${e.rolls.flat().filter(r=>Number(r.qty||0)>0).length}</strong></div><div><small>Allocation</small><strong>${e.scope==="all"?"All D":(e.selected.length?e.selected.map(i=>`D${i}`).join(" · "):"Select D")}</strong></div><div><small>Material Type</small><strong>${e.type==="regular"?"Regular Cloth":"Material"}</strong></div></div></div></article>`).join("");
