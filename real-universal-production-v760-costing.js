...
const VERSION = "V760_COSTING_FOUNDATION_UI";
...
async function openCostingPanelV760(canonical, focusDepartment = "") {
  ...
  overlay.querySelectorAll(".v760-save-rate").forEach(button => {
    button.onclick = async () => {
      const row = button.closest(".v760-rate-row");
      const department = button.dataset.department;
      const rate = Number(row.querySelector(".v760-actual-rate")?.value || 0);

      if (rate <= 0) {
        alert("Actual Rate zero se zyada fill karein.");
        return;
      }
      ...
      try {
        const client = getClient();
        const requestId =
          pendingV760Submit?.departmentCode === department
        ...
        alert(`${department} Actual Rate save ho gayi.`);

        const resume =
          pendingV760Submit
          && pendingV760Submit.canonical === canonical
          && pendingV760Submit.departmentCode === department;

        closeCostingPanelV760();

        if (resume) {
          const context = pendingV760Submit;
          pendingV760Submit = null;

          await ensureDepartmentContext(context.rowData.department_code);
          selectColourCard(context.rowData.colour_code);
          clickExistingButton("submitBtn");
        } else {
          await openCostingPanelV760(canonical, department);
        }
      } catch (error) {
