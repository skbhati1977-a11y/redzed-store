...
function loadScript(src,timeoutMs){
  return new Promise((resolve,reject)=>{
    ...
    const finish=(error)=>{
      ...
      script.onload=null;
      script.onerror=null;
      if(error){try{script.remove();}catch(_){} reject(error)}else resolve();
    };
...
setTimeout(()=>{
  ...
  if(!text.includes('loading cutting master')&&!text.includes('connecting')&&!text.includes('loading inline product cards')&&!text.includes('starting cutting master'))return;
...
(async()=>{
  try{
    ...
    if(!window.supabaseClient&&!window.supabaseDb&&!window.redzedSupabase&&!window.sb)throw new Error('Factory data client was not created');
