...
async function loadContext() {
  ...
  try {
    ...
    state.context = await rpc("rr_upm_universal_form_v733", {
    ...
    if (state.context?.lot) {
      ...
      [["cb_no","identityCb"],["art_no","identityArt"],["print_no","identityPrint"],["frame_no","identityFrame"]].forEach(([key,id]) => {
        ...
        if ($(id)) $(id).textContent = value || "MAPPING REQUIRED";
        if (value) state.lot[key] = value;
      });
    }
...
function renderSummary() {
  ...
  $("summary").innerHTML = [
    ["Inbound", total.inbound, ""], ["Assigned", total.assigned, ""],
    ["Good", total.good, "good"], ["Ready Submit", total.ready_to_submit, "good"],
    ["Outbound", total.outbound, ""], ["Direct Pending", total.pending, "pending"],
  ...
  if (freeze) freeze.innerHTML = [
    ["ALTER PENDING", total.alter, "alter"], ["LINE MAN PENDING", total.line_man_pending, "remake"],
    ["WORKER REMAKE PENDING", total.remake, "remake"], ["DAMAGE", total.damage, "damage"]
  ].map(([label,value,kind]) => `<div class="freeze-card ${kind}${num(value)>0 ? " blink-live" : ""}"><span>${label}</span><strong>${num(value)} PCS</strong></div>`).join("");
}
  ...
  $("colours").innerHTML = colourGroups.map(group => {
    ...
    return `<article class="colour-card ${assigned ? "assigned" : ""} ${done ? "done" : ""} ${!assigned && !canAssign ? "waiting" : ""}" data-colour-key="${esc(rowKey(group))}">
      ...
      <div class="size-wrap"><table>
        <thead><tr><th>Size</th><th>Cut</th><th>Inbound</th><th>Direct Pending</th><th>Alter Fill</th><th>Alter Pending</th><th>Remake Issue</th><th>Line Man Pending</th><th>Remake Delivered</th><th>Worker Remake Pending</th><th>Remake Submit</th><th>Saved Damage</th><th>Add Damage</th><th>Damage From</th><th>Good Total</th><th>Ready Submit</th><th>Outbound</th><th>Status</th></tr></thead>
        <tbody>${group.rows.map((row, rowIndex) => {
          ...
          return `<tr data-row-index="${group.indexes[rowIndex]}">
            ...
            <td>${num(row.good_qty)}</td><td>${num(row.submit_ready_qty)}</td><td>${num(row.outbound_qty)}</td><td>${esc(row.status)}</td>
          </tr>`;
        }).join("")}</tbody>
      </table></div>
    </article>`;
...
async function runDebug() {
  ...
  try {
    ...
    const output = await rpc("rr_upm_debug_lot_flow_v733", {
      p_canonical_lot_id: state.lot.canonical_lot_id,
      p_department_code: $("dept").value
    });
...
async function saveAlterEvidence() {
  const files = [...($("alterEvidenceFiles").files || [])].slice(0, 3);
