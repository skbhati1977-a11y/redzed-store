...
function installStyles() {
  if (byId("mcSearchableMappingStyle9139")) return;
  const style = document.createElement("style");
  style.id = "mcSearchableMappingStyle9139";
  style.textContent = `
    .mc-search-wrap-9139{position:relative}
    .mc-suggest-9139{position:absolute;left:0;right:0;top:calc(100% + 4px);z-index:3000;display:none;max-height:260px;overflow:auto;border:1px solid #5b3b3f;border-radius:12px;background:#35241e;box-shadow:0 18px 42px rgba(0,0,0,.45)}
    .mc-suggest-9139.open{display:block}
    .mc-suggest-9139 button{width:100%;display:block;text-align:left;border:0;border-bottom:1px solid rgba(255,255,255,.12);border-radius:0;background:transparent;color:#fff;padding:11px 12px;font-weight:800}
    .mc-suggest-9139 button:last-child{border-bottom:0}
    .mc-suggest-9139 small{display:block;margin-top:3px;color:#d0c3bd;font-weight:600}
    .mc-add-row-9139{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}
    .mc-add-row-9139 button{flex:1}
  `;
...
function wrapInput(input, listId) {
  ...
  let wrap = input.closest(".mc-search-wrap-9139");
  if (!wrap) {
    wrap = document.createElement("div");
    wrap.className = "mc-search-wrap-9139";
  ...
  if (!list) {
    ...
    list.id = listId;
    list.className = "mc-suggest-9139";
...
function addButton(id, text, afterEl, onClick) {
  ...
  let row = afterEl.parentElement?.querySelector(".mc-add-row-9139");
  if (!row) {
    row = document.createElement("div");
    row.className = "mc-add-row-9139";
...
function ensureFabricSearch() {
  ...
  const parts = wrapInput(input, "mcFabricSuggest9139");
  ...
  addButton("mcAddFabricBtn9139", "+ New Matching Fabric Name", input, () => {
...
function ensureVendorSearch() {
  ...
  input.autocomplete = "off";
  const parts = wrapInput(input, "mcVendorSuggest9139");
  if (!parts) return input;
  ...
  addButton("mcAddVendorBtn9139", "+ New Vendor Name", input, () => {
    input.value = "";
    input.dataset.supplierLedgerId = "";
...
async function refreshFabricSearch() {
  ...
  try {
    ...
    console.warn("MC fabric searchable mapping v9139 failed", error);
  }
}
...
async function refreshVendorSearch() {
  ...
  try {
    ...
    console.warn("MC vendor searchable mapping v9139 failed", error);
  }
}
