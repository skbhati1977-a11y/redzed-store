...
for(const text of ["rr_real_chat_work_inbox_v72","rr_real_chat_work_search_v2","rr_real_chat_message_bridge_v70","rr_real_chat_mark_receipt_v70","IDENTITY_FIRST_V83","RECEIPTS_ONLY_ACTIONS_USE_EXISTING_FORMS","Art","Print","Sticker","Metal ID"])assert.ok(js.includes(text),`Missing live adapter contract: ${text}`);
...
for(const text of ["receiver_name:p.receiver_name","message:message||p.action_message||friendlyAction","action-note","test70-real-chat-live-v70.js?v=142"])assert.ok(js.includes(text)||html.includes(text),`Missing on-behalf/action-message UI contract: ${text}`);
...
for(const text of ["function flowCard","ART_DECIDE","READY_FOR_CUTTING","CUTTING_RELEASED","['OPEN','WORKING','REOPENED']","test70-real-chat-live-v70.js?v=142","PERSONAL CHAT\")+' · '+S.status"])assert.ok(js.includes(text)||html.includes(text),`Missing V113 conversational renderer: ${text}`);
