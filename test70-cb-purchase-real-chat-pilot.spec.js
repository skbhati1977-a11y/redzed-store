...
for(const text of ["receiver_name:p.receiver_name","message:message||p.action_message||friendlyAction","action-note","test70-real-chat-live-v70.js?v=113"])assert.ok(js.includes(text)||html.includes(text),`Missing on-behalf/action-message UI contract: ${text}`);
...
for(const text of ["function flowCard","ART_DECIDE","READY_FOR_CUTTING","CUTTING_RELEASED","['OPEN','WORKING','REOPENED']","test70-real-chat-live-v70.js?v=113","PERSONAL CHAT\")+' · '+S.status"])assert.ok(js.includes(text)||html.includes(text),`Missing V113 conversational renderer: ${text}`);
...
for(const text of ["next=arr(p.next_actions)","next.length?next","source_status:isPurchase?status"])assert.ok(js.includes(text),`Missing exact multi-action renderer: ${text}`);
