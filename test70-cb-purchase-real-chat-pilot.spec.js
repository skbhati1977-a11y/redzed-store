...
for(const text of ["receiver_name:p.receiver_name","message:p.message||p.action_message||friendlyAction","action-note","test70-real-chat-live-v70.js?v=98"])assert.ok(js.includes(text)||html.includes(text),`Missing on-behalf/action-message UI contract: ${text}`);
...
for(const text of ["function flowCard","ART_DECIDE","READY_FOR_CUTTING","CUTTING_RELEASED","['OPEN','WORKING','REOPENED']","test70-real-chat-live-v70.js?v=98","for(const candidate of ['OPEN','WORKING','CLOSE'])","PERSONAL CHAT\")+' · '+S.status"])assert.ok(js.includes(text)||html.includes(text),`Missing V98 conversational renderer: ${text}`);
