...
for(const text of ["messageSide(c,kind,participantName)","message_side:messageSide(c,kind,name)","data-side=\"'+side+'\"","work-card.outgoing","work-card.incoming:before","work-card.outgoing:before"])assert.ok(js.includes(text)||html.includes(text),`Missing directional chat bubble contract: ${text}`);
...
for(const text of ["Performed by "," · on behalf of ","receiver_name:p.receiver_name","test70-real-chat-live-v70.js?v=82"])assert.ok(js.includes(text)||html.includes(text),`Missing on-behalf UI contract: ${text}`);
