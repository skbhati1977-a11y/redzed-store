...
function show(lv){
 ...
 if(!isMobile())return;
 if(lv==='inbox'||lv==='base') $('inbox')?.classList.remove('hide');
 else $('inbox')?.classList.add('hide');
}
function go(lv,replace=false){
 const h='#rr-'+lv;
 if(location.hash===h){show(lv);return}
 if(replace) history.replaceState({rrChat:true,level:lv},'',h);
