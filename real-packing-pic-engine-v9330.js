(()=>{
  ...
  async function loadRate(statusOnly=false){const l=lot();if(!l)return{approved:false,status:"NOT_REQUESTED"};const s=await rpc("rr_pack_rate_status_v9340",{p_lot_no:l,p_data_mode:MODE});if(l!==lot()||$("packWorkspace")?.hidden)return s;rateSnapshot={lot:l,...s};const st=$("rrRateStatus"),req=$("rrRequestRate"),requested=!['NOT_REQUESTED','NONE'].includes(String(s.status||'').toUpperCase());if(st){const text=s.approved?`APPROVED${s.art_code?` · ${esc(s.art_code)}`:''}`:s.status==="NOT_REQUESTED"?"Approval not requested":`${esc(s.status)} · Admin action pending`;const html=`<span class="fg-chip"><b>${text}</b></span>`;if(st.innerHTML!==html)st.innerHTML=html}
    if(req){req.hidden=requested||!!s.approved;req.disabled=!algoReady()||requested}
    const run=$("runPackAlgo"),pcs=$("packPcsPerBox")?.closest?.('.fg-field');if(run)run.hidden=requested;if(pcs)pcs.hidden=requested;
    if(!statusOnly)$("rrSourcePics").hidden=s.visibility==='PACKING_STATUS_ONLY'&&requested;return s}
