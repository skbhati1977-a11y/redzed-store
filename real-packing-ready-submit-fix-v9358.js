(()=>{
'use strict';
/* Single permanent Ready Packing Lots transport.
   IMPORTANT: disable the older manager-behalf Ready Lots wrapper first so only one transport owns this RPC. */
window.__RR_PACK_SUBMIT_SYNC_9356__=true;
window.__RR_PACK_READY_REST_BRIDGE_9344__=true;
if(window.__RR_PACK_READY_RPC_FIX_9369__)return;
window.__RR_PACK_READY_RPC_FIX_9369__=true;
...
async function readyRest(c,args){
  const sessionRes=await bounded(c.auth.getSession(),2500,'Login session timeout');
...
function install(){
  ...
  if(c.__rrReadyOnly9369)return true;
  ...
  c.rpc=function(name,args,options){
    ...
    return (async()=>{
      let last=null;
      for(let i=0;i<2;i++){
        try{return {data:await readyRest(c,args),error:null,status:200};}
        catch(e){last=e;console.warn(`Ready Lots transport attempt ${i+1} failed`,e);if(i===0)await sleep(300);}
      }
      return {data:null,error:last||new Error('Ready Lots could not load')};
    })();
  };
  c.__rrReadyOnly9369=true;
  return true;
}

/* Install as early as possible; also listen for the config-ready event. */
install();
window.addEventListener('redzed:supabase-ready',install,{once:true});
[0,25,75,150,300,600,1200].forEach(ms=>setTimeout(install,ms));

/* If old core call started before this transport owned the RPC, force one fresh call after binding. */
let kicked=false;
function kickIfStuck(){
  ...
  if(!cards||!btn||typeof btn.onclick!=='function')return;
  if(/Ready lots load ho rahe hain/i.test(String(cards.textContent||''))){
    kicked=true;
    btn.click();
  }
}
[350,700,1200,2000].forEach(ms=>setTimeout(kickIfStuck,ms));
})();