...
/*
  REDZED REAL — Cutting Master PM Core V719.2
...
function releaseErrorText(error) {
  ...
  if (/dev code must be d1|variant[_ ]code|d1, d2, d3 or d4/i.test(text)) {
    return `${text} — Supabase में REDZED_V719_1_MULTI_SUBDEV_RELEASE_PATCH.sql run करें.`;
  }
...
function productionPurchaseExists(cbId) {
  return purchaseRows.some(row => String(row.cb_id) === String(cbId));
}
...
async function loadMatchingStockSource(client) {
  ...
  if (!tableResult.error) {
    return (tableResult.data || []).map(normalizeMatchingStockRow);
  }

  console.warn(
    "rr_matching_stock_balance unavailable; trying matching stock RPC.",
    tableResult.error
  );
  ...
  if (!rpcResult.error) {
    return (rpcResult.data || []).map(normalizeMatchingStockRow);
  }
...
window.RRCuttingMasterPM = {
  version: "pm-core-v719.2-matching-stock-card-raw-cost",
