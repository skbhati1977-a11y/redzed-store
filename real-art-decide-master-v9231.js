(()=>{
"use strict";
if(window.__RR_ART_DECIDE_MASTER_9232__)return;
window.__RR_ART_DECIDE_MASTER_9232__=true;

const $=id=>document.getElementById(id);
const state={
  client:null,role:null,filter:"all",decisions:[],counts:{},units:[],purchases:[],
  arts:[],prints:[],stickers:[],metals:[],assignments:[],printAssignments:[],
...
const MASTER_META={
  art:{label:"Art",url:"art-v9148/?v=9232&from=art-decide",focus:"artNo"},
  print:{label:"Print",url:"real-print-master.html?v=9232&from=art-decide",focus:"printNo"},
  sticker:{label:"Sticker",url:"real-sticker-master-v804.html?v=9232&from=art-decide",focus:"itemNo",open:"newItem"},
  metal:{label:"Metal ID",url:"real-metal-id-master-v804.html?v=9232&from=art-decide",focus:"itemNo",open:"newItem"}
};
...
async function loadData(){
  ...
  try{
    const [allR,countR,arts,prints,stickers,metals,assignments,printAssignments,stickerAssignments,metalAssignments,stickerInstructions,metalInstructions]=await Promise.all([
    ...
    Object.assign(state,{units,purchases,arts:activeOnly(arts),prints:activeOnly(prints),stickers:activeOnly(stickers),metals:activeOnly(metals),assignments,printAssignments,stickerAssignments,metalAssignments,stickerInstructions,metalInstructions});
...
function bind(){
  ...
  document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;if(!$("masterSheet").classList.contains("hidden"))closeMasterSheet(true);else if(!$("decisionSheet").classList.contains("hidden"))closeDecision()});
}
