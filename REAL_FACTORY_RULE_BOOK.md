...
- Department Head canonical actual rate applies to all colours. Only if legacy assignment rows contain differing actual rates and no canonical Head rate exists, department cost is PCS-weighted: `sum(actual rate × assigned PCS) / sum(rated PCS)`. Standard rate is never presented as actual.
- Sales enters the Sales Suggestion. Admin/Owner reviews mapped material cost, relevant department actual/weighted rates and Sales Suggestion, then enters the whole-rupee Final Sale Rate. Approval writes through the existing RRQ engine; App and Real Chat read the same approval/RRQ rows.
- Any applicable journey department missing its actual rate blocks rate request/final approval. Approved historical rows remain immutable; open legacy, present and future rows use this contract.
