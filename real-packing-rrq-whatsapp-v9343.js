(()=>{
  ...
  function phone(v){const d=String(v||'').replace(/\D/g,'');return d.length===10?'91'+d:d;}
  function waUrl(item){const p=phone(item?.mobile),t=String(item?.message_text||'').trim();if(!p||!t)return'';return `https://wa.me/${p}?text=${encodeURIComponent(t)}`;}
  async function openPending(l){
    for(let attempt=0;attempt<8;attempt++){
      ...
      if(!url)throw Error('WhatsApp number/message missing');
      try{await rpc('rr_comm_mark_whatsapp_opened_v853',{p_message_id:item.message_id});}catch(_){/* do not block open */}
      show('Request save ho gayi · WhatsApp open ho raha hai. Send button manually press karein.','ok');
