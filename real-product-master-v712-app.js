...
async function rollbackCreatedCb(cbId, uploadedMedia) {
  ...
  await attempt("division rows", () =>
    supabaseClient.from("rr_cb_units").delete().eq("purchase_id", cbId)
  );

  await attempt("fabric purchase", () =>
    supabaseClient.from("rr_fabric_purchases").delete().eq("id", cbId)
  );
...
function getSupabaseClient() {
  ...
  /*
   * config.js commonly declares:
   *   const supabaseClient = supabase.createClient(...)
   *
   * A top-level const is available to later classic scripts, but it does not
   * appear as window.supabaseClient. Therefore check the direct identifier
   * first and only then check common window aliases.
   */
  try {
    if (
  ...
  if (!client) {
    ...
    client = candidates.find(candidate =>
      candidate &&
  ...
  /*
   * If config.js used another window alias, expose the selected client under
   * the name used by the rest of Product Master.
   */
...
async function waitForRuntime() {
  ...
  while (Date.now() - startedAt < 10000) {
    const clientReady = Boolean(getSupabaseClient());
  ...
  if (!getSupabaseClient()) {
    missing.push("Supabase client from config.js");
  }
  ...
  if (
    ...
    missing.push("RR from real-common.js");
  }

  throw new Error(
    `Required runtime not found: ${missing.join(", ")}. Check that config.js and real-common.js are in the same folder as this page.`
  );
}
...
console.info("REDZED Product Master V712 boot script loaded.");
  ...
  try {
    gallery.innerHTML = `
      <article class="pm-empty-card">
        ...
        <h3>Connecting Product Master V712</h3>
        <p>Checking login and database…</p>
      </article>
    `;

    say("Starting Product Master V712…");
    ...
    console.error("Product Master V712 boot failed:", error);
    ...
    say(
      `V712 error: ${error.message || "Product Master could not open."}`,
      "error"
    );
  }
})();
