(() => {
"use strict";
window.REDZED_ROLE_PERMISSION_VERSION="776.7.0";
  ...
  try{
    ...
    try{
      const leadershipResult=await state.client
        .from("rr_worker_leadership_board_v776")
        .select("*")
        .order("worker_name",{ascending:true});
    ...
    return `
      <article class="card">
        <div class="row between">
          <div>
            ${(()=>{
              const leadership=leadershipFor(w.worker_id);
              let leadershipTag="";
              if(leadership && leadership.leadership_role!=="NONE"){
                const managed=Array.isArray(leadership.managed_departments)
                  ?leadership.managed_departments
                  :[];

                const managedLabels=managed.map(code=>
                  departmentLabel(code)
                ).filter(Boolean);

                if(leadership.leadership_role==="DEPARTMENT_HEAD"){
                  leadershipTag=managedLabels.length
                    ?`${managedLabels.join(" / ")} DEPT HEAD`
                    :"DEPT HEAD";
                }else if(
                  leadership.leadership_role==="PRODUCTION_MANAGER"
                ){
                  leadershipTag=managedLabels.length
                    ?`PRODUCTION MANAGER · ${managedLabels.join(" / ")}`
                    :"PRODUCTION MANAGER";
                }else{
                  leadershipTag=leadership.leadership_role.replaceAll("_"," ");
                }
              }

              return `
                <h4 class="worker-name-line">
                  <span>${safe(w.worker_name||"Unnamed Worker")}</span>
                  <span class="worker-code-inline">
                    ${safe(w.worker_code||"—")}
                  </span>
                  ${leadershipTag?`
                    <span class="worker-leadership-inline">
                      ${safe(leadershipTag)}
                    </span>
                  `:""}
                </h4>
        ...
        <div class="worker-permanent-id">
          <span class="worker-id-label">PERMANENT WORKER ID</span>
          <span class="worker-id-value">${safe(w.worker_id||"—")}</span>
        </div>
        ${w.mobile?`
          <div class="worker-mobile-line">
            Mobile: ${safe(w.mobile)}
          </div>
        `:""}
  ...
  $("workerSearch").oninput=e=>{
    ...
    const filtered=!q?rows:rows.filter(w=>
      JSON.stringify([
        ...
        identityFor(w.worker_id),leadershipFor(w.worker_id)
      ]).toLowerCase().includes(q)
    );
...
function normalizeWorkerNameV7767(value){
  return lower(String(value||"").trim().replace(/\s+/g," "));
}

function duplicateWorkerInDepartmentV7767(
  workerName,
  departmentCode,
  editingWorkerId=null
){
  const normalizedName=normalizeWorkerNameV7767(workerName);
  const normalizedDepartment=lower(departmentCode);

  if(!normalizedName||!normalizedDepartment)return null;

  return (state.data?.workers||[]).find(w=>
    String(w.worker_id)!==String(editingWorkerId||"")
    && normalizeWorkerNameV7767(w.worker_name)===normalizedName
    && lower(w.department_code)===normalizedDepartment
  )||null;
}

function suggestWorkerNameV7767(workerName,departmentCode,editingWorkerId=null){
  const base=String(workerName||"").trim().replace(/\s+/g," ");
  if(!base)return "";
  ...
  while(
    duplicateWorkerInDepartmentV7767(
      suggestion,
      departmentCode,
      editingWorkerId
    )
...
function validateWorkerDuplicateV7767(){
  const name=$("wName")?.value||"";
  const department=$("wDepartment")?.value||"";
  const editingId=state.editingWorker?.worker_id||null;

  const duplicate=duplicateWorkerInDepartmentV7767(
    name,
    department,
    editingId
  );
  ...
  if(!duplicate){
    if(
      $("workerMessage")
      && $("workerMessage").dataset.duplicateWarning==="1"
    ){
      say("","info","workerMessage");
      delete $("workerMessage").dataset.duplicateWarning;
    }
    return null;
  }

  const suggestion=suggestWorkerNameV7767(
    name,
    department,
    editingId
  );
  ...
  $("workerMessage").dataset.duplicateWarning="1";
  say(
    `Same Department me "${duplicate.worker_name}" pehle se maujood hai. `+
    `Naya Worker alag pehchan ke liye suffix use karein, jaise "${suggestion}".`,
    "error",
    "workerMessage"
  );
...
function enforceLeadershipDepartmentSelectionV7764(){
  const role=$("leadershipRole").value;
  const checks=[...document.querySelectorAll(".leadership-managed-dept")];

  if(role==="NONE"){
    checks.forEach(x=>{x.checked=false;x.disabled=true});
    return;
  }
    ...
    checks.forEach(x=>{
      x.onchange=()=>{
        ...
        checks.forEach(other=>{
          if(other!==x)other.checked=false;
        });
      };
    });
  }else{
    checks.forEach(x=>{x.onchange=null});
  }
}

function updateLeadershipCompensationFieldsV776(){
  const role=$("leadershipRole").value;
  const payroll=$("leadershipPayrollCategory").value;
  const mode=$("leadershipCompensationMode").value;

  $("leadershipManagedBlock").classList.toggle("hidden",role==="NONE");

  if($("leadershipDepartmentRuleNote")){
    $("leadershipDepartmentRuleNote").textContent=
      role==="DEPARTMENT_HEAD"
        ?"Department Head को exactly one department assign होगा."
        :role==="PRODUCTION_MANAGER"
          ?"Production Manager को one या multiple departments assign किए जा सकते हैं."
          :"Leadership NONE होने पर Managed Departments blank रहेंगे.";
  }

  const needsFlat=["PCS_PLUS_FLAT","PCS_PLUS_FLAT_RATE","HYBRID"].includes(mode);
  const needsRate=["PCS_PLUS_RATE","PCS_PLUS_FLAT_RATE","HYBRID"].includes(mode);
  ...
  if(payroll==="SALARIED" && !["SALARY_ONLY","HYBRID"].includes(mode)){
    $("leadershipCompensationMode").value="SALARY_ONLY";
    updateLeadershipCompensationFieldsV776();
    return;
  }

  enforceLeadershipDepartmentSelectionV7764();
}
...
function openWorkerSkills(workerId){
  ...
  $("leadershipAssignmentSection").classList.toggle("hidden",!ownerMode);
  $("leadershipCompensationSection").classList.toggle("hidden",!ownerMode);
  $("leadershipOwnerOnlyNote").classList.toggle("hidden",ownerMode);
  $("saveWorkerSkillsButton").textContent=ownerMode
    ?"Save Job + Leadership"
    :"Save Job / Skills";
...
async function saveWorkerSkills(event){
  ...
  try{
    ...
    /*
      ADMIN path:
      Only Job/Skills are saved. Leadership RPC is never called, therefore
      OWNER-only backend rejection cannot create a misleading partial error.
    */
    if(!isOwner()){
      ...
      showTab("workers");
      say("Job departments aur additional skills saved. Leadership unchanged.","success");
      return;
    }
    ...
    if(leadershipRole==="NONE" && managedDepartments.length!==0){
      throw new Error("Leadership NONE ho to Managed Departments blank hone chahiye.");
    }

    if(leadershipRole==="DEPARTMENT_HEAD" && managedDepartments.length!==1){
      throw new Error("DEPARTMENT HEAD ko exactly one Managed Department assign karein.");
    }

    if(leadershipRole==="PRODUCTION_MANAGER" && managedDepartments.length<1){
      throw new Error("PRODUCTION MANAGER ko kam se kam one Managed Department assign karein.");
    }

    /*
      OWNER path:
      Validation is completed before either write. Backend V776.3 remains
      authoritative for Leadership.
    */
    await rpc("rr_owner_set_worker_departments_v1",{
      p_worker_id:workerId,
      p_rows:rows
    ...
    await rpc("rr_set_worker_leadership_v776",{
      p_worker_id:workerId,
      ...
      p_reason:"Role & Permission V776.4 Leadership configuration"
    });
    ...
    showTab("workers");
    say(
      "Job departments, additional skills aur Leadership Compensation saved.",
      "success"
    );
  }catch(e){
...
function bind(){document.querySelectorAll("[data-close]").forEach(x=>x.onclick=()=>closeSheet(x.dataset.close));$("tabs").querySelectorAll("button").forEach(b=>b.onclick=()=>showTab(b.dataset.tab));$("impactNo").onclick=()=>setImpactMode("NON_IMPACT");$("impactYes").onclick=()=>setImpactMode("IMPACT");$("addRoute").onclick=()=>{state.routes=collectRoutes();state.routes.push({target_module_code:"printing",relation_mode:"VIEW_REFERENCE"});renderRoutes()};$("fieldForm").onsubmit=saveFieldDraft;$("runDemo").onclick=runDemo;$("activateField").onclick=activateField;$("suspendField").onclick=suspendField;$("revokeField").onclick=revokeField;$("userForm").onsubmit=createUser;$("workerForm").onsubmit=createManualWorker;$("departmentForm").onsubmit=saveDepartment;$("workerSkillsForm").onsubmit=saveWorkerSkills;
  ...
  $("leadershipPayrollCategory").onchange=updateLeadershipCompensationFieldsV776;
  $("leadershipCompensationMode").onchange=updateLeadershipCompensationFieldsV776;$("workerIdentityForm").onsubmit=saveWorkerIdentityV773;$("departmentWitnessForm").onsubmit=saveDepartmentPrimaryWitnessV774;$("rerollDepartmentSecondary").onclick=rerollDepartmentSecondaryV774;$("identityDeviceMode").onchange=updateIdentityModeFieldsV773;$("unlinkWorkerLogin").onclick=unlinkWorkerLoginV773;document.addEventListener("keydown",e=>{if(e.key==="Escape"){const sheet=document.querySelector(".sheet:not(.hidden)");if(sheet)closeSheet(sheet.id)}})}
