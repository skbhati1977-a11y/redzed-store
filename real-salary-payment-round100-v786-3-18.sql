-- REAL FACTORY SALARY + ADVANCE + ₹100 READY PAY V786.3.18
...
alter table public.rr_salary_payment_batches_v785
  ...
  add column if not exists total_round_100_ready_payment numeric(16,2) not null default 0,
  add column if not exists total_round_100_carry_forward numeric(16,2) not null default 0;
...
alter table public.rr_salary_payment_batch_lines_v785
  ...
  add column if not exists round_100_ready_payment numeric(16,2) not null default 0,
  add column if not exists round_100_carry_forward numeric(16,2) not null default 0,
...
begin
  ...
  create temporary table if not exists rr_salary_preview_tmp_v786(
    ...
    round_100_ready_payment numeric(16,2),round_100_carry_forward numeric(16,2),
  ...
  set round_100_ready_payment=floor(scope_payable/v_rounding)*v_rounding,
      round_100_carry_forward=scope_payable-(floor(scope_payable/v_rounding)*v_rounding)
  ...
  return jsonb_build_object(
    ...
    'total_round_100_ready_payment',(select round(coalesce(sum(round_100_ready_payment) filter(where payment_selected),0),2) from rr_salary_preview_tmp_v786),
    'total_round_100_carry_forward',(select round(coalesce(sum(round_100_carry_forward) filter(where payment_selected),0),2) from rr_salary_preview_tmp_v786),
...
begin
  ...
  insert into public.rr_salary_payment_batches_v785(
    ...
    total_round_100_ready_payment,total_round_100_carry_forward,
    ...
    (v_preview->>'total_round_100_ready_payment')::numeric,(v_preview->>'total_round_100_carry_forward')::numeric,
  ...
  for r in select * from jsonb_to_recordset(v_preview->'lines') as x(
    ...
    final_total_payable numeric,scope_payable numeric,round_100_ready_payment numeric,
    round_100_carry_forward numeric,raw_allocation numeric,floor_allocation numeric,
    ...
    insert into public.rr_salary_payment_batch_lines_v785(
      ...
      round_100_ready_payment,round_100_carry_forward,
      ...
      r.round_100_ready_payment,r.round_100_carry_forward,
    ...
    v_msg:=concat(case when v_mode='TEST' then '[TEST] ' else '' end,
      ...
      E'\n₹100 Ready Pay: ₹',to_char(r.round_100_ready_payment,'FM9999999990.00'),
      E'\nCarry Forward: ₹',to_char(r.round_100_carry_forward,'FM9999999990.00'),
  ...
  return jsonb_build_object('ok',true,'batch_id',v_batch,'voucher_no',v_voucher,
    ...
    'total_round_100_ready_payment',(v_preview->>'total_round_100_ready_payment')::numeric,
    'total_round_100_carry_forward',(v_preview->>'total_round_100_carry_forward')::numeric,
...
select
  case when
    ...
    and to_regprocedure('public.rr_salary_voucher_preview_v786(text)') is not null
    ...
    and exists(select 1 from information_schema.columns where table_schema='public'
      and table_name='rr_salary_payment_batch_lines_v785' and column_name='round_100_ready_payment')
    and exists(select 1 from information_schema.columns where table_schema='public'
      and table_name='rr_salary_payment_batches_v785' and column_name='total_round_100_carry_forward')
  then 'PASS' else 'FAIL' end as salary_payment_round_100_v786_3_18_result;
