(()=>{
  ...
  function mount(){
    ...
    const pn=$('piNo');if(pn)new MutationObserver(update).observe(pn,{childList:true,subtree:true,characterData:true});
    update();
  }
  ...
  async function run(){
    if(!isSaved())return;
    const btn=$('rrPiPdfWa');let popup=null;
    btn.disabled=true;
    try{
      popup=window.open('about:blank','_blank');if(popup)popup.opener=null;
    ...
    }catch(e){try{if(popup&&!popup.closed)popup.close();}catch(_){}const m=$('msg');if(m)m.textContent=e?.message||'WhatsApp open failed.';}
