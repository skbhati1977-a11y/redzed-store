...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(action==="create_user"){
   ...
   if(["owner","super_admin","superadmin","admin"].includes(role)&&!["owner","super_admin","superadmin"].includes(actorRole))return json({error:"Only Owner/Super Admin can create Admin."},403);
  ...
  if(action==="set_role"){
   if(!["owner","super_admin","superadmin"].includes(actorRole))return json({error:"Only Owner/Super Admin can change primary roles."},403);
  ...
  if(action==="reset_password"){
   ...
   if(["owner","super_admin","superadmin","admin"].includes(clean(target.data?.role_code))&&!["owner","super_admin","superadmin"].includes(actorRole))return json({error:"Only Owner/Super Admin can reset Admin credentials."},403);
