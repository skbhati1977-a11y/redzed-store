-- TEST70 only: keep the existing LOT_RELEASE message key, but take the logical
-- CB-to-Lot actor from the actual Cutting source instead of the login executor.
begin;
...
begin
  ...
  new.personal_payload:=new.personal_payload||jsonb_strip_nulls(jsonb_build_object(
    ...
    'cb_no',v_cb_no,'qty',v_qty,'message','CB से Lot बनाया'));
...
revoke all on function public.rr_real_chat_truthful_cutting_lot_v75() from public,anon,authenticated;

drop trigger if exists rr_real_chat_truthful_cutting_lot_v75 on public.rr_real_chat_message_bridge_v70;
create trigger rr_real_chat_truthful_cutting_lot_v75
before insert or update on public.rr_real_chat_message_bridge_v70
for each row execute function public.rr_real_chat_truthful_cutting_lot_v75();

-- Legacy reconciliation. Trigger order is V73 names -> V75 cutting actor -> V74 participant IDs.
...
'TEST70 legacy/present/future CB-to-Lot messages use rr_cutting_lots_v3.operator_name as the logical sender.';
commit;
