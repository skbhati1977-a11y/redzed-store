-- TEST70: privileged staff may see the all-department work inbox by default.
-- Department-scoped staff and workers remain restricted; an explicit department
-- filter always narrows the result.
...
declare
  ...
  v_find text:=lower(trim(coalesce(p_search,''))); v_staff boolean; v_global boolean; v_cards jsonb;
begin
  ...
  v_staff:=v_global or v_role in ('MANAGER','LINE_MANAGER','LINE_MAN','DEPARTMENT_HEAD','PRODUCTION','CUTTING_MASTER','ACCOUNTANT','ACCOUNTS','ACCOUNT');
  v_dept:=public.rr_upm_core_department_v9077(
    case when nullif(trim(p_department_code),'') is not null then p_department_code
         when v_global then null else v_profile.department_code end
  );
