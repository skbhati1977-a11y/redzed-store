...
begin
 ...
 loop
  ...
  loop
   if not exists(select 1 from jsonb_array_elements(v_cards)c where upper(coalesce(c->>'department_code',''))=v_dept and upper(coalesce(c->>'lot_no',''))=upper(v_lot->>'lot_no')) then
