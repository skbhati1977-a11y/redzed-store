-- TEST67: distributor may revise the customer-facing PI until customer CI creation.
...
begin
  ...
  for v_input in select value from jsonb_array_elements(p_lines) loop
    begin v_qty:=(v_input->>'qty')::integer;
    exception when others then raise exception 'PI quantity must be a whole number.';
    end;
  ...
  update public.rr_market_partner_order_v67 set
    distributor_pi_ref=v_ref,distributor_pi_created_at=coalesce(distributor_pi_created_at,now()),
    distributor_pi_visible=true,distributor_pi_pushed_at=now(),distributor_pi_status='WAITING',
    distributor_pi_note=nullif(trim(p_note),''),distributor_pi_responded_at=null,
  ...
  values(v_owner,v_order.id,'DISTRIBUTOR_PI_SENT_TO_CUSTOMER',nullif(trim(p_note),''),'DISTRIBUTOR',
    jsonb_build_object('distributor_pi_ref',v_ref,'requirement_display_no',v_order.requirement_display_no,
      'requirement_closed',true,'pi_revision',v_order.distributor_pi_ref is not null,
      'next_action','DISTRIBUTOR_SEND_TO_REDZED','customer_ref',
      (select customer_ref from public.rr_market_partner_customer_v67 where id=v_order.partner_customer_id)));
...
create or replace function public.rr_market_partner_customer_pi_charges_v67(
 ...
 p_value_pct numeric default 0,p_freight numeric default 0,p_other numeric default 0,p_tax_pct numeric default 0
) returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 update public.rr_market_partner_order_v67 set distributor_pi_value_pct=coalesce(p_value_pct,0),
