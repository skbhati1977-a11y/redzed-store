-- TEST70 complete worker/department membership matrix. MAIN workflow stays untouched.
create table if not exists public.rr_real_chat_department_membership_v70 (
  department_code text not null,
  worker_id uuid not null,
  membership_side text not null check (membership_side in ('WORKER','STAFF')),
  source_rule text not null,
  is_active boolean not null default true,
  updated_at timestamptz not null default now(),
  updated_by uuid,
  primary key(department_code,worker_id,membership_side)
);
alter table public.rr_real_chat_department_membership_v70 enable row level security;
revoke all on public.rr_real_chat_department_membership_v70 from anon,authenticated;
grant select on public.rr_real_chat_department_membership_v70 to authenticated;

insert into public.rr_real_chat_department_membership_v70(department_code,worker_id,membership_side,source_rule)
select public.rr_upm_core_department_v9077(d.department_code),d.worker_id,'WORKER','HOME_DEPARTMENT'
from public.rr_worker_directory_unified_v1 d where coalesce(d.is_active,false)
on conflict(department_code,worker_id,membership_side) do update set is_active=true,source_rule=excluded.source_rule,updated_at=now();
...
with departments(code) as (values
 ...
 select d.worker_id,public.rr_upm_core_department_v9077(d.department_code) home_department,
   lower(trim(d.worker_name)) worker_name,upper(coalesce(d.role_code,'')) role_code
 from public.rr_worker_directory_unified_v1 d where coalesce(d.is_active,false) and (
   upper(coalesce(d.role_code,'')) in ('OWNER','SUPER_ADMIN','ADMIN','MANAGER','ACCOUNT','ACCOUNTS','ACCOUNTANT','LINE_MANAGER','LINE_MAN','DEPARTMENT_HEAD')
...
from departments x cross join universal_staff s
where not (s.worker_name in ('badsha','sanju','kartik') and x.code=s.home_department)
on conflict(department_code,worker_id,membership_side) do update set is_active=true,source_rule=excluded.source_rule,updated_at=now();

create policy rr_rc_membership_select_v70 on public.rr_real_chat_department_membership_v70
...
begin
 ...
 with canonical(code,name,sort_order) as (values
  ...
  where m.is_active and coalesce(d.is_active,false)
 )
 ...
 return jsonb_build_object('version','TEST70_REAL_CHAT_DIRECTORY_V71','actor',jsonb_build_object('name',v_profile.full_name,'role',v_role,'worker_id',v_worker,'is_global',v_global),
...
comment on table public.rr_real_chat_department_membership_v70 is 'TEST70 explicit department membership: home workers plus universal Redzed staff overlay.';
