(() => {
  ...
  function values(x) { const qty = Number(x.distributor_pi_qty ?? x.proposed_qty ?? x.requested_qty ?? x.quantity ?? x.qty ?? 0), rate = Number(x.final_rate ?? x.rate ?? x.customer_rate ?? x.base_rate ?? 0), discount = Number(x.discount ?? x.discount_amount ?? 0), net = Math.max(0, rate - discount); return { qty, rate, discount, net, amount: qty * net }; }
  function canEditQty() { return (role === "DISTRIBUTOR" || role === "REDZED") && !doc?.ref; }

  function render() {
    let gross = 0, discount = 0, total = 0; const staff = role === "REDZED";
    $("godownHead").classList.toggle("internal-only", !staff);
    $("rows").innerHTML = lines.map((x, i) => { const v = values(x); gross += v.qty * v.rate; discount += v.qty * v.discount; total += v.amount; return `<tr><td>${i + 1}</td><td>${x.image_url ? `<img class="pic" src="${esc(x.image_url)}" alt="">` : "👕"}</td><td><b>${esc(x.lot_no || x.article_name || "-")}</b></td><td>${esc(x.category || x.article_name || "-")}</td><td>${esc(x.size_text || x.size || "-")}</td><td>${canEditQty() ? `<input class="num" data-qty="${esc(x.id)}" type="number" min="0" value="${v.qty}">` : v.qty}</td><td>${money(v.rate)}</td><td>${money(v.discount)}</td><td>${money(v.net)}</td><td>${money(v.amount)}</td>${staff ? `<td class="rr-godown-view-only">${esc(x.godown || "—")}</td>` : ""}</tr>`; }).join("");
    $("gross").textContent = money(gross); $("discount").textContent = money(discount); $("total").textContent = money(total + Number(doc?.freight || 0) + Number(doc?.other || 0)); $("freight").textContent = money(doc?.freight || 0); $("other").textContent = money(doc?.other || 0);
    ...
    $("savePi").hidden = !["DISTRIBUTOR", "REDZED"].includes(role) || !!doc?.ref; $("convertCi").hidden = !["DISTRIBUTOR", "REDZED"].includes(role) || !doc?.ref || doc?.kind === "CI"; $("share").hidden = !doc?.ref;
  ...
  async function loadDistributor() {
    const a = auth(), [w, p] = await Promise.all([rpc("rr_market_partner_workspace_v67", a), rpc("rr_market_partner_customer_pi_state_v67", a)]);
    ...
    lines = order.lines || []; doc = order.distributor_pi_ref ? { ref: order.distributor_pi_ref, status: order.distributor_pi_status || "WAITING", pushed_at: order.distributor_pi_pushed_at, kind: order.customer_ci_visible && order.ci_ref ? "CI" : "PI" } : { kind: "PI" }; if (doc.kind === "CI") doc.ref = order.ci_ref;
  }
  ...
  async function boot() { try { if (role === "DISTRIBUTOR") await loadDistributor(); else if (role === "REDZED") await loadRedzed(); else await loadCustomer(); render(); $("savePi").onclick = savePi; $("convertCi").onclick = convertCi; $("sendResponse").onclick = respond; $("share").onclick = share; } catch (e) { $("msg").textContent = e.message; } }
  boot();
})();
