(() => {
  ...
  function render() {
    ...
    const saved=chargeValues(), editable=role==="DISTRIBUTOR"&&!doc?.ref; [["valuePct",saved.valuePct],["freightInput",saved.freight],["otherInput",saved.other],["taxPct",saved.taxPct]].forEach(([id,v])=>{if(document.activeElement!==$(id))$(id).value=v;$(id).readOnly=!editable;});
