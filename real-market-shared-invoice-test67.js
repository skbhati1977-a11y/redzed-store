(() => {
  ...
  async function loadRedzed() {
    ...
    order = { id: detail.id, batch_ref: detail.batch_ref, batch_kind: detail.batch_kind, customer_name: detail.direct_customer_name, requirement_display_no: detail.requirement_display_no || detail.batch_ref, collection_display_no: `${Number((detail.orders || []).length)} source requirement(s)` };
  ...
  async function savePi() {
    try {
      if (role === "REDZED" && doc?.ref) {
        message(`PI ${doc.ref} पहले ही distributor को भेजी जा चुकी है ✓`, "ok");
        render();
        return;
      }
  ...
  async function cancelCi() {
    const reason=prompt("CI cancellation reason"); if(reason===null)return;
    if(!String(reason).trim())throw Error("Cancellation reason is required.");
