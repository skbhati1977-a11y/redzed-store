(() => {
  ...
  async function savePi() {
    try {
      ...
      else { const a = auth(), c=readCharges(), payload = lines.map((x) => ({ line_id: x.id, qty: Math.max(0, Math.floor(Number(document.querySelector(`[data-qty="${CSS.escape(x.id)}"]`)?.value ?? values(x).qty))) })); await rpc("rr_market_partner_customer_pi_charges_v67",{...a,p_order_id:order.id,p_value_pct:c.valuePct,p_freight:c.freight,p_other:c.other,p_tax_pct:0}); const r = await rpc("rr_market_partner_make_customer_pi_v67", { ...a, p_order_id: order.id, p_lines: payload, p_note: null }); doc = { kind: "PI", ref: r.distributor_pi_ref, status: r.distributor_pi_status || "WAITING", pushed_at: new Date().toISOString(),charges:{value_pct:c.valuePct,freight:c.freight,other:c.other,tax_pct:0} }; lines.forEach((x) => { x.distributor_pi_qty = payload.find((y) => y.line_id === x.id)?.qty; }); try { await rpc("rr_market_partner_chat_send_v67", { ...a, p_lane: "CUSTOMER_GROUP", p_partner_customer_id: order.customer_id, p_message: `[DPI:${order.id}] ${doc.ref} · ${order.requirement_display_no || order.order_ref || "REQUIREMENT"} · SENT TO CUSTOMER`, p_attachment: null }); } catch (e) { console.warn("PI saved; chat notice failed", e); } }
  ...
  async function boot() { try { if (role === "DISTRIBUTOR") await loadDistributor(); else if (role === "REDZED") await loadRedzed(); else await loadCustomer(); render(); ["valuePct","freightInput","otherInput"].forEach((id)=>$(id).addEventListener("input",render)); $("savePi").onclick = savePi; $("convertCi").onclick = convertCi; $("sendResponse").onclick = respond; $("share").onclick = share; } catch (e) { $("msg").textContent = e.message; } }
  boot();
})();
