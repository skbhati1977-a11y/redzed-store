(() => {
  ...
  function chargeValues() { const c = doc?.charges || {}; return { valuePct:Number(c.value_pct || 0), freight:Number(c.freight || 0), other:Number(c.other || 0), taxPct:Number(c.tax_pct || 0) }; }
  function readCharges() { return { valuePct:Number($("valuePct").value || 0), freight:Math.max(0,Number($("freightInput").value || 0)), other:Math.max(0,Number($("otherInput").value || 0)), taxPct:Math.max(0,Number($("taxPct").value || 0)) }; }
  ...
  function render() {
    ...
    $("rows").innerHTML = lines.map((x, i) => { const v = values(x); gross += v.qty * v.grossRate; discount += v.qty * v.discount; total += v.amount; return `<tr><td>${i + 1}</td><td>${x.image_url ? `<img class="pic" src="${esc(x.image_url)}" alt="">` : "👕"}</td><td><b>${esc(x.lot_no || x.article_name || "-")}</b></td><td>${esc(x.category || x.article_name || "-")}</td><td>${esc(x.size_text || x.size || "-")}</td><td>${canEditQty() ? `<input class="num" data-qty="${esc(x.id)}" type="number" min="0" value="${v.qty}">` : v.qty}</td><td>${money(role === "DISTRIBUTOR" ? v.grossRate : v.finalRate)}${role === "DISTRIBUTOR" ? `<small class="pricing-detail pricing-private">Purchase ${money(v.purchase)} + Margin ${money(v.margin)} · Effective margin ${money(v.effectiveMargin)}</small>` : ""}</td>${role === "DISTRIBUTOR" ? `<td class="pricing-private">${money(v.discount)}</td>` : ""}<td>${money(v.finalRate)}</td><td>${money(v.amount)}</td>${staff ? `<td class="rr-godown-view-only">${esc(x.godown || "—")}</td>` : ""}</tr>`; }).join("");
    const saved=chargeValues(), editable=role==="DISTRIBUTOR"&&!doc?.ref; if(!chargesReady){[["valuePct",saved.valuePct],["freightInput",saved.freight],["otherInput",saved.other],["taxPct",saved.taxPct]].forEach(([id,v])=>{$(id).value=v;});chargesReady=true;} ["valuePct","freightInput","otherInput","taxPct"].forEach((id)=>{$(id).readOnly=!editable;});
    const c=readCharges(), valueAmount=total*c.valuePct/100, taxBase=total+valueAmount+c.freight+c.other, tax=Math.max(0,taxBase)*c.taxPct/100, beforeRound=taxBase+tax, rounded=Math.round(beforeRound), round=rounded-beforeRound;
    $("gross").textContent=money(gross); $("discount").textContent=money(discount); $("subtotal").textContent=money(total); $("valueAmount").textContent=money(valueAmount); $("freight").textContent=money(c.freight); $("other").textContent=money(c.other); $("tax").textContent=money(tax); $("round").textContent=money(round); $("total").textContent=money(rounded);
  ...
  async function savePi() {
    try {
      ...
      else { const a = auth(), c=readCharges(), payload = lines.map((x) => ({ line_id: x.id, qty: Math.max(0, Math.floor(Number(document.querySelector(`[data-qty="${CSS.escape(x.id)}"]`)?.value ?? values(x).qty))) })); await rpc("rr_market_partner_customer_pi_charges_v67",{...a,p_order_id:order.id,p_value_pct:c.valuePct,p_freight:c.freight,p_other:c.other,p_tax_pct:c.taxPct}); const r = await rpc("rr_market_partner_make_customer_pi_v67", { ...a, p_order_id: order.id, p_lines: payload, p_note: null }); doc = { kind: "PI", ref: r.distributor_pi_ref, status: r.distributor_pi_status || "WAITING", pushed_at: new Date().toISOString(),charges:{value_pct:c.valuePct,freight:c.freight,other:c.other,tax_pct:c.taxPct} }; lines.forEach((x) => { x.distributor_pi_qty = payload.find((y) => y.line_id === x.id)?.qty; }); try { await rpc("rr_market_partner_chat_send_v67", { ...a, p_lane: "CUSTOMER_GROUP", p_partner_customer_id: order.customer_id, p_message: `[DPI:${order.id}] ${doc.ref} · ${order.requirement_display_no || order.order_ref || "REQUIREMENT"} · SENT TO CUSTOMER`, p_attachment: null }); } catch (e) { console.warn("PI saved; chat notice failed", e); } }
  ...
  async function boot() { try { if (role === "DISTRIBUTOR") await loadDistributor(); else if (role === "REDZED") await loadRedzed(); else await loadCustomer(); render(); ["valuePct","freightInput","otherInput","taxPct"].forEach((id)=>$(id).addEventListener("input",render)); $("savePi").onclick = savePi; $("convertCi").onclick = convertCi; $("sendResponse").onclick = respond; $("share").onclick = share; } catch (e) { $("msg").textContent = e.message; } }
  boot();
})();
