(() => {
  ...
  function render() {
    ...
    $("reqNo").textContent = order?.requirement_display_no || doc?.requirement_display_no || order?.batch_ref || "Requirement —"; $("docNo").textContent = (doc?.kind === "CI" ? "CI" : "PI") + " No. " + (doc?.ref || "DRAFT"); $("docTitle").textContent = doc?.kind === "CI" ? (order?.batch_kind === "CONSOLIDATED" ? "CONSOLIDATED CI" : "CI · COMMERCIAL INVOICE") : (order?.batch_kind === "CONSOLIDATED" ? "CONSOLIDATED PI" : "PI · PROFORMA INVOICE");
    $("status").value = statusLabel(doc?.status); $("docDate").textContent = new Date(doc?.pushed_at || doc?.created_at || Date.now()).toLocaleString("en-IN");
    const operator = ["DISTRIBUTOR", "REDZED"].includes(role), redzedPiSent = role === "REDZED" && doc?.kind !== "CI" && !!doc?.ref;
    $("saveDraft").hidden = role!=="REDZED" || doc?.kind === "CI" || !!doc?.ref; $("savePi").hidden = !operator || doc?.kind === "CI"; $("savePi").dataset.locked=redzedPiSent?"1":"0"; $("savePi").dataset.lockedLabel=redzedPiSent?`${refLabel("PI",doc.ref)} · SENT ✓`:""; $("savePi").disabled=redzedPiSent; $("savePi").textContent=redzedPiSent?$("savePi").dataset.lockedLabel:role==="REDZED"?"MAKE & SEND PI TO DISTRIBUTOR":(doc?.ref?"UPDATE LIVE PI":"SAVE & LINK PI"); $("sendChat").hidden = role!=="DISTRIBUTOR" || !doc?.ref; renderPiChatState(); $("convertCi").hidden = !operator || !doc?.ref || doc?.kind === "CI"; $("convertCi").textContent=role==="REDZED"?(order?.batch_kind==="CONSOLIDATED"?"CONVERT TO CONSOLIDATED CI":"CONVERT PI TO CI"):"CONVERT TO CUSTOMER CI · CONFIRMATION OPTIONAL"; $("cancelCi").hidden = !operator || doc?.kind !== "CI"; $("confirmationRule").hidden = !operator || doc?.kind === "CI"; $("download").hidden = !operator || !doc?.ref; $("share").hidden = !operator || !doc?.ref;
  ...
  async function loadRedzed() {
    ...
    lines = (detail.orders || []).flatMap((o) => (o.lines || []).map((x) => ({ ...x, order_ref: o.order_ref, requirement_display_no: o.requirement_display_no, customer_ref: o.customer_ref })));
  ...
  async function saveDraft() {
    ...
    const payload=proposals(),r=await rpc("rr_market_staff_save_batch_draft_v67",{p_batch_id:order.id,p_line_proposals:payload});
    lines.forEach((x)=>{x.proposed_qty=payload.find((y)=>y.line_id===x.id)?.proposed_qty;});
    doc.status=r.status;message(`${r.requirement_display_no || "Requirement"} draft saved ✓`,"ok");render();
  }

  async function savePi() {
    try {
      if (role === "REDZED" && doc?.ref) {
        message(`${refLabel("PI", doc.ref)} पहले ही distributor को भेजी जा चुकी है ✓`, "ok");
        render();
        return;
      }
      if (role === "REDZED") { const payload = proposals(), r = await rpc("rr_market_staff_propose_batch_v67", { p_batch_id: order.id, p_line_proposals: payload, p_pi_ref: "" }); doc = { kind: "PI", ref: r.pi_ref, status: r.status, pushed_at: new Date().toISOString() }; lines.forEach((x) => { x.proposed_qty = payload.find((y) => y.line_id === x.id)?.proposed_qty; }); if (r.already_sent) { message(`${refLabel("PI", r.pi_ref)} पहले ही distributor को भेजी जा चुकी है ✓`, "ok"); render(); return; } await staffNotice(r.id); }
