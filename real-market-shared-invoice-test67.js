(() => {
  ...
  function render() {
    ...
    $("saveDraft").hidden = role!=="REDZED" || doc?.kind === "CI" || !!doc?.ref; $("savePi").hidden = !operator || doc?.kind === "CI"; $("savePi").textContent=role==="REDZED"?(doc?.ref?"UPDATE & RESEND PI":"MAKE & SEND PI TO DISTRIBUTOR"):(doc?.ref?"UPDATE LIVE PI":"SAVE & LINK PI"); $("sendChat").hidden = role!=="DISTRIBUTOR" || !doc?.ref; renderPiChatState(); $("convertCi").hidden = !operator || !doc?.ref || doc?.kind === "CI"; $("convertCi").textContent=role==="REDZED"?(order?.batch_kind==="CONSOLIDATED"?"CONVERT TO CONSOLIDATED CI":"CONVERT PI TO CI"):"CONVERT TO CUSTOMER CI · CONFIRMATION OPTIONAL"; $("cancelCi").hidden = !operator || doc?.kind !== "CI"; $("confirmationRule").hidden = !operator || doc?.kind === "CI"; $("download").hidden = !operator || !doc?.ref; $("share").hidden = !operator || !doc?.ref;
