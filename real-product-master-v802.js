...
window.REAL_FACTORY_PRODUCT_MASTER_VERSION = "V802-BACKEND-DUE-TABS";
...
async function loadData(){
  ...
  try{
    const [categories,gallery,purchases,rolls,colours,allocations,arts,prints,assignments,printAssignments,media,lots,_mcLoaded,grEntries,exchangeEntries,damageClaims,damageMedia,lotCostAdjustments,pmDecisionRows,pmDecisionCounts]=await Promise.all([
      requiredTable(TABLES.categories),loadGalleryRows(),requiredTable(TABLES.purchases,"*","created_at"),optionalTable(TABLES.rolls),requiredTable(TABLES.colours,"*","colour_order"),optionalTable(TABLES.allocations),requiredTable(TABLES.arts,"*","updated_at"),loadPrints(),requiredTable(TABLES.assignments,"*","updated_at"),requiredTable(TABLES.printAssignments,"*","sequence_no"),optionalTable(TABLES.media),optionalTable(TABLES.lots,"*","created_at"),loadMc(),optionalTable(TABLES.gr,"*","created_at"),optionalTable(TABLES.exchanges,"*","created_at"),optionalTable(TABLES.damage,"*","created_at"),optionalTable(TABLES.damageMedia,"*","created_at"),optionalTable(TABLES.lotAdjustments,"*","created_at"),loadPmDecisionRows(),loadPmDecisionCounts()
    ]);
    Object.assign(state,{categories,galleryRows:gallery,purchases,rolls,colours,allocations,arts,prints,assignments,printAssignments,media,lots,grEntries,exchangeEntries,damageClaims,damageMedia,lotCostAdjustments,pmDecisionRows,pmDecisionCounts});
