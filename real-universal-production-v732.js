...
async function loadContext() {
  ...
  try {
    ...
    state.context = await rpc("rr_upm_universal_form_v731", {
...
function renderSummary() {
  ...
  $("summary").innerHTML = [
    ...
    ["Alter Pending", total.alter],
    ["Line Man Pending", total.line_man_pending],
    ["Worker Remake Pending", total.remake],
    ["Damage", total.damage],
...
async function runDebug() {
  ...
  try {
    ...
    const output = await rpc("rr_upm_debug_lot_flow_v731", {
      p_canonical_lot_id: state.lot.canonical_lot_id,
      p_department_code: $("dept").value
    });
