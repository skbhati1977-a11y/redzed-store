...
const VERSION = "V760_3_SUBMIT_RESUME_CANCEL_FIX_UI";
...
async function resumePendingSubmitV760(pending) {
  ...
  // Close any stale inline panel and refresh the latest matrix/rows.
  closeInlineAction(pending.rowElement);
  checkinSignature = "";

  await new Promise(resolve => setTimeout(resolve, 250));
  await syncAll();
  await ensureDepartmentContext(pending.rowData.department_code);

  const card = selectColourCard(pending.rowData.colour_code);
  if (!card) {
    throw new Error(
      `${pending.rowData.colour_code} ka hidden Submit engine nahi mila.`
    );
  }

  // Give the legacy selection/change handlers time to bind the card.
  await new Promise(resolve => setTimeout(resolve, 300));

  // Re-enter through the visible Colour-row Submit action so all existing
  // confirmation/next-department logic remains exactly the same.
  const visibleSubmit = [...document.querySelectorAll(
    '.v756-action[data-v756-action="SUBMIT"]'
  )].find(button =>
    upper(button.dataset.v756Colour) === upper(pending.rowData.colour_code)
  );

  v760SubmitBypassOnce = {
    canonical: pending.canonical,
    departmentCode: pending.departmentCode,
    colourCode: upper(pending.rowData.colour_code)
  };
  ...
  // Fallback only when the visible row has not re-rendered.
...
function showV756Badge() {
  ...
  if (!badge) {
    ...
    badge.textContent = "V760.3 ACTIVE";
    badge.style.cssText = `
