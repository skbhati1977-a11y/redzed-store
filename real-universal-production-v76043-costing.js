...
const VERSION = "V760_4_3_RANDOM_ASSIGN_DEPARTMENT_FIX_UI";
...
async function fetchAssignableDepartmentsV76043() {
  ...
  const { data, error } = await client
    ...
    .select(
      ...
      "colour_assignment_enabled,worker_assignment_enabled,sequence_no"
    )
    .eq("is_active", true)
  ...
  fetchAssignableDepartmentsV76043()
    .then(departments => {
      window.v757BulkDepartmentSearch = createSearchableDropdown({
        ...
        emptyText: "No eligible assignment department",
        onSelect: item => renderBulkWorkers(item.value)
      });
      renderBulkWorkers("");
    })
    .catch(error => {
      console.error("V760.4.3 department list failed", error);
    });
...
async function openAssignPanel(rowData, rowElement, card) {
  const departments = await fetchAssignableDepartmentsV76043();
  ...
  try {
    ...
    const { data, error } = await client.rpc(
      "rr_upm_claim_colours_v741",
      {
        p_canonical_lot_id: identity.canonical_lot_id,
        p_lot_no: identity.lot_no,
        p_department_code: upper(department),
        p_rows: rows,
        p_remarks:
...
function showV756Badge() {
  ...
  if (!badge) {
    ...
    badge.textContent = "V760.4.3 ACTIVE";
    badge.style.cssText = `
