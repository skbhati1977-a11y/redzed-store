...
test('deep-linked Cutting action renders one exact history card and never reopens release', () => {
  ...
  assert.match(cutting, /const exactDivision = await client[\s\S]*\.from\("rr_cb_units"\)[\s\S]*\.eq\("id", requestedDivisionId\)/);
  assert.match(cutting, /return galleryRowsFromDivisions\(client, exactDivision\.data \|\| \[\]\)/);
  ...
  assert.match(cutting, /\.in\("id", purchaseIds\)/);
