...
if(rrIsDashboardPath(window.location.pathname)){
 const pinLatestDashboardRoutes=()=>{
  document.querySelectorAll('a[href]').forEach(a=>{
   ...
   else if(text==='art decide master')a.href=`${RR_REPO_BASE}real-art-decide-master.html?v=${RR_TRAVEL_VERSION}`;
   ...
   else if(text==='material master')a.href=`${RR_REPO_BASE}real-material-master-v853.html?v=${RR_TRAVEL_VERSION}`;
   ...
   try{const u=new URL(a.getAttribute('href'),window.location.href);if(u.origin===window.location.origin&&u.pathname.startsWith(RR_REPO_BASE)&&!/\/(?:s|real-market-share-v9420)\.html$/i.test(u.pathname)){u.searchParams.set('v',RR_TRAVEL_VERSION);a.href=u.pathname+u.search+u.hash}}catch(_e){}
  });
 };
 document.addEventListener('DOMContentLoaded',pinLatestDashboardRoutes,{once:true});setTimeout(pinLatestDashboardRoutes,0);new MutationObserver(pinLatestDashboardRoutes).observe(document.documentElement,{childList:true,subtree:true});
}
...
if(/\/real-finished-goods-v787\.html$/i.test(window.location.pathname)){const p=document.createElement("script");p.src=`${RR_REPO_BASE}real-packing-manager-behalf-v9309.js?v=9360`;p.async=false;document.head.appendChild(p);if(new URLSearchParams(window.location.search).get('view')==='receive'){const r=document.createElement('script');r.src=`${RR_REPO_BASE}real-store-receive-grouped-v9401.js?v=9402`;r.async=false;document.head.appendChild(r)}}
...
if(rrIsRealChatPath(window.location.pathname)){
 ...
 const push61=document.createElement('script');push61.src=`${RR_REPO_BASE}real-chat-staff-floating-notify-v61.js?v=61push8`;push61.async=false;(document.head||document.documentElement).appendChild(push61);
...
if(/\/(?:s|real-customer-invite-test67)\.html$/i.test(window.location.pathname)){
 const permission67=document.createElement('script');permission67.src='redzed-permission-onboarding-test67.js?v=67';permission67.async=false;(document.head||document.documentElement).appendChild(permission67);
 const crossParty67=document.createElement('script');crossParty67.src=`${RR_REPO_BASE}real-chat-cross-party-notifications-test67.js?v=68`;crossParty67.async=false;(document.head||document.documentElement).appendChild(crossParty67);
}
