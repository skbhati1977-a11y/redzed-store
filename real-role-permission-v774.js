(() => {
"use strict";
window.REDZED_ROLE_PERMISSION_VERSION="774.0.0";
...
function bind(){document.querySelectorAll("[data-close]").forEach(x=>x.onclick=()=>closeSheet(x.dataset.close));$("tabs").querySelectorAll("button").forEach(b=>b.onclick=()=>showTab(b.dataset.tab));$("impactNo").onclick=()=>setImpactMode("NON_IMPACT");$("impactYes").onclick=()=>setImpactMode("IMPACT");$("addRoute").onclick=()=>{state.routes=collectRoutes();state.routes.push({target_module_code:"printing",relation_mode:"VIEW_REFERENCE"});renderRoutes()};$("fieldForm").onsubmit=saveFieldDraft;$("runDemo").onclick=runDemo;$("activateField").onclick=activateField;$("suspendField").onclick=suspendField;$("revokeField").onclick=revokeField;$("userForm").onsubmit=createUser;$("workerForm").onsubmit=createManualWorker;$("departmentForm").onsubmit=saveDepartment;$("workerSkillsForm").onsubmit=saveWorkerSkills;$("workerIdentityForm").onsubmit=saveWorkerIdentityV773;$("departmentWitnessForm").onsubmit=saveDepartmentPrimaryWitnessV774;$("rerollDepartmentSecondary").onclick=rerollDepartmentSecondaryV774;$("identityDeviceMode").onchange=updateIdentityModeFieldsV773;$("unlinkWorkerLogin").onclick=unlinkWorkerLoginV773;document.addEventListener("keydown",e=>{if(e.key==="Escape"){const sheet=document.querySelector(".sheet:not(.hidden)");if(sheet)closeSheet(sheet.id)}})}
