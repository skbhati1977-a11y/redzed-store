(() => {
  ...
  function filterModules() {
    ...
    let visible = 0;
    cards.forEach((card) => { const match = !query || card.textContent.toLowerCase().includes(query); card.hidden = !match; if (match) visible += 1; });
    document.querySelectorAll("[data-module-section]").forEach((section) => { section.hidden = !section.querySelector("[data-module]:not([hidden])"); });
    if (moduleCount) moduleCount.textContent = query ? `${visible} of ${cards.length} modules` : `${cards.length} latest modules`;
  }
  async function requireOwner() {
    const { data, error } = await supabaseClient.auth.getSession();
    if (error || !data.session) { window.location.replace("real-login.html"); throw new Error("Login required."); }
    const user = data.session.user;
    const { data: profile, error: profileError } = await supabaseClient.from("rr_user_profiles").select("full_name, role_code, is_active").eq("auth_user_id", user.id).single();
    if (profileError || !profile?.is_active || !["owner", "admin"].includes(profile.role_code)) { await supabaseClient.auth.signOut(); window.location.replace("real-login.html"); throw new Error("Owner/Admin access required."); }
    const publicRoleLabel = ["owner", "admin"].includes(profile.role_code) ? "SUPER ADMIN" : String(profile.role_code || "USER").replaceAll("_", " ").toUpperCase();
  ...
  async function loadLots() {
    const { data, error } = await supabaseClient.from("rr_live_lot_status").select("*").order("updated_at", { ascending: false }).limit(12); if (error) throw error;
    if (!lotList) return;
    if (!data || data.length === 0) { lotList.innerHTML = `<p class="rr-muted">No production lots yet.</p>`; return; }
    lotList.innerHTML = data.map((lot) => { const itemName = lot.item_name || lot.product_name || ""; const department = lot.current_department || lot.current_department_code || ""; return `<article class="rr-list-row"><div><strong>${safeText(lot.lot_no)}</strong><span>Art ${safeText(lot.art_no)} · ${safeText(itemName)}</span></div><div class="rr-list-meta"><span>${safeText(department)}</span><b>${safeText(lot.status)}</b></div></article>`; }).join("");
  }
  async function refreshDashboard() {
    setMessage(""); if (refreshBtn) { refreshBtn.disabled = true; refreshBtn.textContent = "Loading..."; }
    try { await Promise.all([loadStats(), loadLots()]); } catch (error) { console.error(error); setMessage(error.message || "Dashboard data could not load.", "error"); } finally { if (refreshBtn) { refreshBtn.disabled = false; refreshBtn.textContent = "Refresh"; } }
  }
...
(() => {
  ...
  style.textContent=`.rf-acc{margin:3px 0;border-radius:11px;overflow:hidden}.rf-acc-head{width:100%;display:flex;align-items:center;gap:10px;min-height:48px;padding:9px 10px;border:0;background:transparent;color:#f2f5fa;text-align:left;cursor:pointer;border-radius:10px}.rf-acc-head:active,.rf-acc.open>.rf-acc-head{background:#1b2230}.rf-acc-icon{width:30px;height:30px;display:grid;place-items:center;flex:0 0 30px;border-radius:8px;background:#202735;font-weight:900}.rf-acc-title{flex:1;font-weight:900;font-size:14px}.rf-acc-arrow{font-size:13px;color:#9aa7b9;transition:transform .15s ease}.rf-acc.open .rf-acc-arrow{transform:rotate(90deg)}.rf-acc-body{display:none;padding:3px 4px 7px 30px;margin-left:15px;border-left:1px solid #2d3746}.rf-acc.open .rf-acc-body{display:block}.rf-acc-body .rf-side-item{font-size:13px;min-height:39px;padding:7px 9px}.rf-acc-body .rf-icon{width:24px;height:24px;flex-basis:24px;font-size:11px}.rf-acc.search-hit .rf-acc-body{display:block}.rf-side-divider{height:1px;background:#252d3a;margin:8px 7px}`;
  ...
  side.innerHTML =
    ...
    group('admin','⚙','Admin & Control',A(`real-role-permission-v777-4-final.html?v=${V}`,'RP','Roles & Permissions')+A(`real-data-mode-control-v786.html?v=${V}`,'DM','Data Mode Control')+A(`real-communications-v853.html?v=${V}`,'WA','WhatsApp Messages')+A(`real-flow-audit-v857.html?v=${V}`,'FA','Full Flow Audit'));
  side.addEventListener('click',e=>{const head=e.target.closest('.rf-acc-head');if(!head)return;head.closest('.rf-acc').classList.toggle('open');});
  const applySearch=()=>{const q=String(search?.value||'').trim().toLowerCase();side.querySelectorAll('.rf-acc').forEach(acc=>{let hit=false;acc.querySelectorAll('[data-module]').forEach(a=>{const m=!q||a.textContent.toLowerCase().includes(q)||acc.querySelector('.rf-acc-title').textContent.toLowerCase().includes(q);a.hidden=!m;if(m)hit=true;});acc.style.display=hit?'':'none';acc.classList.toggle('search-hit',!!q&&hit);});};
