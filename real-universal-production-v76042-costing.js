...
const VERSION = "V760_4_2_RANDOM_QUEUE_SUBMIT_FIX_UI";
...
async function fetchCostingPanelV760(canonical) {
  ...
  const [{ data, error }, weighted] = await Promise.all([client.rpc("rr_upm_final_costing_v295", {p_canonical_lot_id: canonical,p_data_mode:'TEST'})]);
...
function showV756Badge() {
  ...
  if (!badge) {
    ...
    badge.textContent = "V760.4.2 ACTIVE";
    badge.style.cssText = `
