...
window.REDZED_PRODUCT_MASTER_VERSION = "719.3-MATCHING-ONLY-HARD-REPAIR";
...
function firstPurchaseTypeHtml() {
  ...
  return `
    <div class="pm-allocation-box pm-first-purchase-type">
      <label><span>Production CB</span></label>
      <small class="pm-muted-copy">
        New CB creates only the Regular/Production identity and D1/D2 children. Purchase remains optional. Matching Cloth never creates a Cutting card; add it later from an existing CB through “Add Matching Purchase”.
      </small>
    </div>
  `;
}
...
function changeFirstPurchaseType(type) {
  ...
  if (type === "matching") {
    say("Matching Cloth cannot create a new Cutting CB. Open an existing Regular CB and use Add Matching Purchase.", "error");
  }
...
function resetCreateForm() {
  ...
  formEntries = [makeEntry({ regularLocked: true, categoryCode: "regular-cloth", entryType: "cb-material" })];
  if ($("addMatchingEntry")) $("addMatchingEntry").style.display = "none";
  renderPurchaseEntries();
}
...
async function saveCreateMode() {
  ...
  if (activeEntries.some(entry => entry.entryType === "matching")) {
    throw new Error("Matching Cloth cannot create its own Cutting CB. Create/open the Regular CB first, then use Add Matching Purchase.");
  }
...
function meaningfulProductionPurchase(row = {}) {
  ...
  // Older rr_create_cb versions could leave a zero-value placeholder
  // in rr_cb_purchase_entries when a Matching-only CB was created.
  // Such a placeholder must never make the card a Production/Cutting CB.
...
function matchingOnlyStockCards(query = "") {
  const ids = [...new Set(
    matchingPurchaseRows
      .map(row => row.cb_id)
      .filter(id => isMatchingOnlyCb(id))
      .map(String)
  )];

  return ids
    .map(cbId => {
      const sourceRows = allGalleryRows.filter(row => String(row.cb_id) === cbId);
      const cbNo = sourceRows[0]?.cb_no || "Matching Stock";
      const stocks = matchingStockFor(cbId);
      const purchases = matchingPurchasesFor(cbId);
      const totalQty = stocks.reduce((sum, row) => sum + Number(row.current_qty ?? row.available_qty ?? 0), 0);
      const totalValue = stocks.reduce((sum, row) => sum + Number(row.current_value ?? 0), 0);
      const avgRate = totalQty > 0
        ? totalValue > 0
          ? totalValue / totalQty
          : Number(stocks[0]?.current_avg_rate ?? stocks[0]?.avg_cost ?? 0)
        : 0;
      return { cbId, cbNo, stocks, purchases, totalQty, totalValue, avgRate };
    })
    .filter(card => {
      if (!(currentFilter === "all" || currentFilter === "purchase")) return false;
      const haystack = [
        "matching cloth",
        card.cbNo,
        ...card.stocks.map(row => `${row.fabric_name || ""} ${row.category_name || ""}`),
        ...card.purchases.map(row => `${row.vendor_name || ""} ${row.vendor_bill_no || ""} ${row.fabric_name || ""}`)
      ].join(" ").toLowerCase();
      return haystack.includes(String(query || "").toLowerCase());
    });
}

function matchingStockCardHtml(card) {
  const stockLines = card.stocks.length
    ? card.stocks.map(row => `
        <span>
          <small>${safe(row.fabric_name || "Matching Cloth")}</small>
          <strong>${qty(row.current_qty ?? row.available_qty ?? 0)}</strong>
        </span>
      `).join("")
    : `<span><small>Stock</small><strong>0.000 kg</strong></span>`;

  return `
    <article class="pm-work-card is-matching-stock" data-matching-stock-card="${safe(card.cbId)}">
      <div class="pm-work-caption">
        <div class="pm-caption-id-row">
          <span><small>CARD NAME</small><strong>Matching Cloth</strong></span>
          <span><small>SOURCE REF</small><strong>${safe(card.cbNo)}</strong></span>
        </div>
        <div class="pm-caption-status-row">
          <span class="pm-progress-chip is-complete">✓ Stock Only</span>
          <span class="pm-progress-chip is-next">Cutting Mapping Only</span>
        </div>
        <h3>Matching Cloth</h3>
        <p class="pm-product-line">No Art · No Print · No Cutting Lot Release</p>
        <div class="pm-caption-metrics">
          ${stockLines}
          <span><small>Weighted Avg</small><strong>${money(card.avgRate)}/kg</strong></span>
          <span><small>Stock Value</small><strong>${money(card.totalValue)}</strong></span>
        </div>
        <div class="pm-card-footer">
          <span class="pm-muted-copy">Available only as a Matching cost factor inside Cutting Lot.</span>
        </div>
      </div>
    </article>
  `;
...
function renderGallery() {
  ...
  const matchingCards = matchingOnlyStockCards(query);

  gallery.setAttribute("aria-busy", "false");
  applyCardColumns();
  if (!cards.length && !matchingCards.length) {
    gallery.innerHTML = `
      <article class="pm-empty-card">
        <div class="pm-empty-icon">＋</div>
        <h3>No Product or Matching Stock card found</h3>
        <p>Create a production CB or change the search/filter.</p>
      </article>`;
    return;
  }

  const productionHtml = cards.map(({ group, division, assignment }) => {
    const art = assignment ? artById(assignment.art_id) : null;
    const assignedPrints = assignedPrintsForDivision(division.division_id);
    const colours = coloursFor(group.cb_id);
  ...
  gallery.innerHTML = productionHtml + matchingCards.map(matchingStockCardHtml).join("");
...
function openArtAssignment(divisionId) {
  ...
  if (
    isMatchingOnlyCb(context.group.cb_id) ||
    rowExplicitlyMatchingOnly(context.division)
  ) {
    say(
      "Matching Cloth is Stock Only. Art, Print and Cutting Lot are not allowed on this card.",
      "error"
    );
    return;
  }
...
async function loadData() {
  ...
  try {
    ...
    allGalleryRows = loadedGalleryRows || [];
    purchaseRows = purchaseResult.data || [];
    matchingPurchaseRows = matchingPurchaseResult.data || [];
    galleryRows = productionGalleryRows(allGalleryRows);
...
if (purchaseIntroCopy) {
  purchaseIntroCopy.textContent =
    "New CB is always a production/regular identity. Purchase is optional. Matching Cloth is added only inside an existing Regular CB and never creates a Cutting card.";
}
...
$("addMatchingEntry")?.addEventListener("click", () => {
  if (formMode !== "append" || !activeCbId) {
    say("Open an existing Regular CB before adding Matching Cloth.", "error");
    return;
  }
...
console.info("REDZED Product Master V719.3 matching-only hard repair loaded.");
  ...
  try {
    gallery.innerHTML = `
      <article class="pm-empty-card">
        ...
        <h3>Connecting Product Master V719.3</h3>
        <p>Checking login and Universal Purchase database…</p>
      </article>`;
    say("Starting Product Master V719.3…");
    await waitForRuntime();
    await ensureOwnerAccess();
    say("");
    await withTimeout(loadData(), 30000, "Product Master database loading");
  } catch (error) {
    console.error("Product Master V719.2 boot failed:", error);
    ...
    say(`V719.3 error: ${error.message || "Product Master could not open."}`, "error");
  }
})();
})();
