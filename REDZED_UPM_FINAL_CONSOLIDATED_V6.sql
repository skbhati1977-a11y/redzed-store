...
-- ============================================================================
-- V6 REFERENCE VISUALS: Garment/Art and Print thumbnails for lot cards/forms
-- Reads existing image URLs without copying files. It checks the lot-board JSON
-- and common product/print master tables by Art No. Missing images stay empty.
-- ============================================================================
create or replace function public.rr_upm_json_image_values_v6(p_data jsonb, p_path text default '')
returns table(key_path text, image_url text)
language plpgsql immutable set search_path=public
as $$
declare k text; v jsonb; i integer;
begin
    ...
    when 'object' then
      for k,v in select * from jsonb_each(p_data) loop
        if jsonb_typeof(v)='string' then
          image_url:=trim(both '"' from v::text);
          key_path:=concat_ws('.',nullif(p_path,''),k);
          if image_url ~* '^https?://' and image_url ~* '\.(png|jpe?g|webp|gif|avif)(\?.*)?$' then return next; end if;
        elsif jsonb_typeof(v) in ('object','array') then
          return query select * from public.rr_upm_json_image_values_v6(v,concat_ws('.',nullif(p_path,''),k));
        end if;
      end loop;
    when 'array' then
      i:=0;
      for v in select value from jsonb_array_elements(p_data) loop
        return query select * from public.rr_upm_json_image_values_v6(v,concat_ws('.',nullif(p_path,''),i::text));
        i:=i+1;
      end loop;
    else null;
  end case;
...
as $$
declare
  v_doc jsonb:='{}'::jsonb; v_more jsonb; r record; v_art text:=nullif(trim(p_art_no),'');
  v_garment text; v_prints jsonb:='[]'::jsonb;
begin
  begin
    execute 'select to_jsonb(x) from public.rr_upm_lot_board_v1 x where x.canonical_lot_id::text=$1 limit 1'
      into v_doc using p_canonical_lot_id;
  exception when others then v_doc:='{}'::jsonb; end;

  if v_art is null then v_art:=coalesce(v_doc->>'art_no',v_doc->>'art_code'); end if;

  if v_art is not null then
    for r in
      select n.nspname schema_name,c.relname table_name,a.attname art_column
      from pg_class c join pg_namespace n on n.oid=c.relnamespace
      join pg_attribute a on a.attrelid=c.oid and a.attnum>0 and not a.attisdropped
      where c.relkind in ('r','p','v','m') and n.nspname='public'
        and a.attname in ('art_no','art_code','article_no','article_code')
        and (c.relname ilike '%product%' or c.relname ilike '%print%' or c.relname ilike '%art%' or c.relname ilike '%style%')
      order by case when c.relname ilike '%product%master%' then 0 when c.relname ilike '%print%master%' then 1 else 2 end
      limit 12
    loop
      begin
        execute format('select to_jsonb(t) from %I.%I t where t.%I::text=$1 limit 1',r.schema_name,r.table_name,r.art_column)
          into v_more using v_art;
        if v_more is not null then v_doc:=v_doc||jsonb_build_object(r.table_name,v_more); end if;
      exception when others then null; end;
    end loop;
  end if;

  select x.image_url into v_garment
  from public.rr_upm_json_image_values_v6(v_doc) x
  where x.key_path ~* '(garment|product|front|main|model|style|art[_ .-]?image|image[_ .-]?url)'
    and x.key_path !~* '(print|artwork|design|logo)'
  order by case when x.key_path ~* '(garment|front|main)' then 0 else 1 end
  limit 1;

  select coalesce(jsonb_agg(distinct x.image_url),'[]'::jsonb) into v_prints
  from public.rr_upm_json_image_values_v6(v_doc) x
  where x.key_path ~* '(print|artwork|design|logo|placement|screen)';

  -- Safe fallback: first discovered image becomes garment reference.
  if v_garment is null then
    select x.image_url into v_garment from public.rr_upm_json_image_values_v6(v_doc) x limit 1;
  end if;

  garment_url:=v_garment; print_urls:=coalesce(v_prints,'[]'::jsonb); return next;
end; $$;
