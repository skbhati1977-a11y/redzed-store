...
window.REDZED_PRODUCT_MASTER_VERSION = "718-MATCHING-STOCK";
...
console.info("REDZED Product Master V716 single-file boot script loaded.");
  ...
  try {
    gallery.innerHTML = `
      <article class="pm-empty-card">
        ...
        <h3>Connecting Product Master V716</h3>
        <p>Checking login and CB Purchase database…</p>
      </article>`;
    say("Starting Product Master V716…");
    await waitForRuntime();
    await ensureOwnerAccess();
    say("");
    await withTimeout(loadData(), 30000, "Product Master database loading");
  } catch (error) {
    console.error("Product Master V716 boot failed:", error);
    ...
    say(`V716 error: ${error.message || "Product Master could not open."}`, "error");
  }
})();
})();
