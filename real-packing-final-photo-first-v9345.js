(()=>{
  ...
  function fullUrl(m){return String(m?.image_url||m?.storage_path||'')}
  function thumbUrl(m){
    try{if(m?.storage_path)return db().storage.from(BUCKET).getPublicUrl(m.storage_path,{transform:{width:260,height:260,resize:'cover',quality:55}}).data.publicUrl}catch(_){}
    return fullUrl(m);
  }
  function installViewer(){
    if($('rrPhotoViewer'))return;
    const s=document.createElement('style');s.id='rrPhotoViewerStyle';s.textContent='.rr-thumb img{cursor:pointer}.rr-photo-viewer{position:fixed;inset:0;z-index:2147483200;background:#000d;display:grid;place-items:center;padding:18px}.rr-photo-viewer[hidden]{display:none}.rr-photo-viewer img{max-width:100%;max-height:86vh;object-fit:contain;border-radius:10px}.rr-photo-viewer button{position:fixed;right:16px;top:16px;border:0;border-radius:999px;background:#1f2937;color:#fff;font-size:28px;line-height:1;padding:10px 16px;font-weight:900}';document.head.appendChild(s);
    const v=document.createElement('div');v.id='rrPhotoViewer';v.className='rr-photo-viewer';v.hidden=true;v.innerHTML='<button type="button" aria-label="Close">×</button><img alt="Final garment photo">';document.body.appendChild(v);v.addEventListener('click',e=>{if(e.target===v||e.target.tagName==='BUTTON')v.hidden=true});
  }
