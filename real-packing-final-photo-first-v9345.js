(()=>{
  ...
  let photoCount=0,uploading=false,lastLot='',refreshing=false;
  ...
  function patchLayout(){
    ...
    if(!root||!pics||!rate)return false;
  ...
  async function refreshPhotos(){
    const l=lot();if(!l||refreshing)return null;
    refreshing=true;
    try{
  ...
  async function uploadBeforeApproval(e){
    ...
    finally{uploading=false;if(btn){btn.disabled=false;btn.textContent='UPLOAD SELECTED FINAL PICS'}}
  }

  async function gateRequest(e){
    const btn=e.target?.closest?.('#rrRequestRate');if(!btn)return;
    e.preventDefault();e.stopImmediatePropagation();
    try{await refreshPhotos()}catch(err){show(String(err?.message||err),'error');return}
    if(photoCount!==3){show(`Final Rate Approval se pehle 3 final garment photos mandatory. Current ${photoCount}/3.`,'error');$('rrSourcePics')?.scrollIntoView({behavior:'smooth',block:'center'});return}
    btn.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window}));
  }

  function syncLot(){
    if(!patchLayout())return;
    const l=lot();if(!l||l===lastLot)return;
    lastLot=l;refreshPhotos().catch(()=>{});
  }

  document.addEventListener('click',uploadBeforeApproval,true);
  document.addEventListener('click',e=>{if(e.target?.closest?.('#rrRequestRate'))gateRequest(e)},true);
  document.addEventListener('click',()=>setTimeout(syncLot,180),true);
  document.addEventListener('change',e=>{if(e.target?.matches?.('#rrCameraPics,#rrGalleryPics'))patchLayout()},true);
  [400,900,1600,2800].forEach(ms=>setTimeout(syncLot,ms));
})();