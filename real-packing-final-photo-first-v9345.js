(()=>{
  'use strict';
  if(window.__RR_PACKING_FINAL_PHOTO_FIRST_V9352__)return;
  window.__RR_PACKING_FINAL_PHOTO_FIRST_V9352__=true;
  ...
  function hasPackRows(){return !!(document.querySelector('#packRows tr')||document.querySelector('#packRows .fg-row')||document.querySelector('[data-box-id]'))}
  function syncRateButton(){const req=$('rrRequestRate');if(!req)return;const ready=photoCount===3&&hasPackRows();req.disabled=!ready;req.classList.toggle('rr-wait',!ready);req.textContent=ready?'REQUEST FINAL RATE APPROVAL':`REQUEST FINAL RATE APPROVAL (${photoCount}/3 PHOTOS)`}
  async function refreshPhotos(){const l=lot();if(!l||refreshing)return null;refreshing=true;try{const s=await rpc('rr_pack_media_summary_v9330',{p_lot_no:l,p_data_mode:MODE});photoCount=Number(s?.camera_count||0);const n=$('rrPicCount');if(n)n.textContent=`Final photos: ${photoCount}/3`;syncRateButton();const p=$('rrCameraPreview'),cams=(s?.media||[]).filter(x=>x.media_role==='CAMERA').slice(0,3);if(p)p.innerHTML=cams.length?cams.map((m,i)=>`<div class="rr-thumb"><span>FINAL ${i+1}</span><img loading="lazy" decoding="async" src="${previewUrl(m).replace(/"/g,'&quot;')}" alt=""><button class="rr-del" data-cam-del="${m.media_id}" data-path="${String(m.storage_path||'').replace(/"/g,'&quot;')}" type="button">×</button></div>`).join(''):`<p class="fg-muted">Abhi final garment pics nahi hain.</p>`;return s}finally{refreshing=false;syncRateButton()}}
  async function compactImage(file){
    if(!file||!/^image\//i.test(file.type||''))return file;
    const max=720,quality=0.58;
    try{
      if(file.size>12*1024*1024)throw Error('Photo bahut heavy hai. Gallery me crop/screenshot karke 12MB se kam photo select karein.');
    ...
    }catch(err){if(String(err?.message||'').includes('12MB'))throw err;return file}
  }
  async function uploadWithTimeout(path,file){
    const job=db().storage.from(BUCKET).upload(path,file,{contentType:file.type||'image/jpeg',upsert:false});
    const timeout=new Promise((_,reject)=>setTimeout(()=>reject(Error('Photo upload timeout. Network slow hai, ek chhoti photo se retry karein.')),25000));
    return Promise.race([job,timeout]);
  }
  async function uploadBeforeApproval(e){const btn=e.target?.closest?.('#rrUploadPics');if(!btn)return;e.preventDefault();e.stopImmediatePropagation();if(uploading)return;uploading=true;try{const l=lot();if(!l)throw Error('Lot select karein.');if(!hasPackRows())throw Error('Pehle Packing Algorithm/Table complete karein.');let files=[...($('rrCameraPics')?.files||[]),...($('rrGalleryPics')?.files||[])].filter(f=>/^image\//i.test(f.type||''));if(!files.length)throw Error('Camera/Gallery images select karein.');btn.disabled=true;btn.textContent='CHECKING…';show('Final photos check ho rahi hain…');await refreshPhotos();if(photoCount>=3)throw Error('3 final garment photos already saved hain. Replace ke liye pehle extra photo delete karein.');if(files.length>1){files=files.slice(0,1);show('Mobile safety: ek baar me sirf 1 photo save hogi. Baaki photos one-by-one add karein.')}if(photoCount+files.length>3)throw Error(`Total final garment photos 3 hi rahengi. Current ${photoCount}/3.`);btn.textContent='SAVING 1 PHOTO…';show('Photo mobile-safe size me optimize ho rahi hai…');const items=[];for(let i=0;i<files.length;i++){if(files[i].size>12*1024*1024)throw Error('Photo bahut heavy hai. Gallery me crop/screenshot karke 12MB se kam photo select karein.');const f=await compactImage(files[i]),path=`packing-final/${MODE}/${encodeURIComponent(l)}/${Date.now()}-${crypto.randomUUID()}.jpg`;show(`Upload ho raha hai… (${Math.max(1,Math.round(f.size/1024))} KB)`);const up=await uploadWithTimeout(path,f);if(up.error)throw up.error;const image_url=db().storage.from(BUCKET).getPublicUrl(path).data.publicUrl;items.push({media_role:'CAMERA',variant_no:photoCount+i+1,image_url,storage_path:path,caption:'[CAMERA] Final packing image',customer_caption:'Final packing image'})}show('Photo record save ho raha hai…');const saved=await rpc('rr_pack_save_media_v9332',{p_lot_no:l,p_items:items,p_data_mode:MODE});if($('rrCameraPics'))$('rrCameraPics').value='';if($('rrGalleryPics'))$('rrGalleryPics').value='';photoCount=Number(saved?.camera_count||photoCount+items.length||0);show(`Final garment photos saved · ${photoCount}/3.`,'ok');await refreshPhotos()}catch(err){show(String(err?.name==='AbortError'?'Photo server response timeout. Retry karein.':err?.message||err),'error')}finally{uploading=false;if(btn){btn.disabled=false;btn.textContent='SAVE SELECTED FINAL PICS'}syncRateButton()}}
  ...
  document.addEventListener('click',uploadBeforeApproval,true);document.addEventListener('click',gateRequest,true);document.addEventListener('click',()=>setTimeout(syncLot,180),true);document.addEventListener('change',e=>{if(e.target?.matches?.('#rrCameraPics,#rrGalleryPics')){patchLayout();show('Photo selected. SAVE SELECTED FINAL PICS dabayein.')}},true);setInterval(syncRateButton,1200);[100,300,600,1000,1600,2800].forEach(ms=>setTimeout(()=>{installBootBridges();syncLot()},ms));
})();
