(()=>{
  ...
  async function refreshPhotos(){const l=lot();if(!l||refreshing)return null;refreshing=true;try{const s=await rpc('rr_pack_media_summary_v9330',{p_lot_no:l,p_data_mode:MODE});photoCount=Number(s?.camera_count||0);const complete=photoCount===3,n=$('rrPicCount');if(n)n.textContent=`Final photos: ${photoCount}/3`;const req=$('rrRequestRate');if(req)req.disabled=!complete||!document.querySelector('#packRows tr');[$('rrCameraPics'),$('rrGalleryPics')].forEach(x=>{const field=x?.closest?.('.fg-field');if(field)field.hidden=complete});if($('rrUploadPics'))$('rrUploadPics').hidden=complete;const p=$('rrCameraPreview'),cams=(s?.media||[]).filter(x=>x.media_role==='CAMERA').slice(0,3);if(p)p.innerHTML=cams.length?cams.map((m,i)=>`<div class="rr-thumb"><span>FINAL ${i+1}</span><img src="${String(m.image_url||m.storage_path||'').replace(/"/g,'&quot;')}" alt=""><button class="rr-del" data-cam-del="${m.media_id}" data-path="${String(m.storage_path||'').replace(/"/g,'&quot;')}" type="button">×</button></div>`).join(''):`<p class="fg-muted">Abhi final garment pics nahi hain.</p>`;return s}finally{refreshing=false}}
