(() => {
"use strict";
window.REDZED_ROLE_PERMISSION_VERSION="773.0.0";
...
function witnessCandidates(physicalWorkerId){
  return (state.data?.workers||[]).filter(w=>{
    ...
    return w.is_active!==false
      ...
      && identity?.identity_status==="LINKED_SELF_LOGIN";
  });
}
...
function openWorkerSkills(workerId){const worker=(state.data?.workers||[]).find(x=>String(x.worker_id)===String(workerId));if(!worker)return;$("skillWorkerId").value=workerId;$("workerSkillsTitle").textContent=`${worker.worker_name} · Departments / Skills`;const current=workerSkills(workerId);const primary=current.find(x=>x.is_primary)?.department_code||worker.department_code;$("workerSkillRows").innerHTML=allDepartments().filter(d=>d.worker_assignment_enabled!==false).map(d=>{const selected=current.some(x=>x.department_code===d.department_code)||d.department_code===worker.department_code;return `<article class="card"><div class="row between"><label class="check"><input class="skill-enabled" data-dept="${safe(d.department_code)}" type="checkbox" ${selected?"checked":""}> ${safe(d.department_name)}</label><label class="check"><input class="skill-primary" name="skillPrimary" data-dept="${safe(d.department_code)}" type="radio" ${d.department_code===primary?"checked":""}> Primary</label></div><small class="muted">${safe(d.department_code)} · ${safe(d.department_type||"PRODUCTION")}</small></article>`}).join("");document.querySelectorAll(".skill-primary").forEach(r=>r.onchange=()=>{if(r.checked){const check=document.querySelector(`.skill-enabled[data-dept="${CSS.escape(r.dataset.dept)}"]`);if(check)check.checked=true}});say("","info","workerSkillsMessage");openSheet("workerSkillsSheet")}
async function saveWorkerSkills(event){event.preventDefault();const button=event.submitter;busy(button,true,"Saving…");try{const rows=[...document.querySelectorAll(".skill-enabled")].filter(x=>x.checked).map(x=>({department_code:x.dataset.dept,is_primary:Boolean(document.querySelector(`.skill-primary[data-dept="${CSS.escape(x.dataset.dept)}"]`)?.checked),is_active:true}));if(!rows.length)throw new Error("Select at least one department.");if(rows.filter(x=>x.is_primary).length!==1)throw new Error("Exactly one Primary department required.");await rpc("rr_owner_set_worker_departments_v1",{p_worker_id:$("skillWorkerId").value,p_rows:rows});closeSheet("workerSkillsSheet");await loadConsole({quiet:true});showTab("workers");say("Worker departments saved. UPM selectors अब primary + additional skills दोनों पढ़ेंगे.","success")}catch(e){say(err(e),"error","workerSkillsMessage")}finally{busy(button,false)}}


function loginOptionsV773(selected=""){
  return `
    <option value="">Select active Login User</option>
    ${activeLoginUsers().map(u=>`
...
function witnessOptionsV773(workerId,selected=""){
  return `
    <option value="">Select active Self-Login Witness</option>
    ${witnessCandidates(workerId).map(w=>`
      <option value="${safe(w.worker_id)}"
        ${String(w.worker_id)===String(selected)?"selected":""}>
        ${safe(w.worker_name)} · ${safe(w.worker_code||"")} ·
        ${safe(departmentLabel(w.department_code))}
      </option>
    `).join("")}
  `;
}
...
async function saveWorkerIdentityV773(event){
  ...
  try{
    ...
    if(deviceMode==="SMARTPHONE"){
      ...
      if(
        ...
        await rpc("rr_unlink_worker_login_v770",{
          p_worker_id:workerId,
          p_reason:"V773 Login replacement before explicit link"
        });
      }
      ...
      await rpc("rr_link_worker_login_v770",{
        ...
        p_reason:"Role & Permission V773 Owner/Admin verified Login Link"
      });
    }else{
      const witnessId=$("identityWitnessWorker").value;

      if(!witnessId){
        throw new Error(
          `${deviceMode} worker ke liye active Self-Login Witness select karein.`
        );
      }

      await rpc("rr_assign_worker_witness_v770",{
        p_physical_worker_id:workerId,
        p_witness_worker_id:witnessId,
        p_reason:"Role & Permission V773 verified Witness assignment"
      });
    }
    ...
    showTab("workers");
    say("Worker Device / Login / Witness identity saved.","success");
  }catch(e){
...
function bindDepartmentButtons(){document.querySelectorAll("[data-edit-department]").forEach(b=>b.onclick=()=>openDepartmentEditor(b.dataset.editDepartment));document.querySelectorAll("[data-department-status]").forEach(b=>b.onclick=()=>changeDepartmentStatus(b.dataset.department,b.dataset.departmentStatus,b))}
...
function bind(){document.querySelectorAll("[data-close]").forEach(x=>x.onclick=()=>closeSheet(x.dataset.close));$("tabs").querySelectorAll("button").forEach(b=>b.onclick=()=>showTab(b.dataset.tab));$("impactNo").onclick=()=>setImpactMode("NON_IMPACT");$("impactYes").onclick=()=>setImpactMode("IMPACT");$("addRoute").onclick=()=>{state.routes=collectRoutes();state.routes.push({target_module_code:"printing",relation_mode:"VIEW_REFERENCE"});renderRoutes()};$("fieldForm").onsubmit=saveFieldDraft;$("runDemo").onclick=runDemo;$("activateField").onclick=activateField;$("suspendField").onclick=suspendField;$("revokeField").onclick=revokeField;$("userForm").onsubmit=createUser;$("workerForm").onsubmit=createManualWorker;$("departmentForm").onsubmit=saveDepartment;$("workerSkillsForm").onsubmit=saveWorkerSkills;$("workerIdentityForm").onsubmit=saveWorkerIdentityV773;$("identityDeviceMode").onchange=updateIdentityModeFieldsV773;$("unlinkWorkerLogin").onclick=unlinkWorkerLoginV773;document.addEventListener("keydown",e=>{if(e.key==="Escape"){const sheet=document.querySelector(".sheet:not(.hidden)");if(sheet)closeSheet(sheet.id)}})}
