...
const qualityNumbers = row => {
  const main = num(row.cutting_qty);
  const alter = num(row.alter_open_qty ?? row.alter_qty);
  const remake = num(row.remake_open_qty ?? row.remake_qty);
  const damage = num(row.damage_qty);
  const good = Math.max(0, main - alter - remake - damage);
  return {main, good, alter, remake, damage};
};
...
function renderSummary() {
  const totals = arr(state.context?.rows).reduce((sum, row) => {
    ...
    sum.remake += q.remake;
    sum.damage += q.damage;
    sum.ready += num(row.submit_ready_qty);
    sum.outbound += num(row.outbound_qty);
    return sum;
  }, {main:0,good:0,alter:0,remake:0,damage:0,ready:0,outbound:0});
  $("summary").innerHTML = [
  ...
  $("colours").innerHTML = colourGroups.map(group => {
    ...
    return `<article class="colour-card ${assigned ? "assigned" : ""} ${done ? "done" : ""} ${!assigned && !canAssign ? "waiting" : ""}" data-colour-key="${esc(rowKey(group))}">
      ...
      <div class="size-wrap"><table>
        <thead><tr><th>Size</th><th>Main Qty</th><th>Good Qty</th><th>Alter Pending</th><th>Remake Pending</th><th>Damage</th><th>Alter Fill</th><th>Remake Assign</th><th>Remake Submit</th><th>Damage Qty</th><th>Damage From</th><th>Status</th></tr></thead>
        <tbody>${group.rows.map((row, rowIndex) => {
          const enabled = assigned && !done;
          const q = qualityNumbers(row);
          const simpleStatus = q.remake > 0 ? "REMAKE PENDING" : q.alter > 0 ? "ALTER PENDING" : q.damage === q.main && q.main > 0 ? "FULL DAMAGE" : num(row.submit_ready_qty) > 0 ? "READY SUBMIT" : esc(row.status || "READY");
          return `<tr data-row-index="${group.indexes[rowIndex]}">
            ...
            <td>${q.remake}</td>
            <td>${q.damage}</td>
            <td><input class="alterEntry" type="number" min="0" max="${q.good}" value="0" ${enabled && q.good > 0 ? "" : "disabled"}></td>
            <td><input class="remakeIssueEntry" type="number" min="0" max="${q.alter}" value="0" ${enabled && q.alter > 0 ? "" : "disabled"}></td>
            <td><input class="remakeCompleteEntry" type="number" min="0" max="${q.remake}" value="0" ${enabled && q.remake > 0 ? "" : "disabled"}></td>
...
function buildActionRows(actionType, inputClass) {
  ...
  selected.forEach(({row, tableRow}) => {
    ...
    const alterOpen = q.alter;
    const remakeOpen = q.remake;
    let sourceBucket = "PENDING";
    let maximum = Math.min(q.good, pending);
...
async function boot() {
  try {
    ...
    $("alterBtn").onclick = () => applyAction("ALTER", "alterEntry", "Alter saved. Alter Pending and Good Qty updated automatically.");
    $("remakeIssueBtn").onclick = () => applyAction("REMAKE_ISSUE", "remakeIssueEntry", "Remake assigned from Alter. Alter Pending reduced automatically.");
    $("remakeCompleteBtn").onclick = () => applyAction("REMAKE_COMPLETE", "remakeCompleteEntry", "Remake submitted. Remake Pending reduced and Good Qty increased automatically.");
    $("damageBtn").onclick = () => applyAction("DAMAGE", "damageEntry", "Damage saved. Selected bucket and Good Qty updated automatically.");
