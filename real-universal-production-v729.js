...
async function saveAlterEvidence() {
  ...
  const result = await rpc("rr_upm_alter_fill_v729", {
    p_canonical_lot_id: state.lot.canonical_lot_id,
    p_department_code: $("dept").value,
...
async function boot() {
  try {
    ...
    $("remakeDeliveredBtn").onclick = () => runRemakeStage("REMAKE_DELIVERED", "remakeIssueEntry", "Remake Delivered saved; responsibility moved to Worker/Karigar.");
