...
window.REDZED_PRODUCT_MASTER_VERSION = "716-CB-PRINT-FIX";
...
function materialName(entry) {
  return categoryById(entry.materialCategoryId)?.category_name ||
    "Material";
}
...
function showFrontendError(context, error) {
  ...
  panel.innerHTML = `
    ...
    <strong>
      REDZED ERROR —
      ${escapeHtml(context || "Product Master")}
    </strong>
  ...
  panel
    .querySelector("[data-close-pm-error]")
...
function entryRolls(entry) {
  ...
  entry.colours.forEach(
    (
      colour,
      colourIndex
    ) => {
      colour.rolls.forEach(
        (
          roll,
          rollIndex
        ) => {
...
function updateGrandSummary() {
  ...
  $("grandQty").textContent =
    qty(totalQty);

  $("grandAmount").textContent =
    money(totalAmount);
}
...
function groupFor(cbId) {
  return (
    groupGalleryRows().find(
      group => String(group.cb_id) === String(cbId)
    ) || null
  );
}
...
function allocationsForDivision(divisionId) {
  return allocationRows.filter(
    row => String(row.division_id) === String(divisionId)
  );
}

function purchaseById(id) {
  return (
    purchaseRows.find(row => String(row.id) === String(id)) ||
    null
  );
}
  ...
  if (!items.length) {
    return `
      <div class="pm-art-placeholder">
        ...
        <small>Photo will pull from Master</small>
      </div>
    `;
  }
  ...
  return `
    ...
    ${
      items.length > 1
        ? `
          <button
            class="pm-carousel-nav pm-carousel-prev"
            type="button"
          ...
          <button
            class="pm-carousel-nav pm-carousel-next"
            type="button"
            data-carousel-move="1"
...
function bindCarousels(root) {
  root
    .querySelectorAll("[data-carousel-move]")
    .forEach(button => {
      button.addEventListener("click", event => {
        event.preventDefault();
        event.stopPropagation();

        const holder = button.closest(
          ".pm-card-hero,.pm-selected-carousel"
        );

        const slides = [
          ...holder.querySelectorAll(".pm-swipe-slide")
        ];
        ...
        const next =
          (
            current +
            Number(button.dataset.carouselMove || 1) +
            slides.length
          ) % slides.length;
        ...
        const counter = holder.querySelector(
          "[data-carousel-count]"
        );
  ...
  root
    .querySelectorAll(".pm-swipe-track")
    .forEach(track => {
      let startX = null;
...
function applyCardColumns() {
  ...
  if ($("pmCardColumns")) {
    $("pmCardColumns").value = String(cardColumnCount);
  }
}

function renderGallery() {
  const query = $("pmSearch")
    .value
    .trim()
    .toLowerCase();
  ...
  const cards = divisionCards().filter(
    ({ group, division, assignment }) => {
      ...
      if (currentFilter === "purchase") {
        ...
        currentFilter === "ready_for_cutting"
      ) {
  ...
  if (!cards.length) {
    gallery.innerHTML = `
      <article class="pm-empty-card">
        ...
        <p>
          Create a CB or change the search/filter.
        </p>
      </article>
    `;
      ...
      return `
        ...
        >
          <div class="pm-card-hero">
            ...
            <span>
              <small>CB NO</small>
              <strong>${safe(group.cb_no)}</strong>
            </span>

            <span>
              <small>CB CHILD</small>
              <strong>
                ${safe(canonicalCbChildCode(division))}
              </strong>
            </span>
          </div>
          ...
          <div class="pm-card-footer">
            <button
              class="pm-open-cb"
              type="button"
              data-open-parent="${safe(group.cb_id)}"
            >
              CB Details
            </button>
  ...
  gallery
    .querySelectorAll("[data-open-art]")
    .forEach(card => {
      const open = event => {
...
function divisionContext(divisionId) {
  for (const group of groupGalleryRows()) {
    const division = group.divisions.find(
      row =>
    ...
    if (division) {
      return {
        ...
        division,
        assignment:
          assignmentForDivision(divisionId)
      };
    }
  }
...
function installPickerInteractionStyles() {
  if (
    document.getElementById(
      "pmPickerInteractionStyles"
    )
  ) {
    return;
  }
  ...
  style.textContent = `
    ...
    #artPickerGrid.pm-hidden,
    #printPickerGrid.pm-hidden,
    #printPickerActionBar.pm-hidden {
    ...
    .pm-print-action-status {
      min-width: 0;
    }

    .pm-print-action-status small {
      display: block;
      margin-bottom: 3px;
      color: var(--muted, #aaa);
      font-size: 10px;
    ...
    .pm-print-done-btn {
      border: 1px solid #b32d39;
      background: #ef3340;
      color: #fff;
    }

    .pm-print-done-btn:disabled {
      cursor: not-allowed;
      opacity: .48;
    }
  `;
...
function renderArtPicker() {
  const query = $("artSearch")
    .value
    .trim()
    .toLowerCase();

  const list = artRows.filter(
    art => {
      const haystack = [
        art.art_no,
        art.product_name,
    ...
    list.length
      ...
      : `
        <div class="pm-art-empty">
          No matching Art found in Art Master.
        </div>
      `;
...
function selectedRealPrintIds() {
  return selectedPrintIds.filter(
    id =>
      String(id) !==
      "__PRINT_NA__"
  );
...
function printDecisionForDivision(
  divisionId
) {
  const assignment =
...
function ensurePrintPickerActionBar() {
  let bar =
    $("printPickerActionBar");
  ...
  bar.innerHTML = `
    <div class="pm-print-action-status">
      <small>
        PRINT SELECTION
      </small>

      <strong id="printApplyStatus">
        Print Due
      </strong>
    </div>

    <div class="pm-print-action-buttons">
      <button
        id="clearPrintSelection"
        class="pm-print-clear-btn"
        type="button"
      >
        Clear
      </button>
      ...
      <button
        id="applyPrintSelection"
        class="pm-print-done-btn"
        type="button"
      >
        Done — Apply Prints
      </button>
  ...
  $("clearPrintSelection")
    .addEventListener(
      ...
      () => {
        ...
        artAssignmentSay(
          "Print selection cleared. Status is Print Due."
        );
      }
    );
    ...
    .addEventListener(
      ...
      () => {
        const printNotApplicable =
          isPrintNotApplicableSelection();

        const count =
          selectedRealPrintIds()
            .length;
        ...
        if (
          ...
          artAssignmentSay(
            "No Print selected. Select Print, or explicitly choose N/A.",
            "error"
          );
        ...
        artAssignmentSay(
          printNotApplicable
            ? "N/A — No Print Required applied. Press Save Art & Print for final database save."
            : `${count} Print${
                count === 1
                  ? ""
                  : "s"
              } applied. Press Save Art & Print for final database save.`,
          "success"
        );
      }
    );
...
function updatePrintPickerActionBar() {
  ensurePrintPickerActionBar();

  const printNotApplicable =
    isPrintNotApplicableSelection();

  const count =
    selectedRealPrintIds().length;
  ...
  $("printPickerGrid").innerHTML = `
    ...
    >
      ...
      >
        ...
        <em>
          Use only when this Art genuinely needs no print.
        </em>

        <b>
          Empty selection remains Print Due
        </b>
      </span>
    </button>
          ...
          printNotApplicable
            ? `
              <span>
                ...
                <em>
                  Explicit final no-print decision
                </em>
              </span>
            `
            : prints.length
              ...
              : `
                <span class="is-empty">
                  ...
                  <em>
                    Select Print, or explicitly choose N/A.
                    </em>
                </span>
              `
        }
...
function openArtAssignment(
  ...
  installPickerInteractionStyles();
  ensurePrintPickerActionBar();
...
async function saveArtPrintAssignmentRecord(
  ...
  console.warn(
    "V716 RPC unavailable or explicit N/A selected; using owner direct-write fallback.",
    rpc.error
  );
...
async function loadCbPrintAssignments() {
  ...
  if (result.error) {
    throw new Error(
      `Run V715 separate Art/Print SQL: ${
...
async function loadData() {
  ...
  try {
    ...
    if (artResult.error) {
      throw new Error(
        `Art Master could not load: ${
          artResult.error.message
        }`
      );
    }
...
console.info(
  "REDZED Product Master V716 CB + Print multi-select workflow loaded."
);
  ...
  try {
    gallery.innerHTML = `
      ...
      >
        ...
        <p>
          Checking login and CB Purchase database…
        </p>
      </article>
    `;
