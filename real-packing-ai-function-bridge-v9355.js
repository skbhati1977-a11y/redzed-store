...
(()=>{
  ...
  function boxType(box){const type=String(box.box_type||'').toUpperCase(),cells=box.cells||[];if(cells.some(x=>String(x.pack_mark||'').toUpperCase()==='MIX'))return 'MIX';if(cells.some(x=>String(x.pack_mark||'').toUpperCase()==='ASST'))return 'ASST';if(cells.some(x=>String(x.pack_mark||'').toUpperCase()==='FRESH'))return 'FRESH';return type==='REGULAR'?'FRESH':(type||'ASST');}
