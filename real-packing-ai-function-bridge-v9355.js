...
(()=>{
  ...
  function normalizeMessage(){const m=$('message');if(!m)return;const t=String(m.textContent||'');if(/Re-run allowed before submit/i.test(t))setMsg('Algorithm table loaded. Ab final photos, rate approval aur 3 AI final images complete karke submit karein.','ok');}
  ...
  function render(detail,status){
    ...
    if(block)block.hidden=false;submit.disabled=String(status||'').toUpperCase()==='SUBMITTED';return true;
  }
  async function loadAssignment(lot){const c=sb();if(!c||!lot)return null;const r=await c.from('rr_fg_packing_assignments_v788').select('id,lot_no,status,pack_plan_id').eq('data_mode','TEST').ilike('lot_no',lot).in('status',['ACCEPTED','SUBMITTED']).limit(1);if(r.error)throw r.error;return (r.data||[])[0]||null;}
  async function recover(reason=''){
    if(busy)return;const lot=selectedLot();if(!lot)return;busy=true;
    try{const a=await loadAssignment(lot);if(!a?.pack_plan_id)return;const d=await sb().rpc('rr_fg_pack_plan_detail_v787',{p_plan_id:a.pack_plan_id});if(d.error)throw d.error;if(render(d.data,a.status)){const stuck=/Equal packing algorithm chal raha/i.test(String($('message')?.textContent||''));if(stuck||reason)setMsg('Algorithm table loaded. Ab final photos, rate approval aur 3 AI final images complete karke submit karein.','ok');}}
    catch(e){setMsg(e.message||String(e),'error')}finally{busy=false;}
  }
  function needsRecover(){const text=String($('message')?.textContent||''),rows=String($('packRows')?.innerHTML||'').trim();return /Equal packing algorithm chal raha/i.test(text)||(!rows&&selectedLot());}
  document.addEventListener('click',e=>{if(e.target?.closest?.('#runPackAlgo'))setTimeout(()=>{if(needsRecover())recover('run-timeout');},8000);},true);
  const start=()=>{if(!document.body)return;new MutationObserver(()=>{normalizeMessage();if(needsRecover())setTimeout(()=>recover('observer'),1200);}).observe(document.body,{childList:true,subtree:true,characterData:true});setInterval(()=>{normalizeMessage();if(needsRecover())recover('interval');},2500);};
