(() => {
  ...
  RR.requireRoles = async (allowedRoles=["owner","admin"]) => {
    ...
    const {data:profile,error:profileError}=await supabaseClient.from("rr_user_profiles").select("id, full_name, role_code, is_active").eq("auth_user_id",user.id).single();
    const role=String(profile?.role_code||"").trim().toLowerCase();
    if(profileError||!profile?.is_active||!allowed.has(role)) throw new Error(`Required permission: ${[...allowed].join(" / ")}.`);
  ...
  RR.getTableColumns = async (table) => {
    const knownColumns={rr_art_master:["id","art_no","item_name","product_name","description","default_margin","is_active","created_at"],rr_art_costs:["id","art_id","cutting_rate","printing_rate","sticker_rate","kr_rate","ov_rate","fld_rate","thread_cut_rate","press_rate","packing_rate","other_rate","created_at"],rr_media:["id","entity_type","entity_id","media_category","file_url","storage_path","file_name","mime_type","caption","source_type","visibility_scope","is_cover","sort_order","created_at"]};
