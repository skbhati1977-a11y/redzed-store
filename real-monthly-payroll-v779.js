(() => {
"use strict";
window.REAL_FACTORY_MONTHLY_PAYROLL_VERSION="779.7.1";
const REAL_FACTORY_PAYROLL_DATA_MODE="TEST";
window.REAL_FACTORY_PAYROLL_DATA_MODE=REAL_FACTORY_PAYROLL_DATA_MODE;
...
async function loadMy(){try{say("Loading payroll…");const data=await rpc("rr_get_my_payroll_history_v779_3",{p_limit:24,p_data_mode:REAL_FACTORY_PAYROLL_DATA_MODE});state.history=data?.payroll_history||[];$("tab-my").innerHTML=`<div class="toolbar"><button id="refreshMy" class="btn">Refresh</button></div><div class="list">${state.history.map(payrollCard).join("")||'<div class="panel empty">Abhi koi Monthly Payroll record nahi hai.</div>'}</div>`;$("refreshMy").onclick=loadMy;bindPayrollButtons();say("")}catch(e){console.error(e);$("tab-my").innerHTML=`<div class="panel empty">${safe(err(e))}</div>`;say(err(e),"error")}}
...
async function generateAllPayroll(){
  ...
  try{
    const d=await rpc("rr_generate_monthly_payroll_batch_v779_5",{
      ...
      p_data_mode:REAL_FACTORY_PAYROLL_DATA_MODE,
...
async function generateLegacyPayroll(workerId){
  ...
  try{
    await rpc("rr_generate_worker_monthly_payroll_legacy_v779_5",{
      ...
      p_reason:reason.trim(),p_data_mode:REAL_FACTORY_PAYROLL_DATA_MODE
    });
...
async function generateAllLegacyPayroll(){
  ...
  try{
    const d=await rpc("rr_generate_monthly_payroll_legacy_batch_v779_5",{
      p_payroll_month:state.month,p_reason:reason.trim(),p_data_mode:REAL_FACTORY_PAYROLL_DATA_MODE
    });
...
async function loadManagement(){
  ...
  try{
    ...
    const d=normalizeManagementBoard(await rpc("rr_get_payroll_management_board_v779_5",{p_payroll_month:state.month,p_data_mode:REAL_FACTORY_PAYROLL_DATA_MODE}));
    state.management=d;
