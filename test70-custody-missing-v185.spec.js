...
for(const text of ["PHYSICAL CUSTODY OWNER · LINE MAN","rr_upm_ready_to_assign_with_custody_v185"])
  assert.ok(dept.includes(text),`Missing App/Chat assignment mirror: ${text}`);
for(const text of ["RECEIVE GOODS","PHYSICALLY RECEIVED GOOD PCS","rr_upm_confirm_assignment_receipt_v9112"])
