-- REDZED ERP UPM V724 — Working Universal Lot Form fix
-- Fixes V723 cutting map source, size rows, worker list, assignment/production summary.
begin;

create or replace function public.rr_upm_universal_form_v724(
...
begin
  ...
  with cutting_lot as (
    ...
    select
      upper('C'||coalesce(c.col_no::text,'0')) colour_code,
  ...
  ) order by regexp_replace(s.colour_code,'[^0-9]','','g')::int,
...
grant execute on function public.rr_upm_universal_form_v724(text,text) to authenticated;
commit;
