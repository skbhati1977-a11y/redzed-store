/* V9309/V9344 — locked FG Packing algorithm + manager behalf + Ready Lots REST bridge + packing final pic AI engine loader. */
(()=>{
  'use strict';
  ...
  function installReadyLotsBridge(){
    ...
    c.rpc=async function(name,args,options){
      ...
      try{
        ...
        console.warn('V9344 Ready Lots REST bridge fallback',error);
  ...
  function retryReadyLotsIfStuck(){
    if(!installReadyLotsBridge())return;
  ...
  function loadPicEngine(){if(window.__RR_PACKING_PIC_ENGINE_V9330__||document.getElementById('rrPackingPicEngineV9330'))return;if(!/real-finished-goods-v787\.html/i.test(location.pathname))return;const s=document.createElement('script');s.id='rrPackingPicEngineV9330';s.src='/redzed-store/real-packing-pic-engine-v9330.js?v=9332';s.async=false;(document.head||document.documentElement).appendChild(s);}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    obs.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','disabled']});
    patch();
    [500,1200,2500].forEach(ms=>setTimeout(retryReadyLotsIfStuck,ms));
