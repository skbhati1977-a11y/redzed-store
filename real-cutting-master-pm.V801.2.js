...
async function loadCuttingWorkerSource(client) {
  const result = await client.rpc("rr_upm_worker_list_v8_3", {
    p_department_code: "CUTTING"
  });

  if (result.error) {
    throw new Error(`Cutting Master backend worker mapping: ${errorText(result.error)}`);
  }
...
function refreshCuttingWorkerControl() {
  ...
  if (!String(input.value || "").trim() && workers.length === 1) {
    input.value = cuttingWorkerOptionValue(workers[0]);
  }
}
...
async function loadAllData() {
  ...
  const [
    ...
    loadCuttingWorkerSource(client),
...
loadMatchingLotSource(client)
  ...
  console.info("REAL FACTORY Cutting Master PM Core V801.2 loaded", {
