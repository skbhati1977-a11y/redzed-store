...
function render(){document.querySelectorAll('.rr-alter-queue-compact').forEach(e=>e.remove());for(const g of cache){const row=findColourRow(findLotCard(g.lot),g.colour);if(!row)continue;const box=document.createElement('div');box.className='rr-alter-queue-compact';box.innerHTML=`<span class="rr-aqc-sizes">${esc(g.sizes)}</span><span class="rr-aqc-owner">${esc(g.ownerLine)}</span>`;row.appendChild(box);}}
const style=document.createElement('style');style.textContent='.rr-alter-queue-compact{width:100%;margin-top:7px;padding-top:6px;border-top:1px solid #ffffff24;font-size:12px;line-height:1.25;font-weight:850;color:#f3f5f8;display:flex;align-items:center;gap:8px;white-space:nowrap;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch}.rr-alter-queue-compact .rr-aqc-sizes,.rr-alter-queue-compact .rr-aqc-owner{display:inline-block;flex:0 0 auto;white-space:nowrap}.rr-alter-queue-compact .rr-aqc-owner{font-size:11px;font-weight:750;color:#cbd3df}';document.head.appendChild(style);
