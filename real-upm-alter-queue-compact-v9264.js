...
async function load(){
 ...
 try{
  ...
  for(const lot of lots){try{const r=await c.rpc('rr_upm_lot_colour_matrix_v755',{p_canonical_lot_id:null,p_lot_no:lot});if(!r.error)for(const x of r.data?.colours||[]){const a=x.alter_journey;if(a?.qty>0)out.push({lot,colour:x.colour_code,sizes:clean(a.compact_line||a.size_details||''),ownerLine:clean(a.compact_owner_line||'')});}}catch(_){}}
  cache=out;render();
